package libcore.io;

import dalvik.bytecode.Opcodes;
import java.nio.ByteOrder;

public final class Memory {
    public static native void memmove(Object obj, int i, Object obj2, int i2, long j);

    public static native byte peekByte(long j);

    public static native void peekByteArray(long j, byte[] bArr, int i, int i2);

    public static native void peekCharArray(long j, char[] cArr, int i, int i2, boolean z);

    public static native void peekDoubleArray(long j, double[] dArr, int i, int i2, boolean z);

    public static native void peekFloatArray(long j, float[] fArr, int i, int i2, boolean z);

    public static native void peekIntArray(long j, int[] iArr, int i, int i2, boolean z);

    private static native int peekIntNative(long j);

    public static native void peekLongArray(long j, long[] jArr, int i, int i2, boolean z);

    private static native long peekLongNative(long j);

    public static native void peekShortArray(long j, short[] sArr, int i, int i2, boolean z);

    private static native short peekShortNative(long j);

    public static native void pokeByte(long j, byte b);

    public static native void pokeByteArray(long j, byte[] bArr, int i, int i2);

    public static native void pokeCharArray(long j, char[] cArr, int i, int i2, boolean z);

    public static native void pokeDoubleArray(long j, double[] dArr, int i, int i2, boolean z);

    public static native void pokeFloatArray(long j, float[] fArr, int i, int i2, boolean z);

    public static native void pokeIntArray(long j, int[] iArr, int i, int i2, boolean z);

    private static native void pokeIntNative(long j, int i);

    public static native void pokeLongArray(long j, long[] jArr, int i, int i2, boolean z);

    private static native void pokeLongNative(long j, long j2);

    public static native void pokeShortArray(long j, short[] sArr, int i, int i2, boolean z);

    private static native void pokeShortNative(long j, short s);

    public static native void unsafeBulkGet(Object obj, int i, int i2, byte[] bArr, int i3, int i4, boolean z);

    public static native void unsafeBulkPut(byte[] bArr, int i, int i2, Object obj, int i3, int i4, boolean z);

    private Memory() {
    }

    public static int peekInt(byte[] src, int offset, ByteOrder order) {
        if (order == ByteOrder.BIG_ENDIAN) {
            int offset2 = offset + 1;
            int i = (src[offset] & Character.DIRECTIONALITY_UNDEFINED) << 24;
            int offset3 = offset2 + 1;
            return i | ((src[offset2] & Character.DIRECTIONALITY_UNDEFINED) << 16) | ((src[offset3] & Character.DIRECTIONALITY_UNDEFINED) << 8) | ((src[offset3 + 1] & Character.DIRECTIONALITY_UNDEFINED) << 0);
        }
        int offset4 = offset + 1;
        int i2 = (src[offset] & Character.DIRECTIONALITY_UNDEFINED) << 0;
        int offset5 = offset4 + 1;
        return i2 | ((src[offset4] & Character.DIRECTIONALITY_UNDEFINED) << 8) | ((src[offset5] & Character.DIRECTIONALITY_UNDEFINED) << 16) | ((src[offset5 + 1] & Character.DIRECTIONALITY_UNDEFINED) << 24);
    }

    public static long peekLong(byte[] src, int offset, ByteOrder order) {
        if (order == ByteOrder.BIG_ENDIAN) {
            int offset2 = offset + 1;
            int i = (src[offset] & Character.DIRECTIONALITY_UNDEFINED) << 24;
            int offset3 = offset2 + 1;
            int i2 = i | ((src[offset2] & Character.DIRECTIONALITY_UNDEFINED) << 16);
            int offset4 = offset3 + 1;
            int i3 = i2 | ((src[offset3] & Character.DIRECTIONALITY_UNDEFINED) << 8);
            int offset5 = offset4 + 1;
            int h = i3 | ((src[offset4] & Character.DIRECTIONALITY_UNDEFINED) << 0);
            int offset6 = offset5 + 1;
            int i4 = (src[offset5] & Character.DIRECTIONALITY_UNDEFINED) << 24;
            int offset7 = offset6 + 1;
            int l = i4 | ((src[offset6] & Character.DIRECTIONALITY_UNDEFINED) << 16) | ((src[offset7] & Character.DIRECTIONALITY_UNDEFINED) << 8) | ((src[offset7 + 1] & Character.DIRECTIONALITY_UNDEFINED) << 0);
            return (((long) h) << 32) | (((long) l) & 4294967295L);
        }
        int offset8 = offset + 1;
        int i5 = (src[offset] & Character.DIRECTIONALITY_UNDEFINED) << 0;
        int offset9 = offset8 + 1;
        int i6 = i5 | ((src[offset8] & Character.DIRECTIONALITY_UNDEFINED) << 8);
        int offset10 = offset9 + 1;
        int i7 = i6 | ((src[offset9] & Character.DIRECTIONALITY_UNDEFINED) << 16);
        int offset11 = offset10 + 1;
        int l2 = i7 | ((src[offset10] & Character.DIRECTIONALITY_UNDEFINED) << 24);
        int offset12 = offset11 + 1;
        int i8 = (src[offset11] & Character.DIRECTIONALITY_UNDEFINED) << 0;
        int offset13 = offset12 + 1;
        int h2 = i8 | ((src[offset12] & Character.DIRECTIONALITY_UNDEFINED) << 8) | ((src[offset13] & Character.DIRECTIONALITY_UNDEFINED) << 16) | ((src[offset13 + 1] & Character.DIRECTIONALITY_UNDEFINED) << 24);
        return (((long) h2) << 32) | (((long) l2) & 4294967295L);
    }

    public static short peekShort(byte[] src, int offset, ByteOrder order) {
        return order == ByteOrder.BIG_ENDIAN ? (short) ((src[offset] << 8) | (src[offset + 1] & Character.DIRECTIONALITY_UNDEFINED)) : (short) ((src[offset + 1] << 8) | (src[offset] & Character.DIRECTIONALITY_UNDEFINED));
    }

    public static void pokeInt(byte[] dst, int offset, int value, ByteOrder order) {
        if (order == ByteOrder.BIG_ENDIAN) {
            int offset2 = offset + 1;
            dst[offset] = (byte) ((value >> 24) & Opcodes.OP_CONST_CLASS_JUMBO);
            int offset3 = offset2 + 1;
            dst[offset2] = (byte) ((value >> 16) & Opcodes.OP_CONST_CLASS_JUMBO);
            dst[offset3] = (byte) ((value >> 8) & Opcodes.OP_CONST_CLASS_JUMBO);
            dst[offset3 + 1] = (byte) ((value >> 0) & Opcodes.OP_CONST_CLASS_JUMBO);
            return;
        }
        int offset4 = offset + 1;
        dst[offset] = (byte) ((value >> 0) & Opcodes.OP_CONST_CLASS_JUMBO);
        int offset5 = offset4 + 1;
        dst[offset4] = (byte) ((value >> 8) & Opcodes.OP_CONST_CLASS_JUMBO);
        dst[offset5] = (byte) ((value >> 16) & Opcodes.OP_CONST_CLASS_JUMBO);
        dst[offset5 + 1] = (byte) ((value >> 24) & Opcodes.OP_CONST_CLASS_JUMBO);
    }

    public static void pokeLong(byte[] dst, int offset, long value, ByteOrder order) {
        if (order == ByteOrder.BIG_ENDIAN) {
            int i = (int) (value >> 32);
            int offset2 = offset + 1;
            dst[offset] = (byte) ((i >> 24) & Opcodes.OP_CONST_CLASS_JUMBO);
            int offset3 = offset2 + 1;
            dst[offset2] = (byte) ((i >> 16) & Opcodes.OP_CONST_CLASS_JUMBO);
            int offset4 = offset3 + 1;
            dst[offset3] = (byte) ((i >> 8) & Opcodes.OP_CONST_CLASS_JUMBO);
            int offset5 = offset4 + 1;
            dst[offset4] = (byte) ((i >> 0) & Opcodes.OP_CONST_CLASS_JUMBO);
            int i2 = (int) value;
            int offset6 = offset5 + 1;
            dst[offset5] = (byte) ((i2 >> 24) & Opcodes.OP_CONST_CLASS_JUMBO);
            int offset7 = offset6 + 1;
            dst[offset6] = (byte) ((i2 >> 16) & Opcodes.OP_CONST_CLASS_JUMBO);
            dst[offset7] = (byte) ((i2 >> 8) & Opcodes.OP_CONST_CLASS_JUMBO);
            dst[offset7 + 1] = (byte) ((i2 >> 0) & Opcodes.OP_CONST_CLASS_JUMBO);
            return;
        }
        int i3 = (int) value;
        int offset8 = offset + 1;
        dst[offset] = (byte) ((i3 >> 0) & Opcodes.OP_CONST_CLASS_JUMBO);
        int offset9 = offset8 + 1;
        dst[offset8] = (byte) ((i3 >> 8) & Opcodes.OP_CONST_CLASS_JUMBO);
        int offset10 = offset9 + 1;
        dst[offset9] = (byte) ((i3 >> 16) & Opcodes.OP_CONST_CLASS_JUMBO);
        int offset11 = offset10 + 1;
        dst[offset10] = (byte) ((i3 >> 24) & Opcodes.OP_CONST_CLASS_JUMBO);
        int i4 = (int) (value >> 32);
        int offset12 = offset11 + 1;
        dst[offset11] = (byte) ((i4 >> 0) & Opcodes.OP_CONST_CLASS_JUMBO);
        int offset13 = offset12 + 1;
        dst[offset12] = (byte) ((i4 >> 8) & Opcodes.OP_CONST_CLASS_JUMBO);
        dst[offset13] = (byte) ((i4 >> 16) & Opcodes.OP_CONST_CLASS_JUMBO);
        dst[offset13 + 1] = (byte) ((i4 >> 24) & Opcodes.OP_CONST_CLASS_JUMBO);
    }

    public static void pokeShort(byte[] dst, int offset, short value, ByteOrder order) {
        if (order == ByteOrder.BIG_ENDIAN) {
            dst[offset] = (byte) ((value >> 8) & Opcodes.OP_CONST_CLASS_JUMBO);
            dst[offset + 1] = (byte) ((value >> 0) & Opcodes.OP_CONST_CLASS_JUMBO);
        } else {
            dst[offset] = (byte) ((value >> 0) & Opcodes.OP_CONST_CLASS_JUMBO);
            dst[offset + 1] = (byte) ((value >> 8) & Opcodes.OP_CONST_CLASS_JUMBO);
        }
    }

    public static int peekInt(long address, boolean swap) {
        int result = peekIntNative(address);
        if (swap) {
            return Integer.reverseBytes(result);
        }
        return result;
    }

    public static long peekLong(long address, boolean swap) {
        long result = peekLongNative(address);
        if (swap) {
            return Long.reverseBytes(result);
        }
        return result;
    }

    public static short peekShort(long address, boolean swap) {
        short result = peekShortNative(address);
        if (swap) {
            return Short.reverseBytes(result);
        }
        return result;
    }

    public static void pokeInt(long address, int value, boolean swap) {
        if (swap) {
            value = Integer.reverseBytes(value);
        }
        pokeIntNative(address, value);
    }

    public static void pokeLong(long address, long value, boolean swap) {
        if (swap) {
            value = Long.reverseBytes(value);
        }
        pokeLongNative(address, value);
    }

    public static void pokeShort(long address, short value, boolean swap) {
        if (swap) {
            value = Short.reverseBytes(value);
        }
        pokeShortNative(address, value);
    }
}
