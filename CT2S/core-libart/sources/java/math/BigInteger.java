...
public class BigInteger extends Number implements Comparable<BigInteger>, Serializable {
    ...
    public BigInteger add(BigInteger value) {
        ...
        return lhs.sign() == 0 ? value : new BigInteger(BigInt.addition(lhs, rhs));
    }

    public BigInteger subtract(BigInteger value) {
        BigInt lhs = getBigInt();
        BigInt rhs = value.getBigInt();
        return rhs.sign() == 0 ? this : new BigInteger(BigInt.subtraction(lhs, rhs));
    }
    ...
    public long longValue() {
        ...
        prepareJavaRepresentation();
        long value = this.numberLength > 1 ? (((long) this.digits[1]) << 32) | (((long) this.digits[0]) & 4294967295L) : ((long) this.digits[0]) & 4294967295L;
    ...
    public boolean equals(Object x) {
        ...
        return (x instanceof BigInteger) && compareTo((BigInteger) x) == 0;
    }
    ...
    private byte[] twosComplement() {
        ...
        if (this.sign < 0) {
            ...
            while (bytesLen2 > firstByteNumber) {
                int digit2 = this.digits[digitIndex2] ^ (-1);
                digitIndex2++;
                if (digitIndex2 == this.numberLength) {
                    bytesInInteger = highBytes;
                }
    ...
    static int multiplyByInt(int[] res, int[] a, int aSize, int factor) {
        ...
        for (int i = 0; i < aSize; i++) {
            long carry2 = carry + ((((long) a[i]) & 4294967295L) * (((long) factor) & 4294967295L));
            res[i] = (int) carry2;
            carry = carry2 >>> 32;
        }
        return (int) carry;
    ...
    private static void parseFromString(BigInteger bi, String value, int radix) {
        ...
        while (true) {
            ...
            if (substrStart < stringLength) {
                int bigRadixDigit = Integer.parseInt(value.substring(substrStart, substrEnd), radix);
                int newDigit = multiplyByInt(digits, digits, digitIndex2, bigRadix);
                digitIndex = digitIndex2 + 1;
                digits[digitIndex2] = newDigit + inplaceAdd(digits, digitIndex2, bigRadixDigit);
