...
class X509Util {
    ...
    static DERObjectIdentifier getAlgorithmOID(String algorithmName) {
        ...
        return algorithms.containsKey(algorithmName2) ? (DERObjectIdentifier) algorithms.get(algorithmName2) : new DERObjectIdentifier(algorithmName2);
    }
    ...
    static Signature getSignatureInstance(String algorithm, String provider) throws NoSuchAlgorithmException, NoSuchProviderException {
        return provider != null ? Signature.getInstance(algorithm, provider) : Signature.getInstance(algorithm);
    }

    static byte[] calculateSignature(DERObjectIdentifier sigOid, String sigName, PrivateKey key, SecureRandom random, ASN1Encodable object) throws NoSuchAlgorithmException, SignatureException, InvalidKeyException, IOException {
        if (sigOid == null) {
    ...
    static byte[] calculateSignature(DERObjectIdentifier sigOid, String sigName, String provider, PrivateKey key, SecureRandom random, ASN1Encodable object) throws NoSuchAlgorithmException, SignatureException, InvalidKeyException, IOException, NoSuchProviderException {
        if (sigOid == null) {
            throw new IllegalStateException("no signature algorithm specified");
        }
