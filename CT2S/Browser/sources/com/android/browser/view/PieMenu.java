...
public class PieMenu extends FrameLayout {
    ...
    private void init(Context ctx) {
        ...
        this.mNormalPaint.setColor(res.getColor(R.color.qc_normal));
    ...
    public void layoutPie() {
        ...
        for (int i = 0; i < this.mLevels; i++) {
            int level = i + 1;
            float sweep = ((float) (3.141592653589793d - ((double) (2.0f * 0.19634955f)))) / this.mCounts[level];
            float angle = 0.19634955f + (sweep / 2.0f);
            this.mPath = makeSlice(getDegrees(0.0d) - 1, 1 + getDegrees(sweep), outer, inner, this.mCenter);
            for (PieItem item : this.mCurrentItems) {
