...
public class SmsMessage extends SmsMessageBase {
    ...
    public static SmsMessage createFromPdu(byte[] pdu) {
        ...
        try {
            msg.parsePdu(pdu);
            if (msg.mBearerData != null && msg.mBearerData.priorityIndicatorSet) {
                CdmaSmsPlugin.onSmsReceivedWithPriorityIndication(msg.mBearerData);
                return msg;
            }
            return msg;
    ...
    public static SmsMessage createFromEfRecord(int index, byte[] bArr) {
        try {
            SmsMessage msg = new SmsMessage();
            ...
            if ((bArr[0] & 1) == 0) {
                ...
                msg.mStatusOnIcc = bArr[0] & 7;
                int i = bArr[1];
                byte[] pdu = new byte[i];
                System.arraycopy(bArr, 2, pdu, 0, i);
    ...
    public static SubmitPdu getSubmitPdu(String scAddr, String destAddr, String message, boolean statusReportRequested, SmsHeader smsHeader, int encoding) {
    ...
    private static SubmitPdu privateGetSubmitPdu(String destAddrStr, boolean statusReportRequested, UserData userData) {
        ...
        String callbackNumber = CdmaSmsPlugin.getCallbackNumber();
        if (callbackNumber != null) {
            bearerData.callbackNumber = CdmaSmsAddress.parse(PhoneNumberUtils.cdmaCheckAndProcessPlusCodeForSms(callbackNumber));
        }
        bearerData.priorityIndicatorSet = CdmaSmsPlugin.isPriorityIndicationSet();
        if (bearerData.priorityIndicatorSet) {
            bearerData.priority = CdmaSmsPlugin.getPriorityIndication();
        }
