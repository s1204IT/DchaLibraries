    ...
    private static Pair<String, String> parseBody(SmsCbHeader header, byte[] pdu) {
        ...
        if (header.isUmtsFormat()) {
            ...
            for (int i = 0; i < nrPages; i++) {
                int offset2 = (i * 83) + 7;
                int length2 = pdu[offset2 + 82];
                if (length2 > 82) {
                    throw new IllegalArgumentException("Page length " + length2 + " exceeds maximum value 82");
                }
                Pair<String, String> p = unpackBody(pdu, encoding, offset2, length2, hasLanguageIndicator, language);
                language = (String) p.first;
                sb.append((String) p.second);
            }
            return new Pair<>(language, sb.toString());
        }
        if (header.isLteFormat()) {
            offset = 8;
            length = (((pdu.length - 8) * 7) + 7) / 8;
        } else {
            offset = 6;
            length = pdu.length - 6;
        }
