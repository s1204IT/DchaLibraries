...
public class StorageVolumePreferenceCategory extends PreferenceCategory {
    ...
    private StorageVolumePreferenceCategory(Context context, StorageVolume volume) {
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    case 1:
                        ...
                        break;
                    case 2:
                        StorageMeasurement.MeasurementDetails details = (StorageMeasurement.MeasurementDetails) msg.obj;
                        StorageVolumePreferenceCategory.this.updateDetails(details);
                        break;
                }
            }
        };
        this.mReceiver = new StorageMeasurement.MeasurementReceiver() {
            ...
            @Override
            public void updateDetails(StorageMeasurement meas, StorageMeasurement.MeasurementDetails details) {
    ...
    public void init() {
        ...
        try {
            ...
            if (showDetails) {
                ...
                if (showUsers) {
                    ...
                    for (UserInfo info : otherUsers) {
                        int count2 = count + 1;
                        int colorRes = count % 2 == 0 ? R.color.memory_user_light : R.color.memory_user_dark;
                        StorageItemPreference userPref = new StorageItemPreference(getContext(), info.name, colorRes, info.id);
                        this.mItemUsers.add(userPref);
            ...
            boolean allowFormat = this.mVolume != null;
            if (allowFormat) {
    ...
    public void updateApproximate(long totalSize, long availSize) {
        ...
        this.mUsageBarPreference.addEntry(0, usedSize / totalSize, -7829368);
    ...
    public void updateDetails(StorageMeasurement.MeasurementDetails details) {
        ...
        if (showDetails) {
            ...
            long dcimSize = totalValues(details.mediaSize, Environment.DIRECTORY_DCIM, Environment.DIRECTORY_MOVIES, Environment.DIRECTORY_PICTURES);
            updatePreference(this.mItemDcim, dcimSize);
            long musicSize = totalValues(details.mediaSize, Environment.DIRECTORY_MUSIC, Environment.DIRECTORY_ALARMS, Environment.DIRECTORY_NOTIFICATIONS, Environment.DIRECTORY_RINGTONES, Environment.DIRECTORY_PODCASTS);
            updatePreference(this.mItemMusic, musicSize);
            long downloadsSize = totalValues(details.mediaSize, Environment.DIRECTORY_DOWNLOADS);
            updatePreference(this.mItemDownloads, downloadsSize);
            ...
            for (StorageItemPreference userPref : this.mItemUsers) {
                long userSize = details.usersSize.get(userPref.userHandle);
    ...
    private void updatePreference(StorageItemPreference pref, long size) {
        if (size > 0) {
            ...
            int order = pref.getOrder();
            this.mUsageBarPreference.addEntry(order, size / this.mTotalSize, pref.color);
            return;
        }
    ...
    public Intent intentForClick(Preference pref) {
        ...
        if (pref != this.mItemMisc) {
            return null;
        }
        Context context = getContext().getApplicationContext();
        Intent intent5 = new Intent(context, (Class<?>) MiscFilesHandler.class);
    ...
    public static class PreferenceHeader extends Preference {
        public PreferenceHeader(Context context, int titleRes) {
            super(context, null, android.R.attr.preferenceCategoryStyle);
            setTitle(titleRes);
        }
