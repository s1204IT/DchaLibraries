...
public class ChartGridView extends View {
    ...
    public ChartGridView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray ta = context.obtainStyledAttributes(taId, com.android.internal.R.styleable.TextAppearance);
    ...
    void setBounds(long start, long end) {
        ...
        this.mLabelStart = makeLabel(DataUsageSummary.formatDateRange(context, start, start));
        this.mLabelMid = makeLabel(DataUsageSummary.formatDateRange(context, mid, mid));
        this.mLabelEnd = makeLabel(DataUsageSummary.formatDateRange(context, end, end));
        invalidate();
    }
    ...
    private Layout makeLabel(CharSequence text) {
        ...
        return new StaticLayout(text, paint, (int) Math.ceil(Layout.getDesiredWidth(text, paint)), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, true);
    }
}
