...
public class SetupWizardIllustration extends FrameLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (this.mAspectRatio != 0.0f) {
            ...
            setPaddingRelative(0, (int) (illustrationHeight - (illustrationHeight % this.mBaselineGridSize)), 0, 0);
        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (this.mBackground != null) {
            this.mBackground.setBounds(0, 0, (int) Math.ceil(layoutWidth / this.mScale), (int) Math.ceil((layoutHeight - this.mForegroundBounds.height()) / this.mScale));
        }
    ...
    public void onDraw(Canvas canvas) {
        if (this.mBackground != null) {
            canvas.save();
            canvas.translate(0.0f, this.mForegroundBounds.height());
