...
public abstract class WindowOrientationListener {
    ...
    public void dump(PrintWriter pw, String prefix) {
        synchronized (this.mLock) {
            ...
            if (this.mSensorEventListener != null) {
                this.mSensorEventListener.dumpLocked(pw, prefix2);
            }
        }
    }
    ...
    final class SensorEventListenerImpl implements SensorEventListener {
        ...
        private boolean isTiltAngleAcceptableLocked(int rotation, int tiltAngle) {
            return tiltAngle >= this.TILT_TOLERANCE[rotation][ACCELEROMETER_DATA_X] && tiltAngle <= this.TILT_TOLERANCE[rotation][1];
        }
        ...
        private boolean isPredictedRotationAcceptableLocked(long now) {
            return now >= this.mPredictedRotationTimestampNanos + PROPOSAL_SETTLE_TIME_NANOS && now >= this.mFlatTimestampNanos + 500000000 && now >= this.mSwingTimestampNanos + 300000000 && now >= this.mAccelerationTimestampNanos + 500000000;
        }
