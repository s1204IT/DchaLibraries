...
public class Utils implements XMPConst {
            ...
            for (int i = 0; i < value.length(); i++) {
                switch (value.charAt(i)) {
                    case ' ':
                        break;
                    case '-':
                    case ImageLoader.DEFAULT_COMPRESS_QUALITY:
                        buffer.append('-');
                        subTag++;
                        break;
                    default:
    ...
    static boolean isInternalProperty(String schema, String prop) {
        ...
        if (XMPConst.NS_XMP.equals(schema)) {
        ...
        if (XMPConst.NS_PDF.equals(schema)) {
        ...
        if (XMPConst.NS_CAMERARAW.equals(schema)) {
            if (!"crs:Version".equals(prop) && !"crs:RawFileName".equals(prop) && !"crs:ToneCurveName".equals(prop)) {
                return false;
            }
            return true;
        }
        if (!XMPConst.NS_ADOBESTOCKPHOTO.equals(schema) && !XMPConst.NS_XMP_MM.equals(schema) && !XMPConst.TYPE_TEXT.equals(schema) && !XMPConst.TYPE_PAGEDFILE.equals(schema) && !XMPConst.TYPE_GRAPHICS.equals(schema) && !XMPConst.TYPE_IMAGE.equals(schema) && !XMPConst.TYPE_FONT.equals(schema)) {
            return false;
        }
        return true;
    }
