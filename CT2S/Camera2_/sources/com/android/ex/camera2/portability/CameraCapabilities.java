...
public class CameraCapabilities {
    ...
    protected int mMaxBrightness;
    protected int mMaxContrast;
    ...
    protected int mMaxSaturation;
    protected int mMaxSharpness;
    protected float mMaxZoomRatio;
    protected int mMinBrightness;
    protected int mMinContrast;
    protected int mMinExposureCompensation;
    protected int mMinSaturation;
    protected int mMinSharpness;
    ...
    protected final EnumSet<IsoMode> mSupportedIsoModes = EnumSet.noneOf(IsoMode.class);
    protected final EnumSet<BurstCapture> mSupportedBurstCaptureModes = EnumSet.noneOf(BurstCapture.class);
    ...
    public static class Stringifier {
        ...
        public static SceneMode sceneModeFromString(String val) {
            if (val == null) {
        ...
        public String stringify(ColorEffect ce) {
            return toApiCase(ce.name());
        }
        ...
        public String stringify(IsoMode iso) {
            String setting = toApiCase(iso.name());
            return setting.substring("ISO".length(), setting.length());
        }

        public IsoMode isoModeFromString(String val) {
            if (val == null) {
                return IsoMode.values()[0];
            }
            try {
                return IsoMode.valueOf("ISO" + toEnumCase(val));
            } catch (IllegalArgumentException e) {
                return IsoMode.values()[0];
            }
        }

        public String stringify(BurstCapture bc) {
            return toApiCase(bc.name()) + "-burst";
        }

        public BurstCapture burstCaptureFromString(String val) {
            if (val == null) {
                return BurstCapture.values()[0];
            }
            try {
                String value = toEnumCase(val);
                return BurstCapture.valueOf(value.substring(0, value.length() - "-burst".length()));
            } catch (IllegalArgumentException e) {
                return BurstCapture.values()[0];
            }
        }

        public String stringify(AntiBanding ab) {
            switch (ab) {
                case AB50HZ:
                    return "50hz";
                case AB60HZ:
        ...
        public AntiBanding antiBandingFromString(String val) {
            if (val == null) {
                return AntiBanding.values()[0];
            }
            try {
                return AntiBanding.valueOf("AB" + toEnumCase(val));
            } catch (IllegalArgumentException e) {
                return AntiBanding.values()[0];
            }
        }
    ...
    public CameraCapabilities(CameraCapabilities src) {
        ...
        this.mSupportedColorEffects.addAll(src.mSupportedColorEffects);
        this.mSupportedIsoModes.addAll(src.mSupportedIsoModes);
        this.mSupportedBurstCaptureModes.addAll(src.mSupportedBurstCaptureModes);
        ...
        this.mMaxContrast = src.mMaxContrast;
        this.mMinContrast = src.mMinContrast;
        this.mMaxBrightness = src.mMaxBrightness;
        this.mMinBrightness = src.mMinBrightness;
        this.mMaxSaturation = src.mMaxSaturation;
        this.mMinSaturation = src.mMinSaturation;
        this.mMaxSharpness = src.mMaxSharpness;
        this.mMinSharpness = src.mMinSharpness;
    }
    ...
    public boolean supports(CameraSettings settings) {
        return zoomCheck(settings) && exposureCheck(settings) && focusCheck(settings) && flashCheck(settings) && photoSizeCheck(settings) && previewSizeCheck(settings) && videoStabilizationCheck(settings);
    }
    ...
    public final Set<IsoMode> getSupportedIsoModes() {
        return new HashSet(this.mSupportedIsoModes);
    }

    public boolean supports(IsoMode iso) {
        return iso != null && this.mSupportedIsoModes.contains(iso);
    }

    public final Set<BurstCapture> getSupportedBurstCaptureModes() {
        return new HashSet(this.mSupportedBurstCaptureModes);
    }

    public boolean supports(BurstCapture bc) {
        return bc != null && this.mSupportedBurstCaptureModes.contains(bc);
    }

    public final Set<AntiBanding> getSupportedAntibanding() {
        return new HashSet(this.mSupportedAntiBanding);
    }

    public boolean supports(AntiBanding ab) {
        return ab != null && this.mSupportedAntiBanding.contains(ab);
    }
    ...
    public int getMinSharpness() {
        return this.mMinSharpness;
    }

    public int getMaxContrast() {
        return this.mMaxContrast;
    }

    public int getMinContrast() {
        return this.mMinContrast;
    }
    ...
    private boolean exposureCheck(CameraSettings settings) {
        ...
        Log.v(TAG, "Exposure compensation index is not supported. Min = " + getMinExposureCompensation() + ", max = " + getMaxExposureCompensation() + Size.DELIMITER + " setting = " + index);
        return false;
    }
