...
public class TwilightCalculator {
    ...
    public void calculateTwilight(long time, double latiude, double longitude) {
        ...
        float trueAnomaly = (C1 * FloatMath.sin(meanAnomaly)) + meanAnomaly + (C2 * FloatMath.sin(2.0f * meanAnomaly)) + (C3 * FloatMath.sin(3.0f * meanAnomaly));
        float solarLng = 1.7965931f + trueAnomaly + 3.1415927f;
        double arcLongitude = (-longitude) / 360.0d;
        float n = Math.round(((double) (daysSince2000 - J0)) - arcLongitude);
        double solarTransitJ2000 = ((double) (J0 + n)) + arcLongitude + ((double) (0.0053f * FloatMath.sin(meanAnomaly))) + ((double) ((-0.0069f) * FloatMath.sin(2.0f * solarLng)));
