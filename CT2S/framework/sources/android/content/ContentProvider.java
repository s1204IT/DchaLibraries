    ...
    class Transport extends ContentProviderNative {
        ...
        public ContentProviderResult[] applyBatch(String callingPkg, ArrayList<ContentProviderOperation> operations) throws OperationApplicationException {
            ...
            for (int i = 0; i < numOperations; i++) {
                ...
                if (operation.isWriteOperation() && enforceWritePermission(callingPkg, uri, null) != 0) {
                    throw new OperationApplicationException("App op not allowed", 0);
                }
            }
            String original = ContentProvider.this.setCallingPackage(callingPkg);
        ...
        public Uri canonicalize(String callingPkg, Uri uri) {
            ...
            if (enforceReadPermission(callingPkg, uri2, null) == 0) {
                String original = ContentProvider.this.setCallingPackage(callingPkg);
                try {
                    uriMaybeAddUserId = ContentProvider.maybeAddUserId(ContentProvider.this.canonicalize(uri2), userId);
                } finally {
                    ContentProvider.this.setCallingPackage(original);
                }
            }
            return uriMaybeAddUserId;
        }
        ...
        public Uri uncanonicalize(String callingPkg, Uri uri) {
            ...
            if (enforceReadPermission(callingPkg, uri2, null) == 0) {
                String original = ContentProvider.this.setCallingPackage(callingPkg);
                try {
                    uriMaybeAddUserId = ContentProvider.maybeAddUserId(ContentProvider.this.uncanonicalize(uri2), userId);
                } finally {
                    ContentProvider.this.setCallingPackage(original);
                }
            }
