...
class TtmlParser {
            ...
            if (active) {
                if (eventType == 2) {
                    ...
                    if (!TextUtils.isEmpty(text)) {
                        parent.mChildren.add(new TtmlNode(TtmlUtils.PCDATA, ProxyInfo.LOCAL_EXCL_LIST, text, 0L, Long.MAX_VALUE, parent, this.mCurrentRunId));
                    }
                } else if (eventType == 3) {
    ...
    private TtmlNode parseNode(TtmlNode parent) throws XmlPullParserException, IOException {
        ...
        for (int i = 0; i < this.mParser.getAttributeCount(); i++) {
            ...
            String attr2 = attr.replaceFirst("^.*:", ProxyInfo.LOCAL_EXCL_LIST);
    ...
    private static boolean isSupportedTag(String tag) {
        return tag.equals(TtmlUtils.TAG_TT) || tag.equals(TtmlUtils.TAG_HEAD) || tag.equals(TtmlUtils.TAG_BODY) || tag.equals(TtmlUtils.TAG_DIV) || tag.equals(TtmlUtils.TAG_P) || tag.equals(TtmlUtils.TAG_SPAN) || tag.equals(TtmlUtils.TAG_BR) || tag.equals(TtmlUtils.TAG_STYLE) || tag.equals(TtmlUtils.TAG_STYLING) || tag.equals(TtmlUtils.TAG_LAYOUT) || tag.equals(TtmlUtils.TAG_REGION) || tag.equals(TtmlUtils.TAG_METADATA) || tag.equals(TtmlUtils.TAG_SMPTE_IMAGE) || tag.equals(TtmlUtils.TAG_SMPTE_DATA) || tag.equals(TtmlUtils.TAG_SMPTE_INFORMATION);
    }
}
