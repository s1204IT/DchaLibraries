...
class Utils {
    ...
    static Range<Integer> parseIntRange(Object o, Range<Integer> fallback) {
        try {
            ...
            int ix = s.indexOf(45);
            if (ix >= 0) {
                fallback = Range.create(Integer.valueOf(Integer.parseInt(s.substring(0, ix), 10)), Integer.valueOf(Integer.parseInt(s.substring(ix + 1), 10)));
            } else {
                int value = Integer.parseInt(s);
                fallback = Range.create(Integer.valueOf(value), Integer.valueOf(value));
            }
        } catch (ClassCastException e) {
    ...
    static Range<Long> parseLongRange(Object o, Range<Long> fallback) {
        try {
            ...
            int ix = s.indexOf(45);
            if (ix >= 0) {
                fallback = Range.create(Long.valueOf(Long.parseLong(s.substring(0, ix), 10)), Long.valueOf(Long.parseLong(s.substring(ix + 1), 10)));
            } else {
                long value = Long.parseLong(s);
                fallback = Range.create(Long.valueOf(value), Long.valueOf(value));
            }
        } catch (ClassCastException e) {
    ...
    static Range<Rational> parseRationalRange(Object o, Range<Rational> fallback) {
        try {
            ...
            int ix = s.indexOf(45);
            if (ix >= 0) {
                fallback = Range.create(Rational.parseRational(s.substring(0, ix)), Rational.parseRational(s.substring(ix + 1)));
            } else {
                Rational value = Rational.parseRational(s);
                fallback = Range.create(value, value);
            }
        } catch (ClassCastException e) {
    ...
    static Pair<Size, Size> parseSizeRange(Object o) {
        Pair<Size, Size> pairCreate = null;
        try {
            String s = (String) o;
            int ix = s.indexOf(45);
            if (ix >= 0) {
                pairCreate = Pair.create(Size.parseSize(s.substring(0, ix)), Size.parseSize(s.substring(ix + 1)));
            } else {
                Size value = Size.parseSize(s);
                pairCreate = Pair.create(value, value);
            }
        } catch (ClassCastException e) {
