...
public class Paint {
    ...
    public float measureText(CharSequence text, int start, int end) {
        ...
        float fMeasureText = measureText(buf, 0, end - start);
    ...
    public int breakText(char[] text, int index, int count, float maxWidth, float[] measuredWidth) {
        ...
        float oldSize = getTextSize();
        setTextSize(this.mCompatScaling * oldSize);
        int iNative_breakText = native_breakText(this.mNativePaint, this.mNativeTypeface, text, index, count, maxWidth * this.mCompatScaling, this.mBidiFlags, measuredWidth);
        setTextSize(oldSize);
    ...
    public int breakText(String text, boolean measureForwards, float maxWidth, float[] measuredWidth) {
        ...
        float oldSize = getTextSize();
        setTextSize(this.mCompatScaling * oldSize);
        int iNative_breakText = native_breakText(this.mNativePaint, this.mNativeTypeface, text, measureForwards, maxWidth * this.mCompatScaling, this.mBidiFlags, measuredWidth);
        setTextSize(oldSize);
        if (measuredWidth != null) {
            measuredWidth[0] = measuredWidth[0] * this.mInvCompatScaling;
            return iNative_breakText;
    ...
    public int getTextWidths(CharSequence text, int start, int end, float[] widths) {
        ...
        char[] buf = TemporaryBuffer.obtain(end - start);
        TextUtils.getChars(text, start, end, buf, 0);
        int textWidths = getTextWidths(buf, 0, end - start, widths);
        TemporaryBuffer.recycle(buf);
        return textWidths;
    }
    ...
    public float getTextRunAdvances(CharSequence text, int start, int end, int contextStart, int contextEnd, boolean isRtl, float[] advances, int advancesIndex) {
        ...
        float textRunAdvances = getTextRunAdvances(buf, start - contextStart, len, 0, contextLen, isRtl, advances, advancesIndex);
        TemporaryBuffer.recycle(buf);
    ...
    public int getTextRunCursor(CharSequence text, int contextStart, int contextEnd, int dir, int offset, int cursorOpt) {
        ...
        char[] buf = TemporaryBuffer.obtain(contextLen);
        TextUtils.getChars(text, contextStart, contextEnd, buf, 0);
        int textRunCursor = getTextRunCursor(buf, 0, contextLen, dir, offset - contextStart, cursorOpt);
        TemporaryBuffer.recycle(buf);
