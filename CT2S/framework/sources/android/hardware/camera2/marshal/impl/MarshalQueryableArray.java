            ...
            if (elementSize != Marshaler.NATIVE_SIZE_DYNAMIC) {
                ...
                if (remaining % elementSize == 0) {
                    if (MarshalQueryableArray.VERBOSE) {
                        Log.v(MarshalQueryableArray.TAG, String.format("Attempting to unpack array (count = %d, element size = %d, bytes remaining = %d) for type %s", Integer.valueOf(arraySize), Integer.valueOf(elementSize), Integer.valueOf(remaining), this.mClass));
                    }
