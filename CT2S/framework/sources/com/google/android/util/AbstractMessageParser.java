...
public abstract class AbstractMessageParser {
    ...
    public static Token tokenForUrl(String url, String text) {
        ...
        if (video == null) {
            ...
            return flickrPhoto != null ? flickrPhoto : new Link(url, text);
        }
        return video;
    }
    ...
    private boolean isValidDomain(String domain) {
        return matches(getResources().getDomainSuffixes(), reverse(domain));
    }
    ...
    private boolean parseURL() {
        ...
        String url = ProxyInfo.LOCAL_EXCL_LIST;
        boolean done = false;
        ...
        if (this.text.charAt(index) == ':') {
            ...
            if (index == this.text.length()) {
                ...
                } else if (isPunctuation(ch2) || Character.isWhitespace(ch2)) {
                    done = true;
                } else if (ch2 != '/' && ch2 != '#') {
                    return false;
                }
    ...
    private boolean isSmileyBreak(int index) {
        return index > 0 && index < this.text.length() && isSmileyBreak(this.text.charAt(index + (-1)), this.text.charAt(index));
    }
    ...
    public static class Video extends Token {
        ...
        public static String getURL(String docid, String extraParams) {
            if (extraParams == null) {
                extraParams = ProxyInfo.LOCAL_EXCL_LIST;
    ...
    public static class YouTubeVideo extends Token {
        ...
        public static String getPrefixedURL(boolean http, String prefix, String docid, String extraParams) {
            String protocol = ProxyInfo.LOCAL_EXCL_LIST;
            if (http) {
                protocol = "http://";
            }
            if (prefix == null) {
    ...
    public static class FlickrPhoto extends Token {
        ...
        public FlickrPhoto(String user, String photo, String grouping, String groupingId, String text) {
            ...
            if (!"tags".equals(user)) {
                this.user = user;
                this.photo = HardwareRenderer.OVERDRAW_PROPERTY_SHOW.equals(photo) ? null : photo;
        ...
        public List<String> getInfo() {
            List<String> info = super.getInfo();
            info.add(getUrl());
            info.add(getUser() != null ? getUser() : ProxyInfo.LOCAL_EXCL_LIST);
            info.add(getPhoto() != null ? getPhoto() : ProxyInfo.LOCAL_EXCL_LIST);
            info.add(getGrouping() != null ? getGrouping() : ProxyInfo.LOCAL_EXCL_LIST);
            info.add(getGroupingId() != null ? getGroupingId() : ProxyInfo.LOCAL_EXCL_LIST);
            return info;
        }
    ...
    public static class Part {
        ...
        public String getType(boolean isSend) {
            return (isSend ? "s" : FullBackup.ROOT_TREE_TOKEN) + getPartType();
        }
