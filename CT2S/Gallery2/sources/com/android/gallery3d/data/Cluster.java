...
class Cluster {
    ...
    public String generateCaption(Context context) {
        ...
        long maxTimestamp = 0;
        for (int i = 0; i < n; i++) {
            long t = this.mItems.get(i).dateInMs;
            if (t != 0) {
                if (minTimestamp == 0) {
                    maxTimestamp = t;
                    minTimestamp = t;
        ...
        if (minDay.substring(4).equals(maxDay.substring(4))) {
            String dateRange = DateUtils.formatDateRange(context, minTimestamp, maxTimestamp, 524288);
            if (minDay.equals(maxDay)) {
                String dateRangeWithOptionalYear = DateUtils.formatDateTime(context, minTimestamp, 65552);
                String dateRangeWithYear = DateUtils.formatDateTime(context, minTimestamp, 65556);
                if (!dateRangeWithOptionalYear.equals(dateRangeWithYear)) {
                    long midTimestamp = (minTimestamp + maxTimestamp) / 2;
                    String caption = DateUtils.formatDateRange(context, midTimestamp, midTimestamp, 65553);
                    return caption;
                }
                return dateRange;
            }
