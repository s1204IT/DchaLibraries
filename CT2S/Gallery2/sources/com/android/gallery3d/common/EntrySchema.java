...
public final class EntrySchema {
    ...
    public void createTables(SQLiteDatabase db) {
        ...
        ColumnInfo[] arr$ = this.mColumnInfo;
        for (ColumnInfo column : arr$) {
            if (!column.isId()) {
                sql.append(',');
        ...
        if (unique.length() > 0) {
            sql.append(",UNIQUE(").append((CharSequence) unique).append(')');
        }
        sql.append(");");
        logExecSql(db, sql.toString());
        sql.setLength(0);
        ColumnInfo[] arr$2 = this.mColumnInfo;
        for (ColumnInfo column2 : arr$2) {
            if (column2.indexed) {
        ...
        if (this.mHasFullTextIndex) {
            ...
            ColumnInfo[] arr$3 = this.mColumnInfo;
            for (ColumnInfo column3 : arr$3) {
                if (column3.fullText) {
                    String columnName = column3.name;
                    sql.append(',');
            ...
            ColumnInfo[] arr$4 = this.mColumnInfo;
            for (ColumnInfo column4 : arr$4) {
                if (column4.fullText) {
                    insertSql.append(',');
                    insertSql.append(column4.name);
                }
            }
            insertSql.append(") VALUES (new._id");
            ColumnInfo[] arr$5 = this.mColumnInfo;
            for (ColumnInfo column5 : arr$5) {
                if (column5.fullText) {
                    insertSql.append(",new.");
                    insertSql.append(column5.name);
                }
            }
            insertSql.append(");");
    ...
    private String parseTableName(Class<? extends Object> clazz) {
        Entry.Table table = (Entry.Table) clazz.getAnnotation(Entry.Table.class);
        if (table == null) {
            return null;
        }
        return table.value();
    ...
    private void parseColumnInfo(Class<? extends Object> clazz, ArrayList<ColumnInfo> columns) {
        ...
        for (int i = 0; i != fields.length; i++) {
            ...
            Entry.Column info = (Entry.Column) field.getAnnotation(Entry.Column.class);
            if (info != null) {
