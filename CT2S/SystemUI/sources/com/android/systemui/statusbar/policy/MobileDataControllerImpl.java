...
public class MobileDataControllerImpl implements NetworkController.MobileDataController {
    ...
    private NetworkController.MobileDataController.DataUsageInfo warn(String msg) {
    ...
    public NetworkController.MobileDataController.DataUsageInfo getDataUsageInfo() {
        ...
        try {
            ...
            if (DEBUG) {
                Log.d("MobileDataController", String.format("history call from %s to %s now=%s took %sms: %s", new Date(start), new Date(end), new Date(now), Long.valueOf(callEnd - callStart), historyEntryToString(entry)));
            }
            if (entry == null) {
                return warn("no entry data");
            }
            long totalBytes = entry.rxBytes + entry.txBytes;
            NetworkController.MobileDataController.DataUsageInfo usage = new NetworkController.MobileDataController.DataUsageInfo();
            ...
            if (usage != null) {
                usage.carrier = this.mNetworkController.getMobileNetworkName();
                return usage;
            }
            return usage;
        } catch (RemoteException e) {
    ...
    private static String getActiveSubscriberId(Context context) {
        ...
        String actualSubscriberId = tele.getSubscriberId(SubscriptionManager.getDefaultDataSubId());
    ...
    private String formatDateRange(long start, long end) {
        ...
        synchronized (PERIOD_BUILDER) {
            ...
            string = DateUtils.formatDateRange(this.mContext, PERIOD_FORMATTER, start, end, 65552, null).toString();
        }
        return string;
    }
}
