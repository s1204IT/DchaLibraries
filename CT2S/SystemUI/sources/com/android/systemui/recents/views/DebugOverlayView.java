...
public class DebugOverlayView extends FrameLayout implements SeekBar.OnSeekBarChangeListener {
    ...
    protected void onDraw(Canvas canvas) {
        if (this.mEnabled) {
            ...
            for (int i = 0; i < numRects; i++) {
                Pair<Rect, Integer> r = this.mRects.get(i);
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        if (seekBar == this.mPrimarySeekBar) {
            this.mCb.onPrimarySeekBarChanged(progress / this.mPrimarySeekBar.getMax());
        } else if (seekBar == this.mSecondarySeekBar) {
            this.mCb.onSecondarySeekBarChanged(progress / this.mSecondarySeekBar.getMax());
        }
    }
}
