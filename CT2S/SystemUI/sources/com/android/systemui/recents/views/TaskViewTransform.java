...
public class TaskViewTransform {
    ...
    public void applyToTaskView(View v, int duration, Interpolator interp, boolean allowLayers, boolean allowShadows, ValueAnimator.AnimatorUpdateListener updateCallback) {
        if (duration > 0) {
            ViewPropertyAnimator anim = v.animate();
            boolean requiresLayers = false;
            if (hasTranslationYChangedFrom(v.getTranslationY())) {
                anim.translationY(this.translationY);
            }
            ...
            anim.setStartDelay(this.startDelay).setDuration(duration).setInterpolator(interp).start();
            return;
        }
        if (hasTranslationYChangedFrom(v.getTranslationY())) {
            v.setTranslationY(this.translationY);
        }
        if (allowShadows && hasTranslationZChangedFrom(v.getTranslationZ())) {
