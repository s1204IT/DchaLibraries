...
class SipHelper {
    ...
    public ClientTransaction sendOptions(SipProfile caller, SipProfile callee, String tag) throws SipException {
        try {
            Request request = caller == callee ? createRequest("OPTIONS", caller, tag) : createRequest("OPTIONS", caller, callee, tag);
    ...
    public static String getCallId(EventObject event) {
        ...
        if (event instanceof TransactionTerminatedEvent) {
            ...
            return getCallId((Transaction) (e.isServerTransaction() ? e.getServerTransaction() : e.getClientTransaction()));
        }
