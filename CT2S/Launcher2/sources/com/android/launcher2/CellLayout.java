...
public class CellLayout extends ViewGroup {
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.mHotseatScale = res.getInteger(R.integer.hotseat_item_scale_percentage) / 100.0f;
        ...
        this.mReorderHintAnimationMagnitude = 0.12f * res.getDimensionPixelSize(R.dimen.app_icon_size);
    ...
    static int widthInPortrait(Resources r, int numCells) {
        ...
        return ((numCells - 1) * minGap) + (cellWidth * numCells);
    }
    ...
    static int heightInLandscape(Resources r, int numCells) {
        ...
        int minGap = Math.min(r.getDimensionPixelSize(R.dimen.workspace_width_gap), r.getDimensionPixelSize(R.dimen.workspace_height_gap));
        return ((numCells - 1) * minGap) + (cellHeight * numCells);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < this.mDragOutlines.length; i++) {
            ...
            if (alpha > 0.0f) {
                ...
                scaleRectAboutCenter(r, this.temp, getChildrenScale());
                Bitmap b = (Bitmap) this.mDragOutlineAnims[i].getTag();
                paint.setAlpha((int) (0.5f + alpha));
                canvas.drawBitmap(b, (Rect) null, this.temp, paint);
            }
        }
        if (this.mPressedOrFocusedIcon != null) {
            ...
            if (b2 != null) {
                canvas.drawBitmap(b2, (this.mPressedOrFocusedIcon.getLeft() + getPaddingLeft()) - padding, (this.mPressedOrFocusedIcon.getTop() + getPaddingTop()) - padding, (Paint) null);
            }
        }
        ...
        for (int i2 = 0; i2 < this.mFolderOuterRings.size(); i2++) {
            ...
            canvas.save();
            canvas.translate(centerX2 - (width2 / 2), centerY2 - (width2 / 2));
        ...
        if (this.mFolderLeaveBehindCell[0] >= 0 && this.mFolderLeaveBehindCell[1] >= 0) {
            ...
            canvas.save();
            canvas.translate(centerX3 - (width3 / 2), centerY3 - (width3 / 2));
    ...
    int[] createArea(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX, int spanY, View dragView, int[] result, int[] resultSpan, int mode) {
        ...
        if ((mode == 1 || mode == 2 || mode == 3) && this.mPreviousReorderDirection[0] != -100) {
