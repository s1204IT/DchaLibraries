...
public final class Pm {
    ...
    private int runShellCommand(String serviceName, String[] args) {
        HandlerThread handlerThread = new HandlerThread("results");
        handlerThread.start();
        try {
            try {
    ...
    private int doCreateSession(PackageInstaller.SessionParams params, String installerPackageName, int userId) throws RemoteException {
        ...
        int sessionId = this.mInstaller.createSession(params, installerPackageName, userId2);
        return sessionId;
    }

    private int doWriteSession(int sessionId, String inPath, long sizeBytes, String splitName, boolean logSuccess) throws Throwable {
        ...
        do {
            ...
            if (opt == null) {
                ...
                try {
                    ...
                    if ((info.applicationInfo.privateFlags & 16) == 0) {
                        System.err.println("Error: package " + pkg + " does not handle web links.");
                        return 1;
                    }
                    if (this.mPm.updateIntentVerificationStatus(pkg, newMode, userId)) {
                        return 0;
                    }
