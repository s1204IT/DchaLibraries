...
public class KeycodeSelectionHelper {
    ...
    private static String formatString(String name) {
        StringBuilder str = new StringBuilder(name.replace("KEYCODE_", "").replace("_", " ").toLowerCase());
    ...
    public static void showKeycodeSelect(Context context, final OnSelectionComplete listener) {
        new AlertDialog.Builder(context).setTitle(R.string.select_keycode).setItems((CharSequence[]) mKeycodeStrings.toArray(new String[0]), new DialogInterface.OnClickListener() {
            @Override
