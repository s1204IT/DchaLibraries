...
public class FlingAnimationUtils {
    ...
    private AnimatorProperties getProperties(float currValue, float endValue, float velocity, float maxDistance) {
        float maxLengthSeconds = (float) (((double) this.mMaxLengthSeconds) * Math.sqrt(Math.abs(endValue - currValue) / maxDistance));
        ...
        if (durationSeconds <= maxLengthSeconds) {
            ...
            VelocityInterpolator velocityInterpolator = new VelocityInterpolator(maxLengthSeconds, velAbs, diff, null);
            InterpolatorInterpolator superInterpolator = new InterpolatorInterpolator(velocityInterpolator, this.mLinearOutSlowIn, this.mLinearOutSlowIn);
    ...
    private AnimatorProperties getDismissingProperties(float currValue, float endValue, float velocity, float maxDistance) {
        float maxLengthSeconds = (float) (((double) this.mMaxLengthSeconds) * Math.pow(Math.abs(endValue - currValue) / maxDistance, 0.5d));
        float diff = Math.abs(endValue - currValue);
        float velAbs = Math.abs(velocity);
        ...
        if (durationSeconds <= maxLengthSeconds) {
            ...
            durationSeconds = maxLengthSeconds;
            VelocityInterpolator velocityInterpolator = new VelocityInterpolator(maxLengthSeconds, velAbs, diff, null);
            InterpolatorInterpolator superInterpolator = new InterpolatorInterpolator(velocityInterpolator, mLinearOutFasterIn, this.mLinearOutSlowIn);
            this.mAnimatorProperties.interpolator = superInterpolator;
        } else {
            durationSeconds = maxLengthSeconds;
    ...
    private static final class VelocityInterpolator implements Interpolator {
        ...
        VelocityInterpolator(float durationSeconds, float velocity, float diff, VelocityInterpolator velocityInterpolator) {
            this(durationSeconds, velocity, diff);
        }
        ...
        @Override
        public float getInterpolation(float input) {
            float time = input * this.mDurationSeconds;
    ...
    private static class AnimatorProperties {
        ...
        AnimatorProperties(AnimatorProperties animatorProperties) {
            this();
        }
