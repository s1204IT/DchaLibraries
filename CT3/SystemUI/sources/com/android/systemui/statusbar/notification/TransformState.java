...
public class TransformState {
    ...
    private void transformViewFrom(TransformState otherState, int transformationFlags, ViewTransformationHelper.CustomTransformation customTransformation, float transformationAmount) {
        int[] otherPosition;
        ...
        if (transformationAmount == 0.0f || ((transformX && getTransformationStartX() == -1.0f) || ((transformY && getTransformationStartY() == -1.0f) || ((transformScale && getTransformationStartScaleX() == -1.0f) || (transformScale && getTransformationStartScaleY() == -1.0f))))) {
            ...
            if (customTransformation == null || !customTransformation.initTransformation(this, otherState)) {
                ...
                if (transformScale && otherView.getWidth() != transformedView.getWidth()) {
                    setTransformationStartScaleX((otherView.getWidth() * otherView.getScaleX()) / transformedView.getWidth());
                ...
                if (transformScale && otherView.getHeight() != transformedView.getHeight()) {
                    setTransformationStartScaleY((otherView.getHeight() * otherView.getScaleY()) / transformedView.getHeight());
    ...
    private void transformViewTo(TransformState otherState, int transformationFlags, ViewTransformationHelper.CustomTransformation customTransformation, float transformationAmount) {
        ...
        float transformationStartScaleX = getTransformationStartScaleX();
        if (transformationStartScaleX != -1.0f) {
            transformedView.setScaleX(NotificationUtils.interpolate(transformationStartScaleX, otherView2.getWidth() / transformedView.getWidth(), interpolatedValue));
        }
        float transformationStartScaleY = getTransformationStartScaleY();
        if (transformationStartScaleY == -1.0f) {
            return;
        }
        transformedView.setScaleY(NotificationUtils.interpolate(transformationStartScaleY, otherView2.getHeight() / transformedView.getHeight(), interpolatedValue));
    }
