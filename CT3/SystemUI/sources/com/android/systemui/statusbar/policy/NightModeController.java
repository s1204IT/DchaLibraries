...
public class NightModeController implements TunerService.Tunable {
    ...
    public void setNightMode(boolean isNight) {
        ...
        if (this.mIsAuto) {
            if (this.mIsNight != isNight) {
                ...
                tunerService.setValue("twilight_mode", i);
                return;
            }
    ...
    private void updateListening() {
        ...
        if (this.mListeners.size() == 0) {
            shouldListen = this.mUpdateMatrix ? this.mAdjustTint : false;
        } else {
            shouldListen = true;
        }
    ...
    public void onTuningChanged(String key, String newValue) {
        ...
        if ("tuner_color_custom_values".equals(key)) {
            this.mCustomMatrix = newValue != null ? toValues(newValue) : null;
            updateCurrentMatrix();
            return;
        }
        if ("tuner_night_mode_adjust_tint".equals(key)) {
            this.mAdjustTint = newValue == null || Integer.parseInt(newValue) != 0;
            updateListening();
            updateCurrentMatrix();
        } else {
            if (!"twilight_mode".equals(key)) {
                return;
            }
    ...
    public void updateCurrentMatrix() {
        if (this.mUpdateMatrix) {
            ...
            float[] values = scaleValues(IDENTITY_MATRIX, NIGHT_VALUES, this.mAdjustTint ? this.mAmount : 0.0f);
    ...
    public void updateNightMode(Intent intent) {
        this.mIsNight = intent != null ? intent.getBooleanExtra("isNight", false) : false;
