...
public class RecentsTaskLoader {
    ...
    public void loadTaskData(Task t) {
        ...
        t.notifyTaskDataLoaded(thumbnail, icon, thumbnailInfo);
    }
    ...
    Drawable getAndUpdateActivityIcon(Task.TaskKey taskKey, ActivityManager.TaskDescription td, Resources res, boolean loadIfNotCached) {
        ...
        if (loadIfNotCached) {
            Drawable icon3 = ssp.getBadgedTaskDescriptionIcon(td, taskKey.userId, res);
            if (icon3 != null) {
                this.mIconCache.put(taskKey, icon3);
                return icon3;
            }
            ActivityInfo activityInfo = getAndUpdateActivityInfo(taskKey);
            if (activityInfo != null && (icon = ssp.getBadgedActivityIcon(activityInfo, taskKey.userId)) != null) {
    ...
    Bitmap getAndUpdateThumbnail(Task.TaskKey taskKey, boolean loadIfNotCached) {
        ...
        if (loadIfNotCached) {
            ...
            if (config.svelteLevel < 3) {
                ThumbnailData thumbnailData2 = ssp.getTaskThumbnail(taskKey.id);
