...
public class DozeLog {
    ...
    private static void init(Context context) {
        synchronized (DozeLog.class) {
            if (sMessages == null) {
                ...
                sProxStats = (SummaryStats[][]) Array.newInstance((Class<?>) SummaryStats.class, 4, 2);
                for (int i = 0; i < 4; i++) {
    ...
    public static void traceProximityResult(Context context, boolean near, long millis, int pulseReason) {
        ...
        init(context);
        sProxStats[pulseReason][near ? (char) 0 : (char) 1].append();
    }
    ...
    private static class SummaryStats {
        ...
        public void dump(PrintWriter pw, String type) {
            ...
            pw.print(" (");
            double perHr = (((double) this.mCount) / (System.currentTimeMillis() - DozeLog.sSince)) * 1000.0d * 60.0d * 60.0d;
            pw.print(perHr);
            pw.print("/hr)");
            pw.println();
        }
