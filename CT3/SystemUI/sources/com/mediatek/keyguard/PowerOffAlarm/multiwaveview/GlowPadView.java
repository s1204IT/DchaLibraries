...
public class GlowPadView extends View {
    ...
    private class AnimationBundle extends ArrayList<Tweener> {
        ...
        AnimationBundle(GlowPadView this$0, AnimationBundle animationBundle) {
            this();
        }
    ...
    public GlowPadView(Context context, AttributeSet attrs) {
        ...
        this.mWaveAnimations = new AnimationBundle(this, animationBundle);
        this.mTargetAnimations = new AnimationBundle(this, animationBundle);
        this.mGlowAnimations = new AnimationBundle(this, animationBundle);
        ...
        this.mHandleDrawable = new TargetDrawable(res, handle != null ? handle.resourceId : 0, 1);
    ...
    public void switchToState(int state, float x, float y) {
        switch (state) {
            case 0:
                deactivateTargets();
    ...
    public void hideTargets(boolean animate, boolean expanded) {
    ...
    public void internalSetTargetResources(int resourceId) {
    ...
    private void startWaveAnimation() {
        ...
        this.mWaveAnimations.add(Tweener.to(this.mPointCloud.waveManager, 1350L, "ease", Ease$Quad.easeOut, "delay", 0, "radius", Float.valueOf(this.mOuterRadius * 2.0f), "onUpdate", this.mUpdateListener, "onComplete", new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animator) {
        ...
        while (k < historySize + 1) {
            ...
            if (this.mDragging) {
                ...
                for (int i = 0; i < ntargets; i++) {
                    ...
                    if (target.isEnabled()) {
                        boolean angleMatches = (angleRad <= targetMinRad || angleRad > targetMaxRad) ? 6.283185307179586d + angleRad > targetMinRad && 6.283185307179586d + angleRad <= targetMaxRad : true;
        ...
        switch (absoluteGravity & 112) {
            ...
            default:
                if (desiredHeight > actualHeight) {
                    scaleY = ((actualHeight * 1.0f) - this.mMaxTargetHeight) / (desiredHeight - this.mMaxTargetHeight);
                }
                break;
        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        float newWaveCenterX = this.mHorizontalInset + ((this.mMaxTargetWidth + placementWidth) / 2.0f);
        float newWaveCenterY = this.mVerticalInset + ((this.mMaxTargetHeight + placementHeight) / 2.0f);
    ...
    private void updateTargetPositions(float centerX, float centerY) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            targetIcon.setX((getRingWidth() / 2.0f) * ((float) Math.cos(angle)));
            targetIcon.setY((getRingHeight() / 2.0f) * ((float) Math.sin(angle)));
        }
    }
