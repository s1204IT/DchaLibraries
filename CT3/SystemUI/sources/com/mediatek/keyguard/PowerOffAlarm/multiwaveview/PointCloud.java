...
public class PointCloud {
    ...
    public int getAlphaForPoint(Point point) {
        ...
        if (glowDistance < this.glowManager.radius) {
            double cos = Math.cos((((double) glowDistance) * 0.7853981633974483d) / ((double) this.glowManager.radius));
            glowAlpha = this.glowManager.alpha * max(0.0f, (float) Math.pow(cos, 10.0d));
        }
        ...
        if (distanceToWaveRing < this.waveManager.width * 0.5f && distanceToWaveRing < 0.0f) {
            double cos2 = Math.cos((((double) distanceToWaveRing) * 0.7853981633974483d) / ((double) this.waveManager.width));
            waveAlpha = this.waveManager.alpha * max(0.0f, (float) Math.pow(cos2, 20.0d));
        }
