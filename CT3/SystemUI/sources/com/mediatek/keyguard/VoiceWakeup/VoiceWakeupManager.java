...
public class VoiceWakeupManager implements KeyguardHostView.OnDismissAction {
    ...
    private LimitedModeApp[] limitedApps = {new LimitedModeApp("com.android.gallery3d/com.android.camera.CameraLauncher", "android.media.action.STILL_IMAGE_CAMERA_SECURE")};
    ...
    private KeyguardUpdateMonitorCallback mUpdateCallback = new KeyguardUpdateMonitorCallback() {
        ...
        public void onKeyguardVisibilityChanged(boolean showing) {
            ...
            if (VoiceWakeupManager.delayToLightUpScreen && !showing) {
                ...
                if (!VoiceWakeupManager.this.mIsDismissAndLaunchApp || !VoiceWakeupManager.this.mPM.isScreenOn() || showing) {
                    return;
                }
    ...
    private final BroadcastReceiver mBroadcastReceiverForTest = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Message msg = VoiceWakeupManager.this.mVoiceCommandHandlerForTest.obtainMessage(what);
            msg.sendToTarget();
        }
    };
    ...
    public void sendVoiceCommand(String pkgName, int mainAction, int subAction, Bundle extraData) {
        if (this.isRegistered) {
            try {
                int errorid = this.mVCmdMgrService.sendCommand(pkgName, mainAction, subAction, extraData);
                if (errorid != 0) {
    ...
    public void registerVoiceCommand(String pkgName) {
        if (!this.isRegistered) {
            try {
    ...
    private String getLimtiedModeActionNameOfApp(String appName) {
        for (int i = 0; i < this.limitedApps.length; i++) {
            if (appName.equals(this.limitedApps[i].normalModeAppName)) {
                String actionName = this.limitedApps[i].limtedModeAppName;
                return actionName;
            }
        }
        return null;
    ...
    private String getLaunchAppNameFromSettings(int commandId, boolean calledFromTest) {
        ...
        if (calledFromTest) {
            ...
            appName = Settings.System.getVoiceCommandValue(this.mContext.getContentResolver(), Settings.System.BASE_VOICE_WAKEUP_COMMAND_KEY, commandId);
        }
    ...
    private void doLaunchAppAndDismissKeyguard(int commandId, boolean isUserDependentMode, boolean calledFromTest) {
        ...
        if (this.mLockPatternUtils.isLockScreenDisabled(KeyguardUpdateMonitor.getCurrentUser()) || this.mPM.isScreenOn()) {
            isInLaterLocked = false;
        } else {
            isInLaterLocked = !this.mViewMediatorCallback.isShowing();
        }
        if (isInLaterLocked) {
        ...
        if (!this.mLockPatternUtils.isSecure(KeyguardUpdateMonitor.getCurrentUser()) && this.securityModel.getSecurityMode() == KeyguardSecurityModel.SecurityMode.None) {
            log("doLaunchAppAndDismissKeyguard() : Keyguard is SLIDE mode, launch full-access mode APP and dismiss keyguard.");
            ComponentName cn2 = ComponentName.unflattenFromString(this.mLaunchApp);
            if (cn2.getClassName().indexOf("VoiceSearchActivity") != -1) {
                lightUpScreen();
            }
    ...
    private boolean isSimPinPukMeModeNow() {
        if (this.securityModel.getSecurityMode() == KeyguardSecurityModel.SecurityMode.SimPinPukMe1 || this.securityModel.getSecurityMode() == KeyguardSecurityModel.SecurityMode.SimPinPukMe2 || this.securityModel.getSecurityMode() == KeyguardSecurityModel.SecurityMode.SimPinPukMe3 || this.securityModel.getSecurityMode() == KeyguardSecurityModel.SecurityMode.SimPinPukMe4) {
            return true;
        }
        return false;
    }
    ...
    public void notifySecurityModeChange(KeyguardSecurityModel.SecurityMode currentMode, KeyguardSecurityModel.SecurityMode nextMode) {
        ...
        if (nextMode != KeyguardSecurityModel.SecurityMode.AlarmBoot && nextMode != KeyguardSecurityModel.SecurityMode.AntiTheft) {
            return;
        }
