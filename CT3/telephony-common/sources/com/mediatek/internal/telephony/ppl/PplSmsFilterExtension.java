...
public class PplSmsFilterExtension extends ContextWrapper implements IPplSmsFilter {
    ...
    public PplSmsFilterExtension(Context context) {
        ...
        this.mAgent = IPplAgent.Stub.asInterface(binder);
        if (this.mAgent == null) {
    ...
    public boolean pplFilter(Bundle params) {
        ...
        if (messages == null) {
            ...
            for (int i2 = 0; i2 < pduCount; i2++) {
                msgs[i2] = SmsMessage.createFromPdu(pdus[i2], format);
            }
        ...
        try {
            PplControlData controlData = PplControlData.buildControlData(this.mAgent.readControlData());
