...
public class TelephonyTester {
    ...
    private void handleTestDialogEventPackageIntent(Intent intent) {
        ...
        if (intent.hasExtra(EXTRA_SENDPACKAGE)) {
            ...
            if (!intent.hasExtra(EXTRA_DIALOGID)) {
                return;
            }
            ImsExternalCallState state = new ImsExternalCallState(intent.getIntExtra(EXTRA_DIALOGID, 0), Uri.parse(intent.getStringExtra("number")), intent.getBooleanExtra(EXTRA_CANPULL, true), intent.getIntExtra(EXTRA_STATE, 1), 2, false);
