...
public class Sms7BitEncodingTranslator {
    ...
    private static char translateIfNeeded(char c, boolean isCdmaFormat) {
        ...
        int translation = mTranslationTableCommon != null ? mTranslationTableCommon.get(c, -1) : -1;
        if (translation == -1) {
            if (isCdmaFormat) {
    ...
    private static void load7BitTranslationTableFromXml() {
        ...
        XmlResourceParser parser = r.getXml(R.bool.auto_data_switch_ping_test_before_switch);
        try {
            try {
                ...
                while (true) {
                    ...
                    if (XML_TRANSLATION_TYPE_TAG.equals(tag)) {
                        String type = parser.getAttributeValue(null, PplSmsFilterExtension.INSTRUCTION_KEY_TYPE);
                        if (DBG) {
