...
public class TestConferenceEventPackageParser {
    ...
    public ImsConferenceState parse() {
        ...
        try {
            ...
            Log.e(LOG_TAG, "Failed to read test conference event package from XML file", e3);
            try {
                this.mInputStream.close();
                return null;
    ...
    private Bundle parseParticipant(XmlPullParser parser) throws XmlPullParserException, IOException {
        ...
        String user = UsimPBMemInfo.STRING_NOT_SET;
        String displayText = UsimPBMemInfo.STRING_NOT_SET;
        String endpoint = UsimPBMemInfo.STRING_NOT_SET;
        String status = UsimPBMemInfo.STRING_NOT_SET;
        int outerDepth = parser.getDepth();
        while (XmlUtils.nextElementWithin(parser, outerDepth)) {
