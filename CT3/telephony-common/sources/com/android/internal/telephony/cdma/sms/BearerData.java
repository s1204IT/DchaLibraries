...
public final class BearerData {
    ...
    public static class TimeStamp extends Time {
        ...
        public static TimeStamp fromByteArray(byte[] data) {
            ...
            ((Time) ts).year = year >= 96 ? year + 1900 : year + ServiceStateTracker.NITZ_UPDATE_DIFF_DEFAULT;
            int month = IccUtils.cdmaBcdByteToInt(data[1]);
            if (month < 1 || month > 12) {
                return null;
            }
            ((Time) ts).month = month - 1;
            int day = IccUtils.cdmaBcdByteToInt(data[2]);
            if (day < 1 || day > 31) {
                return null;
            }
            ((Time) ts).monthDay = day;
        ...
        public String toString() {
            ...
            builder.append("{ year=").append(((Time) this).year);
            builder.append(", month=").append(((Time) this).month);
            builder.append(", day=").append(((Time) this).monthDay);
            builder.append(", hour=").append(((Time) this).hour);
            builder.append(", minute=").append(((Time) this).minute);
            builder.append(", second=").append(((Time) this).second);
            builder.append(" }");
    ...
    private static Gsm7bitCodingResult encode7bitGsm(String msg, int septetOffset, boolean force) throws CodingException {
        try {
            byte[] fullData = GsmAlphabet.stringToGsm7BitPacked(msg, septetOffset, force ? false : true, 0, 0);
            Gsm7bitCodingResult result = new Gsm7bitCodingResult(null);
            result.data = new byte[fullData.length - 1];
            System.arraycopy(fullData, 1, result.data, 0, fullData.length - 1);
            result.septets = fullData[0] & PplMessageManager.Type.INVALID;
            return result;
    ...
    private static void encodeUserDataPayload(UserData uData) throws CodingException {
        if (uData.payloadStr == null && uData.msgEncoding != 0) {
            ...
            uData.payloadStr = UsimPBMemInfo.STRING_NOT_SET;
        }
    ...
    private static String decode7bitAscii(byte[] data, int offset, int numFields) throws CodingException {
        try {
            ...
            int numFields2 = numFields - headerSeptets;
            StringBuffer strBuf = new StringBuffer(numFields2);
            BitwiseInputStream inStream = new BitwiseInputStream(data);
            int wantedBits = (headerSeptets + numFields2) * 7;
    ...
    private static void decodeUserDataPayload(UserData userData, boolean hasUserDataHeader) throws CodingException {
        ...
        switch (userData.msgEncoding) {
            case 0:
                boolean decodingtypeUTF8 = Resources.getSystem().getBoolean(R.^attr-private.hasRoundedCorners);
    ...
    private static void encode7bitAsciiEms(UserData uData, byte[] udhData, boolean force) throws CodingException {
        int udhBytes = udhData.length + 1;
        int udhSeptets = ((udhBytes * 8) + 6) / 7;
        int msgSeptets = uData.payloadStr.length();
        try {
            BitwiseOutputStream outStream = new BitwiseOutputStream((udhSeptets + msgSeptets) * 7);
            outStream.skip(udhSeptets * 7);
            for (int i = 0; i < msgSeptets; i++) {
                int charCode = UserData.charToAscii.get(uData.payloadStr.charAt(i), -1);
                if (charCode == -1) {
                    if (force) {
                        outStream.write(7, 32);
                    } else {
                        throw new CodingException("cannot ASCII encode (" + uData.payloadStr.charAt(i) + ")");
                    }
                } else {
                    outStream.write(7, charCode);
                }
            }
            uData.payload = outStream.toByteArray();
            uData.msgEncoding = 2;
            uData.msgEncodingSet = true;
            uData.numFields = udhSeptets + msgSeptets;
            uData.payload[0] = (byte) udhData.length;
            System.arraycopy(udhData, 0, uData.payload, 1, udhData.length);
    ...
    public static GsmAlphabet.TextEncodingDetails calcTextEncodingDetails(CharSequence msg, boolean force7BitEncoding, int encodingType) {
        ...
        GsmAlphabet.TextEncodingDetails ted2 = calcTextEncodingDetails(msg, force7BitEncoding, true);
        if (ted2.msgCount == 1 && ted2.codeUnitSize == 1) {
            ted2.codeUnitCount = msg.length();
            int octets = ted2.codeUnitCount * 2;
            if (octets > 140) {
                ted2.msgCount = (octets + 133) / 134;
                ted2.codeUnitsRemaining = ((ted2.msgCount * 134) - octets) / 2;
            } else {
                ted2.msgCount = 1;
    ...
    private static byte cdmaIntToBcdByte(int value) {
        ...
        byte ret = (byte) (((value2 / 10) << 4) | (value2 % 10));
        return ret;
    }
    ...
    private static void encodeMsgCenterTimeStamp(BearerData bData, BitwiseOutputStream outStream) throws BitwiseOutputStream.AccessException {
        outStream.write(8, 6);
        int year = ((Time) bData.msgCenterTimeStamp).year - 2000;
        if (year < 0) {
            year = ((Time) bData.msgCenterTimeStamp).year - 1900;
        }
        outStream.write(8, cdmaIntToBcdByte(year));
        outStream.write(8, cdmaIntToBcdByte(((Time) bData.msgCenterTimeStamp).month + 1));
        outStream.write(8, cdmaIntToBcdByte(((Time) bData.msgCenterTimeStamp).monthDay));
        outStream.write(8, cdmaIntToBcdByte(((Time) bData.msgCenterTimeStamp).hour));
        outStream.write(8, cdmaIntToBcdByte(((Time) bData.msgCenterTimeStamp).minute));
        outStream.write(8, cdmaIntToBcdByte(((Time) bData.msgCenterTimeStamp).second));
    }

    private static String getZoneId() {
        String id = SystemProperties.get("cdma.operator.nitztimezoneid", UsimPBMemInfo.STRING_NOT_SET);
        if (id.equals(UsimPBMemInfo.STRING_NOT_SET)) {
            return TimeZone.getDefault().getID();
        }
        String[] values = id.split(",");
        int i = 0;
        while (i < values.length && values[i].equals(UsimPBMemInfo.STRING_NOT_SET)) {
            i++;
        }
        if (ENG) {
            Rlog.d(LOG_TAG, "getZoneId() " + values[i] + " i = " + i);
        }
        return values[i];
    }
}
