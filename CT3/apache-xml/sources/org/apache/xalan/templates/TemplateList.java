...
public class TemplateList implements Serializable {
    ...
    public void setTemplate(ElemTemplate template) {
        ...
        if (template.getName() != null) {
            ...
            if (existingTemplate == null || (newPrecedence = template.getStylesheetComposed().getImportCountComposed()) > (existingPrecedence = existingTemplate.getStylesheetComposed().getImportCountComposed())) {
        ...
        ?? expression = matchXPath.getExpression();
        if (expression instanceof StepPattern) {
            insertPatternInTable(expression, template);
            return;
        }
        if (!(expression instanceof UnionPattern)) {
            return;
        }
        StepPattern[] pats = expression.getPatterns();
        for (StepPattern stepPattern : pats) {
            insertPatternInTable(stepPattern, template);
        }
    }
    ...
    private TemplateSubPatternAssociation insertAssociationIntoList(TemplateSubPatternAssociation head, TemplateSubPatternAssociation item, boolean isWildCardInsert) {
        ...
        if (next == null || insertPoint == head) {
            ...
            if (importLevel > insertPoint.getImportLevel()) {
                ...
                insertBefore = true;
            } else if (priority >= workPriority2 && docOrder >= insertPoint.getDocOrderPos()) {
                insertBefore = true;
            } else {
                insertBefore = false;
            }
    ...
    public TemplateSubPatternAssociation getHead(XPathContext xctxt, int targetNode, DTM dtm) {
        ...
        TemplateSubPatternAssociation head2 = this.m_wildCardPatterns;
        return head2;
    }
    ...
    public ElemTemplate getTemplateFast(XPathContext xctxt, int targetNode, int expTypeID, QName mode, int maxImportLevel, boolean quietConflictWarnings, DTM dtm) throws TransformerException {
        ...
        if (head == null && (head = this.m_wildCardPatterns) == null) {
            return null;
        }
    ...
    public class TemplateWalker {
        ...
        TemplateWalker(TemplateList this$0, TemplateWalker templateWalker) {
            this();
        }
        ...
        public ElemTemplate next() {
            ...
            do {
                ...
                ct = (ElemTemplate) this.m_compilerCache.get(new Integer(template.getUid()));
            } while (ct != null);
