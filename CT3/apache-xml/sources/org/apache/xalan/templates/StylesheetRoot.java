...
public class StylesheetRoot extends StylesheetComposed implements Serializable, Templates {
    ...
    public void recompose() throws TransformerException {
        ...
        for (int i2 = 0; i2 < n; i2++) {
            getGlobalImport(i2).recompose(recomposableElements);
        }
        ...
        for (int i3 = recomposableElements.size() - 1; i3 >= 0; i3--) {
            ((ElemTemplateElement) recomposableElements.elementAt(i3)).recompose(this);
        }
        initComposeState();
        ...
        for (int i4 = 0; i4 < n2; i4++) {
            StylesheetComposed imported = getGlobalImport(i4);
            int includedCount = imported.getIncludeCountComposed();
            for (int j2 = -1; j2 < includedCount; j2++) {
                Stylesheet included = imported.getIncludeComposed(j2);
    ...
    protected void addImports(Stylesheet stylesheet, boolean addToList, Vector importList) {
        ...
        if (n2 > 0) {
            for (int i2 = 0; i2 < n2; i2++) {
                Stylesheet included = stylesheet.getInclude(i2);
    ...
    public NamespaceAlias getNamespaceAliasComposed(String uri) {
        return (NamespaceAlias) (this.m_namespaceAliasComposed != null ? this.m_namespaceAliasComposed.get(uri) : null);
    }
    ...
    private void initDefaultRule(ErrorListener errorListener) throws TransformerException {
        ...
        XPath defMatch2 = new XPath("text() | @*", this, this, 1, errorListener);
        this.m_defaultTextRule.setMatch(defMatch2);
        ElemValueOf elemValueOf = new ElemValueOf();
        this.m_defaultTextRule.appendChild((ElemTemplateElement) elemValueOf);
        XPath selectPattern = new XPath(Constants.ATTRVAL_THIS, this, this, 0, errorListener);
        elemValueOf.setSelect(selectPattern);
        this.m_defaultRootRule = new ElemTemplate();
        this.m_defaultRootRule.setStylesheet(this);
        XPath defMatch3 = new XPath(PsuedoNames.PSEUDONAME_ROOT, this, this, 1, errorListener);
        this.m_defaultRootRule.setMatch(defMatch3);
    ...
    class ComposeState {
        ...
        int addVariableName(QName qname) {
            ...
            int frameSize = this.m_variableNames.size() - getGlobalsSize();
