...
public class Compiler extends OpMap {
    ...
    protected Expression variable(int opPos) throws TransformerException {
        ...
        String str = -2 == nsPos ? null : (String) getTokenQueue().elementAt(nsPos);
        String localname = (String) getTokenQueue().elementAt(getOp(opPos2 + 1));
        QName qname = new QName(str, localname);
        var.setQName(qname);
        return var;
    }
    ...
    Expression compileFunction(int opPos) throws TransformerException {
        ...
        int opPos3 = opPos2 + 1;
        if (-1 != funcID) {
            ?? function = this.m_functionTable.getFunction(funcID);
            if (function instanceof FuncExtFunctionAvailable) {
                function.setFunctionTable(this.m_functionTable);
            }
