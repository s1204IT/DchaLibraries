...
public class Variable extends Expression implements PathComponent {
    ...
    public ElemVariable getElemVariable() {
        ...
        ?? previousSiblingElem = expressionOwner;
        boolean z = previousSiblingElem instanceof Stylesheet;
        previousSiblingElem = previousSiblingElem;
        if (!z) {
            while (previousSiblingElem != 0 && !(previousSiblingElem.getParentNode() instanceof Stylesheet)) {
                ...
                while (true) {
                    ...
                    if (previousSiblingElem != 0) {
                        if (previousSiblingElem instanceof ElemVariable) {
                            if (previousSiblingElem.getName().equals(this.m_qname)) {
        ...
        if (previousSiblingElem != 0) {
            return previousSiblingElem.getStylesheetRoot().getVariableOrParamComposed(this.m_qname);
        }
        return elemVariable;
    }
    ...
    public int getAnalysisBits() {
        ...
        if (vvar != null && (xpath = vvar.getSelect()) != null && (expression = xpath.getExpression()) != null && (expression instanceof PathComponent)) {
