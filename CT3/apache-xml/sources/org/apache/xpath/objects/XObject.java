...
public class XObject extends Expression implements Serializable, Cloneable {
    ...
    public DocumentFragment rtree(XPathContext support) {
        ...
        if (-1 == result) {
            DTM frag = support.createDocumentFragment();
            frag.appendTextChild(str());
            DocumentFragment docFrag = (DocumentFragment) frag.getNode(frag.getDocument());
            return docFrag;
        }
        DTM frag2 = support.getDTM(result);
        DocumentFragment docFrag2 = (DocumentFragment) frag2.getNode(frag2.getDocument());
        return docFrag2;
    }
    ...
    public boolean deepEquals(Expression expr) {
        return isSameClass(expr) && equals((XObject) expr);
    }
}
