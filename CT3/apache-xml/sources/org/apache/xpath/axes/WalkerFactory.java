...
public class WalkerFactory {
    ...
    public static DTMIterator newDTMIterator(Compiler compiler, int opPos, boolean isTopLevel) throws TransformerException {
        ...
        if (isOneStep && walksSelfOnly(analysis) && isWild(analysis) && !hasPredicate(analysis)) {
            walkingIteratorSorted = new SelfIteratorNoPredicate(compiler, opPos, analysis);
        } else if (walksChildrenOnly(analysis) && isOneStep) {
            if (isWild(analysis) && !hasPredicate(analysis)) {
                walkingIteratorSorted = new ChildIterator(compiler, opPos, analysis);
            } else {
                walkingIteratorSorted = new ChildTestIterator(compiler, opPos, analysis);
            }
        } else if (isOneStep && walksAttributes(analysis)) {
            walkingIteratorSorted = new AttributeIterator(compiler, opPos, analysis);
            ...
            if (!walksNamespaces(analysis) && (walksInDocOrder(analysis) || isSet(analysis, BIT_PARENT))) {
                walkingIteratorSorted = new OneStepIteratorForward(compiler, opPos, analysis);
            } else {
                walkingIteratorSorted = new OneStepIterator(compiler, opPos, analysis);
            }
        } else if (isOptimizableForDescendantIterator(compiler, firstStepPos, 0)) {
            walkingIteratorSorted = new DescendantIterator(compiler, opPos, analysis);
        } else if (isNaturalDocOrder(compiler, firstStepPos, 0, analysis)) {
            walkingIteratorSorted = new WalkingIterator(compiler, opPos, analysis, true);
        } else {
            walkingIteratorSorted = new WalkingIteratorSorted(compiler, opPos, analysis, true);
        }
        if (walkingIteratorSorted instanceof LocPathIterator) {
            walkingIteratorSorted.setIsTopLevel(isTopLevel);
        }
    ...
    public static boolean canCrissCross(int analysis) {
        ...
        return ((walksDownOnly(analysis) && !canSkipSubtrees(analysis)) || walksChildrenAndExtraAndSelfOnly(analysis) || walksDescendantsAndExtraAndSelfOnly(analysis) || walksUpOnly(analysis) || walksExtraNodesOnly(analysis) || !walksSubtree(analysis) || (!walksSideways(analysis) && !walksUp(analysis) && !canSkipSubtrees(analysis))) ? false : true;
    }
