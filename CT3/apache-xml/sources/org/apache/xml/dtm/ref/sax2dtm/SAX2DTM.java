...
public class SAX2DTM extends DTMDefaultBaseIterators implements EntityResolver, DTDHandler, ContentHandler, ErrorHandler, DeclHandler, LexicalHandler {
    ...
    protected boolean nextNode() {
        ...
        ?? DeliverMoreNodes = this.m_incrementalSAXSource.deliverMoreNodes(true);
        if (!(DeliverMoreNodes instanceof Boolean)) {
            if (DeliverMoreNodes instanceof RuntimeException) {
                throw DeliverMoreNodes;
            }
            if (DeliverMoreNodes instanceof Exception) {
                throw new WrappedRuntimeException(DeliverMoreNodes);
            }
            clearCoRoutine();
            return false;
        }
    ...
    public String getUnparsedEntityURI(String name) {
        ...
        for (int i = 0; i < n; i += 4) {
            ...
            if (ename != null && ename.equals(name)) {
                ...
                String url = (String) this.m_entities.elementAt(i + 1);
                if (url == null) {
    ...
    public String getPrefix(String qname, String uri) {
        ...
        if (qname != null) {
            ...
            if (indexOfNSSep2 > 0) {
                if (qname.startsWith(Constants.ATTRNAME_XMLNS)) {
                    return qname.substring(indexOfNSSep2 + 1);
                }
                String prefix = qname.substring(0, indexOfNSSep2);
                return prefix;
            }
            if (qname.equals("xmlns")) {
                return "";
