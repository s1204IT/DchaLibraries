...
final class WriterToUTF8Buffered extends Writer implements WriterChain {
    ...
    public void write(String s) throws IOException {
        ...
        int i = 0;
        int count_loc3 = count_loc2;
        while (i < length) {
            char c2 = chars[i];
            if (c2 >= 128) {
                break;
            }
            buf_loc[count_loc3] = (byte) c2;
            i++;
            count_loc3++;
        }
        while (i < length) {
            char c3 = chars[i];
            if (c3 < 128) {
                count_loc = count_loc3 + 1;
                buf_loc[count_loc3] = (byte) c3;
            } else if (c3 < 2048) {
                int count_loc4 = count_loc3 + 1;
                buf_loc[count_loc3] = (byte) ((c3 >> 6) + 192);
                buf_loc[count_loc4] = (byte) ((c3 & '?') + 128);
                count_loc = count_loc4 + 1;
            } else if (c3 >= 55296 && c3 <= 56319) {
                i++;
                char low = chars[i];
                int count_loc5 = count_loc3 + 1;
                buf_loc[count_loc3] = (byte) ((((c3 + '@') >> 8) & 240) | 240);
                int count_loc6 = count_loc5 + 1;
                ...
                int count_loc8 = count_loc3 + 1;
                buf_loc[count_loc3] = (byte) ((c3 >> '\f') + 224);
                int count_loc9 = count_loc8 + 1;
            ...
            i++;
            count_loc3 = count_loc;
        }
        this.count = count_loc3;
    }
