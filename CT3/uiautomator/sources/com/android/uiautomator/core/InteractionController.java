...
class InteractionController {
    ...
    public boolean clickAndSync(int x, int y, long timeout) {
        ...
        return runAndWaitForEvents(clickRunnable(x, y), new WaitForAnyEventPredicate(2052), timeout) != null;
    }
    ...
    public boolean clickAndWaitForNewWindow(int x, int y, long timeout) {
        ...
        Log.d(LOG_TAG, logString);
        return runAndWaitForEvents(clickRunnable(x, y), new WaitForAllEventPredicate(2080), timeout) != null;
    }
    ...
    private boolean touchDown(int x, int y) {
        ...
        MotionEvent event = MotionEvent.obtain(this.mDownTime, this.mDownTime, 0, x, y, 1);
    ...
    public boolean swipe(int downX, int downY, int upX, int upY, int steps) {
        return swipe(downX, downY, upX, upY, steps, false);
    }
    ...
    public boolean performMultiPointerGesture(MotionEvent.PointerCoords[]... touches) {
        ...
        boolean ret = injectEventSync(event);
        for (int x3 = 1; x3 < touches.length; x3++) {
            MotionEvent event2 = MotionEvent.obtain(downTime, SystemClock.uptimeMillis(), getPointerAction(MOTION_EVENT_INJECTION_DELAY_MILLIS, x3), x3 + 1, properties, pointerCoords, 0, 0, 1.0f, 1.0f, 0, 0, 4098, 0);
        ...
        for (int x6 = 1; x6 < touches.length; x6++) {
            MotionEvent event4 = MotionEvent.obtain(downTime, SystemClock.uptimeMillis(), getPointerAction(6, x6), x6 + 1, properties, pointerCoords, 0, 0, 1.0f, 1.0f, 0, 0, 4098, 0);
