...
public class PerfServiceManager implements IPerfServiceManager {
    ...
    public void notifyFrameUpdate(int level) {
        if (level != 0) {
            nativePerfSetFavorPid(RENDER_BIT | level);
        }
    ...
    public boolean touchDisable() {
        ...
        this.mHandler.startCheckRenderAwareTimerMs(RENDER_AWARE_DURATION_MS);
        return true;
    }
    ...
    private class PerfServiceThreadHandler extends Handler {
            ...
            try {
                switch (msg.what) {
                    ...
                    case 4:
                        ...
                        if (passedObject.mState == 1 && PerfServiceManager.this.mCurrPack != passedObject.mPackName) {
                            try {
                                ...
                                ApplicationInfo info = PerfServiceManager.this.mPm.getApplicationInfo(PerfServiceManager.this.mCurrPack, 0);
                                int isSystem = info.isSystemApp() ? 1 : 0;
                                PerfServiceManager.nativePerfSetPackAttr(isSystem, 0);
                                PerfServiceManager.this.boostDisable(3);
                                int type = PerfServiceManager.nativePerfUserGetCapability(12);
                                if (type == 1) {
                    ...
                    default:
                        ...
                        if (msgId >= 100 && msgId < MESSAGE_TIMER_SCN_USER_BASE) {
                            ...
                            switch (scenario) {
                                case 7:
                                    ...
                                    float utilization = PerfServiceManager.this.mRuntime.getTargetHeapUtilization();
                                    PerfServiceManager.this.log("set utilization:" + utilization);
                                    break;
                            }
                            return;
                        }
                        if (msgId < MESSAGE_TIMER_SCN_USER_BASE) {
                            return;
                        }
        ...
        private void startCheckTimer(int scenario, int timeout) {
            ...
            msg.arg1 = scenario;
            sendMessageDelayed(msg, timeout * 1000);
        ...
        private void startCheckTimerMs(int scenario, int timeout_ms) {
            ...
            msg.what = scenario + 100;
            msg.arg1 = scenario;
            sendMessageDelayed(msg, timeout_ms);
            if (PerfServiceManager.this.mTimeList.contains(Integer.valueOf(scenario))) {
                return;
            }
            PerfServiceManager.this.mTimeList.add(Integer.valueOf(scenario));
        ...
        private void startCheckUserTimer(int handle, int timeout) {
            Message msg = obtainMessage();
            msg.what = handle + MESSAGE_TIMER_SCN_USER_BASE;
            msg.arg1 = handle;
            sendMessageDelayed(msg, timeout * 1000);
        ...
        private void stopCheckUserTimer(int handle) {
            int timer = handle + MESSAGE_TIMER_SCN_USER_BASE;
            removeMessages(timer);
        }
        ...
        private void stopAllUserTimer() {
            ...
            for (int i = 0; i < PerfServiceManager.this.mTimeList.size(); i++) {
                int handle = PerfServiceManager.this.mTimeList.get(i).intValue();
                if (handle < 17) {
                    timer = handle + 100;
                } else {
                    timer = handle + MESSAGE_TIMER_SCN_USER_BASE;
                }
                removeMessages(timer);
            }
        }
    ...
    private class SmartObserver extends UEventObserver {
        ...
        public void onUEvent(UEventObserver.UEvent event) {
            ...
            PerfServiceManager.this.setUeventIndex(index, action);
        }
    }
