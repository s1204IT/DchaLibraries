...
public final class LowStorageHandle {
    ...
    public void systemReadyLSM() {
        ...
        if (freeKBStorage <= 8192 && freeKBStorage > 4096) {
            ...
            piggyKBSize = (long) (freeKBStorage * 0.8d);
            LSMRemoveCoredump();
        }
        ...
        if (!f.exists()) {
            try {
                ...
                try {
                    byte[] buffer = new byte[PackageManagerService.DumpState.DUMP_VERIFIERS];
                    for (int dataWrite = 0; dataWrite < piggyKBSize / 2; dataWrite++) {
                        out.write(buffer, 0, PackageManagerService.DumpState.DUMP_VERIFIERS);
                    }
                } finally {
                    out.close();
                }
