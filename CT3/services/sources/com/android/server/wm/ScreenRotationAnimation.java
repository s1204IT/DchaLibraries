...
class ScreenRotationAnimation {
    ...
    public ScreenRotationAnimation(Context context, DisplayContent displayContent, SurfaceSession session, boolean inTransaction, boolean forceDefaultOrientation, boolean isSecure) {
        ...
        Trace.traceBegin(32L, "ScreenRotationAnimation:Create");
        ...
        int flags = isSecure ? 132 : 4;
        try {
            try {
                if (WindowManagerDebugConfig.DEBUG_SURFACE_TRACE) {
                    this.mSurfaceControl = new WindowSurfaceController.SurfaceTrace(session, "ScreenshotSurface", this.mWidth, this.mHeight, -1, flags);
    ...
    private void setSnapshotTransformInTransaction(Matrix matrix, float alpha) {
        if (this.mSurfaceControl != null) {
            ...
            Trace.traceBegin(32L, "ScreenRotationAnimation:SetMatrix dsdx=" + this.mTmpFloats[0] + " dtdx=" + this.mTmpFloats[3] + " dsdy=" + this.mTmpFloats[1] + " dtdy=" + this.mTmpFloats[4]);
            Trace.traceEnd(32L);
        }
    }
    ...
    private boolean startAnimation(SurfaceSession session, long maxAnimationDuration, float animationScale, int finalWidth, int finalHeight, boolean dismissing, int exitAnim, int enterAnim) {
        ...
        if (exitAnim == 0 || enterAnim == 0) {
            ...
            switch (delta) {
                case 0:
                    this.mRotateExitAnimation = AnimationUtils.loadAnimation(this.mContext, R.anim.overlay_task_fragment_close_to_top);
                    this.mRotateEnterAnimation = AnimationUtils.loadAnimation(this.mContext, R.anim.overlay_task_fragment_close_to_right);
                    break;
                case 1:
                    this.mRotateExitAnimation = AnimationUtils.loadAnimation(this.mContext, R.anim.push_down_in_no_alpha);
                    this.mRotateEnterAnimation = AnimationUtils.loadAnimation(this.mContext, R.anim.push_down_in);
                    break;
                case 2:
                    this.mRotateExitAnimation = AnimationUtils.loadAnimation(this.mContext, R.anim.overlay_task_fragment_open_from_right);
                    this.mRotateEnterAnimation = AnimationUtils.loadAnimation(this.mContext, R.anim.overlay_task_fragment_open_from_left);
                    break;
                case 3:
                    this.mRotateExitAnimation = AnimationUtils.loadAnimation(this.mContext, R.anim.progress_indeterminate_material);
                    this.mRotateEnterAnimation = AnimationUtils.loadAnimation(this.mContext, R.anim.progress_indeterminate_horizontal_rect2);
                    break;
            }
        } else {
            customAnim = true;
        ...
        try {
            try {
                ...
                this.mEnteringBlackFrame = new BlackFrame(session, outer2, inner2, 2010000, layerStack, false);
                SurfaceControl.closeTransaction();
                if (!WindowManagerDebugConfig.SHOW_LIGHT_TRANSACTIONS) {
                    return true;
                }
    ...
    public boolean dismiss(SurfaceSession session, long maxAnimationDuration, float animationScale, int finalWidth, int finalHeight, int exitAnim, int enterAnim) {
        Trace.traceBegin(32L, "ScreenRotationAnimation:Begin");
    ...
    public void kill() {
        if (this.mSurfaceControl != null) {
            ...
            Trace.traceBegin(32L, "ScreenRotationAnimation:End");
