...
final class Session extends IWindowSession.Stub implements IBinder.DeathRecipient {
    ...
    public void finishDrawing(IWindow window) {
        ...
        Trace.traceBegin(32L, "wmFinishDrawing");
    ...
    public boolean startMovingTask(IWindow window, float startX, float startY) {
        ...
        long ident = Binder.clearCallingIdentity();
        try {
            return this.mService.startMovingTask(window, startX, startY);
        } finally {
            Binder.restoreCallingIdentity(ident);
        }
    }
    ...
    public Bundle sendWallpaperCommand(IBinder window, String action, int x, int y, int z, Bundle extras, boolean sync) {
        ...
        synchronized (this.mService.mWindowMap) {
            long ident = Binder.clearCallingIdentity();
            try {
                bundleSendWindowWallpaperCommand = this.mService.mWallpaperControllerLocked.sendWindowWallpaperCommand(this.mService.windowForClientLocked(this, window, true), action, x, y, z, extras, sync);
            } finally {
                Binder.restoreCallingIdentity(ident);
            }
        }
