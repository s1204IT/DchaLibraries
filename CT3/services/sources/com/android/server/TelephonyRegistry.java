...
class TelephonyRegistry extends ITelephonyRegistry.Stub {
    ...
    private void listen(String callingPackage, IPhoneStateListener callback, int events, boolean notifyNow, int subId) {
        ...
        synchronized (this.mRecords) {
            ...
            r.callingPackage = callingPackage;
            r.callerUserId = callerUserId;
            boolean isPhoneStateEvent = (events & 16620) != 0;
            r.canReadPhoneState = isPhoneStateEvent ? canReadPhoneState(callingPackage) : false;
    ...
    public void notifyCallStateForPhoneInfo(int phoneType, int phoneId, int subId, int state, String incomingNumber) {
        if (checkNotifyPermission("notifyCallState()")) {
            if (VDBG) {
                log("notifyCallStateForPhoneInfo: phoneType=" + phoneType + " phoneId =" + phoneId + " subId=" + subId + " state=" + state + " incomingNumber=" + incomingNumber);
            }
            synchronized (this.mRecords) {
                if (validatePhoneId(phoneId)) {
                    this.mCallState[phoneId] = state;
        ...
        if (checkNotifyPermission("notifyServiceState()")) {
            synchronized (this.mRecords) {
                ...
                if (validatePhoneId(phoneId)) {
                    ...
                    for (Record r : this.mRecords) {
                        if (r.matchPhoneStateListenerEvent(1) && idMatch(r.subId, subId)) {
                            try {
        ...
        if (checkNotifyPermission("notifySignalStrength()")) {
            ...
            synchronized (this.mRecords) {
                if (validatePhoneId(phoneId)) {
                    ...
                    for (Record r : this.mRecords) {
                        if (r.matchPhoneStateListenerEvent(256) && idMatch(r.subId, subId)) {
                            try {
                        ...
                        } else if (!r.matchPhoneStateListenerEvent(2) && idMatch(r.subId, subId)) {
                            try {
    ...
    public void notifyCarrierNetworkChange(boolean active) {
        ...
        synchronized (this.mRecords) {
            ...
            for (Record r : this.mRecords) {
                if (r.matchPhoneStateListenerEvent(PackageManagerService.DumpState.DUMP_INSTALLS)) {
                    try {
    ...
    public void notifyCellInfoForSubscriber(int subId, List<CellInfo> cellInfo) {
        ...
        synchronized (this.mRecords) {
            ...
            if (validatePhoneId(phoneId)) {
                ...
                for (Record r : this.mRecords) {
                    if (validateEventsAndUserLocked(r, 1024) && idMatch(r.subId, subId)) {
                        try {
    ...
    public void notifyMessageWaitingChangedForPhoneId(int phoneId, int subId, boolean mwi) {
        ...
        synchronized (this.mRecords) {
            if (validatePhoneId(phoneId)) {
                ...
                for (Record r : this.mRecords) {
                    if (r.matchPhoneStateListenerEvent(4) && idMatch(r.subId, subId)) {
                        try {
    ...
    public void notifyCallForwardingChangedForSubscriber(int subId, boolean cfi) {
        ...
        synchronized (this.mRecords) {
            ...
            if (validatePhoneId(phoneId)) {
                ...
                for (Record r : this.mRecords) {
                    if (r.matchPhoneStateListenerEvent(8) && idMatch(r.subId, subId)) {
                        try {
            ...
            synchronized (this.mRecords) {
                ...
                if (validatePhoneId(phoneId)) {
                    ...
                    if (modified) {
                        ...
                        for (Record r : this.mRecords) {
                            if (r.matchPhoneStateListenerEvent(64) && idMatch(r.subId, subId)) {
                                try {
                        ...
                        this.mPreciseDataConnectionState[phoneId] = new PreciseDataConnectionState(state, networkType, apnType, apn, reason, linkProperties, "");
                        for (Record r2 : this.mRecords) {
                            if (r2.matchPhoneStateListenerEvent(4096) && idMatch(r2.subId, subId)) {
                                try {
                                    if (VDBG) {
    ...
    public void notifyDataConnectionFailedForSubscriber(int subId, String reason, String apnType) {
        ...
        synchronized (this.mRecords) {
            ...
            this.mPreciseDataConnectionState[phoneId] = new PreciseDataConnectionState(-1, 0, apnType, "", reason, null, "");
            for (Record r : this.mRecords) {
                if (r.matchPhoneStateListenerEvent(4096) && idMatch(r.subId, subId)) {
                    try {
                        r.callback.onPreciseDataConnectionStateChanged(this.mPreciseDataConnectionState[phoneId]);
                    } catch (RemoteException e) {
                        this.mRemoveList.add(r.binder);
    ...
    public void notifyCellLocationForSubscriber(int subId, Bundle cellLocation) {
        ...
        synchronized (this.mRecords) {
            ...
            if (validatePhoneId(phoneId)) {
                ...
                for (Record r : this.mRecords) {
                    if (validateEventsAndUserLocked(r, 16) && idMatch(r.subId, subId)) {
                        try {
    ...
    public void notifyPreciseCallState(int ringingCallState, int foregroundCallState, int backgroundCallState) {
        ...
        synchronized (this.mRecords) {
            ...
            for (Record r : this.mRecords) {
                if (r.matchPhoneStateListenerEvent(PackageManagerService.DumpState.DUMP_VERIFIERS)) {
                    try {
                        r.callback.onPreciseCallStateChanged(this.mPreciseCallState);
                    } catch (RemoteException e) {
                        this.mRemoveList.add(r.binder);
    ...
    public void notifyOemHookRawEventForSubscriber(int subId, byte[] rawData) {
        ...
        synchronized (this.mRecords) {
            for (Record r : this.mRecords) {
                ...
                if (r.matchPhoneStateListenerEvent(PackageManagerService.DumpState.DUMP_VERSION) && (r.subId == subId || r.subId == Integer.MAX_VALUE)) {
                    try {
    ...
    private void enforceCarrierPrivilege() {
        ...
        for (String pkg : pkgs) {
            if (tm.checkCarrierPrivilegesForPackage(pkg) == 1) {
                return;
            }
        }
    ...
    private boolean validateEventsAndUserLocked(Record r, int events) {
        ...
        try {
            ...
            boolean valid = r.callerUserId == foregroundUser ? r.matchPhoneStateListenerEvent(events) : false;
            return valid;
        } finally {
    ...
    private void logServiceStateChanged(String s, int subId, int phoneId, ServiceState state) {
        ...
        if (this.logSSC[this.next] == null) {
            this.logSSC[this.next] = new LogSSC(logSSC);
        }
        Time t = new Time();
        t.setToNow();
