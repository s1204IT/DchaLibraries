...
public class MediaFocusControl {
    ...
    private void removeFocusStackEntryForClient(IBinder cb) {
        ...
        if (this.mFocusStack.isEmpty()) {
            ...
            isTopOfStackForClientToRemove = this.mFocusStack.peek().hasSameBinder(cb);
        }
        Iterator<FocusRequester> stackIterator = this.mFocusStack.iterator();
        while (stackIterator.hasNext()) {
    ...
    private boolean isLockedFocusOwner(FocusRequester fr) {
        if (fr.hasSameClient("AudioFocus_For_Phone_Ring_And_Calls")) {
            return true;
        }
    ...
    void addFocusFollower(IAudioPolicyCallback ff) {
        ...
        synchronized (mAudioFocusLock) {
            boolean found = false;
            Iterator pcb$iterator = this.mFocusFollowers.iterator();
            while (true) {
                if (!pcb$iterator.hasNext()) {
                    break;
                }
                IAudioPolicyCallback pcb = (IAudioPolicyCallback) pcb$iterator.next();
    ...
    protected boolean isAppInFocus(String name) {
        ...
        synchronized (mAudioFocusLock) {
            if (!this.mFocusStack.empty()) {
                isInFocus = this.mFocusStack.peek().hasSamePackage(name);
            }
        }
        return isInFocus;
    }
