...
final class TaskRecord {
    ...
    TaskRecord(ActivityManagerService service, int _taskId, ActivityInfo info, Intent _intent, ActivityManager.TaskDescription _taskDescription, ActivityManager.TaskThumbnailInfo thumbnailInfo) {
        this.mLockTaskAuth = 1;
        this.mLockTaskUid = -1;
        this.lastTaskDescription = new ActivityManager.TaskDescription();
        this.isPersistable = false;
    ...
    private void setIntent(Intent _intent, ActivityInfo info) {
        ...
        if ((info.flags & PackageManagerService.DumpState.DUMP_PREFERRED_XML) != 0) {
        ...
        this.mPrivileged = (info.applicationInfo.privateFlags & 8) != 0;
        setLockTaskAuth();
    }
    ...
    void setTaskToReturnTo(int taskToReturnTo) {
        this.mTaskToReturnTo = taskToReturnTo == 2 ? 1 : taskToReturnTo;
            ...
            if (!r.finishing && r.realActivity.equals(newR.realActivity)) {
                ...
                while (activityNdx2 < numActivities) {
                    ...
                    if (!r2.finishing) {
                        ...
                        if (this.stack != null && this.stack.finishActivityLocked(r2, 0, null, "clear-task-stack", false)) {
                            activityNdx2--;
                            numActivities--;
                        }
                    }
                ...
                if (this.stack != null) {
                    this.stack.finishActivityLocked(r, 0, null, "clear-task-top", false);
                    return null;
                }
                return null;
            }
    ...
    public ActivityManager.TaskThumbnail getTaskThumbnailLocked() {
        ...
        if (this.stack != null && (resumedActivity = this.stack.mResumedActivity) != null && resumedActivity.task == this) {
            Bitmap thumbnail = this.stack.screenshotActivitiesLocked(resumedActivity);
    ...
    boolean isLockTaskWhitelistedLocked() {
        ...
        if (pkg == null || (packages = this.mService.mLockTaskPackages.get(this.userId)) == null) {
            return false;
        }
    ...
    boolean isResizeable() {
        ...
        return (this.mService.mForceResizableActivities || ActivityInfo.isResizeableMode(this.mResizeMode)) && !this.mTemporarilyUnresizable;
    }
    ...
    void updateTaskDescription() {
        int numActivities = this.mActivities.size();
        boolean relinquish = (numActivities == 0 || (this.mActivities.get(0).info.flags & 4096) == 0) ? false : true;
        ...
        while (true) {
            ...
            if (relinquish && (r.info.flags & 4096) == 0) {
                activityNdx++;
                break;
            } else if (r.intent != null && (r.intent.getFlags() & PackageManagerService.DumpState.DUMP_FROZEN) != 0) {
                break;
            } else {
                activityNdx++;
            }
        ...
        this.lastTaskDescription = new ActivityManager.TaskDescription(label, null, iconFilename, colorPrimary, colorBackground);
    ...
    void saveToXml(XmlSerializer out) throws XmlPullParserException, IOException {
        ...
        for (int activityNdx = 0; activityNdx < numActivities; activityNdx++) {
            ...
            if (((r.intent.getFlags() & PackageManagerService.DumpState.DUMP_FROZEN) | PackageManagerService.DumpState.DUMP_PREFERRED_XML) == 524288 && activityNdx > 0) {
                return;
            }
    ...
    static TaskRecord restoreFromXml(XmlPullParser in, ActivityStackSupervisor stackSupervisor) throws XmlPullParserException, IOException {
        ...
        for (int attrNdx = in.getAttributeCount() - 1; attrNdx >= 0; attrNdx--) {
            ...
            } else if (attrName.startsWith("task_thumbnailinfo_")) {
        ...
        while (true) {
            ...
            if (event == 2) {
                ...
                if (TAG_AFFINITYINTENT.equals(name)) {
                    intentRestoreFromXml2 = Intent.restoreFromXml(in);
        ...
        if (effectiveUid <= 0) {
            Intent checkIntent = intentRestoreFromXml != null ? intentRestoreFromXml : intentRestoreFromXml2;
            effectiveUid = 0;
            if (checkIntent != null) {
        ...
        if (this.mFullscreen) {
            if (this.mBounds != null && ActivityManager.StackId.persistTaskBounds(this.stack.mStackId)) {
                ...
                if (MultiWindowManager.isSupported()) {
                    this.mLastNonFullscreenBounds = new Rect(this.mBounds);
                }
            }
            this.mBounds = null;
            this.mOverrideConfig = Configuration.EMPTY;
            ...
            this.mOverrideConfig = calculateOverrideConfig(this.mTmpRect, insetBounds, this.mTmpRect.right != bounds.right, this.mTmpRect.bottom != bounds.bottom);
        }
    ...
    private void subtractNonDecorInsets(Rect inOutBounds, Rect inInsetBounds, boolean overrideWidth, boolean overrideHeight) {
        ...
        int leftInset = this.mTmpRect2.left - inInsetBounds.left;
        int topInset = this.mTmpRect2.top - inInsetBounds.top;
        int rightInset = overrideWidth ? 0 : inInsetBounds.right - this.mTmpRect2.right;
        int bottomInset = overrideHeight ? 0 : inInsetBounds.bottom - this.mTmpRect2.bottom;
    ...
    private Configuration calculateOverrideConfig(Rect bounds, Rect insetBounds, boolean overrideWidth, boolean overrideHeight) {
        ...
        subtractNonDecorInsets(this.mTmpNonDecorBounds, insetBounds != null ? insetBounds : bounds, overrideWidth, overrideHeight);
        subtractStableInsets(this.mTmpStableBounds, insetBounds != null ? insetBounds : bounds, overrideWidth, overrideHeight);
        Configuration serviceConfig = this.mService.mConfiguration;
        Configuration config = new Configuration(Configuration.EMPTY);
        float density = serviceConfig.densityDpi * 0.00625f;
        config.screenWidthDp = Math.min((int) (this.mTmpStableBounds.width() / density), serviceConfig.screenWidthDp);
        config.screenHeightDp = Math.min((int) (this.mTmpStableBounds.height() / density), serviceConfig.screenHeightDp);
        ...
        int sl = Configuration.resetScreenLayout(serviceConfig.screenLayout);
    ...
    void sanitizeOverrideConfiguration(Configuration globalConfig) {
        ...
        this.mOverrideConfig.screenLayout = (((newScreenLayout & (-49)) | (overrideScreenLayout & 48)) & (-16)) | (overrideScreenLayout & 15);
    ...
    Rect getLaunchBounds() {
        ...
        if (!ActivityManager.StackId.persistTaskBounds(stackId)) {
            return this.stack.mBounds;
        }
    ...
    private void setShowForAllUsers(ActivityInfo info) {
        this.mShowForAllUsers = (info.flags & 1024) != 0;
