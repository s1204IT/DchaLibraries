...
final class ProcessList {
    ...
    private static final long[] sFirstAwakePssTimes = {JobStatus.DEFAULT_TRIGGER_MAX_DELAY, JobStatus.DEFAULT_TRIGGER_MAX_DELAY, JobStatus.DEFAULT_TRIGGER_UPDATE_DELAY, 20000, 20000, 20000, 20000, 20000, 20000, 20000, 20000, 30000, 30000, 30000, 30000, 30000, 30000};
    static final long MAX_EMPTY_TIME = 1800000;
    private static final long[] sSameAwakePssTimes = {900000, 900000, JobStatus.DEFAULT_TRIGGER_MAX_DELAY, 900000, 900000, 900000, 900000, 900000, 900000, 900000, 1200000, 1200000, MAX_EMPTY_TIME, MAX_EMPTY_TIME, MAX_EMPTY_TIME, MAX_EMPTY_TIME, MAX_EMPTY_TIME};
    private static final long[] sTestFirstAwakePssTimes = {3000, 3000, 3000, 20000, 20000, 20000, DataShapingUtils.CLOSING_DELAY_BUFFER_FOR_MUSIC, DataShapingUtils.CLOSING_DELAY_BUFFER_FOR_MUSIC, DataShapingUtils.CLOSING_DELAY_BUFFER_FOR_MUSIC, DataShapingUtils.CLOSING_DELAY_BUFFER_FOR_MUSIC, DataShapingUtils.CLOSING_DELAY_BUFFER_FOR_MUSIC, DataShapingUtils.CLOSING_DELAY_BUFFER_FOR_MUSIC, DataShapingUtils.CLOSING_DELAY_BUFFER_FOR_MUSIC, DataShapingUtils.CLOSING_DELAY_BUFFER_FOR_MUSIC, DataShapingUtils.CLOSING_DELAY_BUFFER_FOR_MUSIC, DataShapingUtils.CLOSING_DELAY_BUFFER_FOR_MUSIC, DataShapingUtils.CLOSING_DELAY_BUFFER_FOR_MUSIC};
    ...
    static {
        ProcessADJ.INVALID_ADJ = INVALID_ADJ;
        ProcessADJ.UNKNOWN_ADJ = 1001;
        ProcessADJ.CACHED_APP_MAX_ADJ = CACHED_APP_MAX_ADJ;
        ProcessADJ.CACHED_APP_MIN_ADJ = CACHED_APP_MIN_ADJ;
        ProcessADJ.SERVICE_B_ADJ = SERVICE_B_ADJ;
        ProcessADJ.PREVIOUS_APP_ADJ = PREVIOUS_APP_ADJ;
        ProcessADJ.HOME_APP_ADJ = 600;
        ProcessADJ.SERVICE_ADJ = 500;
        ProcessADJ.HEAVY_WEIGHT_APP_ADJ = HEAVY_WEIGHT_APP_ADJ;
        ProcessADJ.BACKUP_APP_ADJ = BACKUP_APP_ADJ;
        ProcessADJ.PERCEPTIBLE_APP_ADJ = PERCEPTIBLE_APP_ADJ;
        ProcessADJ.VISIBLE_APP_ADJ = 100;
        ProcessADJ.FOREGROUND_APP_ADJ = 0;
        ProcessADJ.PERSISTENT_SERVICE_ADJ = PERSISTENT_SERVICE_ADJ;
        ProcessADJ.PERSISTENT_PROC_ADJ = PERSISTENT_PROC_ADJ;
        ProcessADJ.SYSTEM_ADJ = SYSTEM_ADJ;
        ProcessADJ.NATIVE_ADJ = -1000;
    }
    ...
    ProcessList() {
        ...
        this.mOomAdj = Resources.getSystem().getIntArray(R.array.config_lowMemoryKillerOomAdj);
        this.mOomMinFree = Resources.getSystem().getIntArray(R.array.config_lowMemoryKillerMinFreeKbytes);
        if (this.mOomAdj == null || this.mOomMinFree == null || this.mOomMinFree[0] == 0) {
            if (this.mTotalMemMb <= 512) {
                this.mOomAdj = new int[]{0, 100, PERCEPTIBLE_APP_ADJ, BACKUP_APP_ADJ, CACHED_APP_MIN_ADJ, CACHED_APP_MAX_ADJ};
    ...
    private void updateOomLevels(int displayWidth, int displayHeight, boolean write) {
        if (this.mTotalMemMb <= 1024) {
            ...
            if (write) {
                ...
                writeLmkd(buf);
                if (this.mTotalMemMb > 512) {
                    int reserve = Resources.getSystem().getInteger(R.integer.config_extraFreeKbytes);
                    if (reserve < 0) {
        ...
        float scaleDisp = ((displayWidth * displayHeight) - 384000.0f) / 640000;
        ...
        int minfree_adj = Resources.getSystem().getInteger(android.R.integer.autofill_max_visible_datasets);
        int minfree_abs = Resources.getSystem().getInteger(android.R.integer.auto_data_switch_validation_max_retry);
        boolean is64bit = Build.SUPPORTED_64_BIT_ABIS.length > 0;
        for (int i2 = 0; i2 < this.mOomAdj.length; i2++) {
            int low = this.mOomMinFreeLow[i2];
            int high = this.mOomMinFreeHigh[i2];
            if (is64bit) {
                if (i2 == 4) {
                    high = (high * 3) / 2;
                } else if (i2 == 5) {
            ...
            this.mOomMinFree[i2] = (int) (low + ((high - low) * scale));
        }
        if (minfree_abs >= 0) {
            for (int i3 = 0; i3 < this.mOomAdj.length; i3++) {
                this.mOomMinFree[i3] = (int) ((minfree_abs * this.mOomMinFree[i3]) / this.mOomMinFree[this.mOomAdj.length - 1]);
            }
        }
        if (minfree_adj != 0) {
            for (int i4 = 0; i4 < this.mOomAdj.length; i4++) {
                int[] iArr = this.mOomMinFree;
                iArr[i4] = iArr[i4] + ((int) ((minfree_adj * this.mOomMinFree[i4]) / this.mOomMinFree[this.mOomAdj.length - 1]));
                if (this.mOomMinFree[i4] < 0) {
                    this.mOomMinFree[i4] = 0;
                }
            }
        }
        this.mCachedRestoreLevel = (getMemLevel(CACHED_APP_MAX_ADJ) / 1024) / 3;
        Slog.i(TAG, "mCachedRestoreLevel = " + Long.toString(this.mCachedRestoreLevel));
        int reserve2 = (((displayWidth * displayHeight) * 4) * 3) / 1024;
        int reserve_adj = Resources.getSystem().getInteger(android.R.integer.bugreport_state_show);
        int reserve_abs = Resources.getSystem().getInteger(android.R.integer.bugreport_state_hide);
        if (reserve_abs >= 0) {
            reserve2 = reserve_abs;
        }
        if (reserve_adj != 0 && (reserve2 = reserve2 + reserve_adj) < 0) {
            reserve2 = 0;
        }
        if (!write) {
            return;
        ...
        for (int i5 = 0; i5 < this.mOomAdj.length; i5++) {
            buf2.putInt((this.mOomMinFree[i5] * 1024) / 4096);
            buf2.putInt(this.mOomAdj[i5]);
        }
        writeLmkd(buf2);
        SystemProperties.set("sys.sysctl.extra_free_kbytes", Integer.toString(reserve2));
    }
