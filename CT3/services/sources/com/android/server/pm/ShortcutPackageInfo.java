...
class ShortcutPackageInfo {
    ...
    public static ShortcutPackageInfo newEmpty() {
        return new ShortcutPackageInfo(-1, new ArrayList(0), false);
    }
    ...
    public boolean canRestoreTo(ShortcutService s, PackageInfo target) {
        ...
        if (target.versionCode < this.mVersionCode) {
            Slog.w(TAG, String.format("Can't restore: package current version %d < backed up version %d", Integer.valueOf(target.versionCode), Integer.valueOf(this.mVersionCode)));
            return false;
        }
        if (BackupUtils.signaturesMatch(this.mSigHashes, target)) {
            return true;
        }
    ...
    public static ShortcutPackageInfo generateForInstalledPackage(ShortcutService s, String packageName, int packageUserId) {
        ...
        ShortcutPackageInfo ret = new ShortcutPackageInfo(pi.versionCode, BackupUtils.hashSignatureArray(pi.signatures), false);
        return ret;
    }
    ...
    public void saveToXml(XmlSerializer out) throws IOException {
        ...
        for (int i = 0; i < this.mSigHashes.size(); i++) {
            ...
            ShortcutService.writeAttr(out, ATTR_SIGNATURE_HASH, Base64.encode(this.mSigHashes.get(i)));
            out.endTag(null, TAG_SIGNATURE);
        }
        out.endTag(null, TAG_ROOT);
    }

    public void loadFromXml(XmlPullParser parser, boolean fromBackup) throws XmlPullParserException, IOException {
        int versionCode = ShortcutService.parseIntAttribute(parser, ATTR_VERSION);
        boolean booleanAttribute = !fromBackup ? ShortcutService.parseBooleanAttribute(parser, ATTR_SHADOW) : true;
        ...
        this.mVersionCode = versionCode;
        this.mIsShadow = booleanAttribute;
