...
final class VoiceInteractionSessionConnection implements ServiceConnection {
    ...
    public boolean showLocked(Bundle args, int flags, int disabledContext, IVoiceInteractionSessionShowCallback showCallback, IBinder activityToken, List<IBinder> topActivities) {
        if (this.mBound) {
            ...
            boolean structureEnabled = isAssistDataAllowed && (disabledContext2 & 1) == 0;
            boolean screenshotEnabled = isAssistDataAllowed && structureEnabled && (disabledContext2 & 2) == 0;
            this.mShowArgs = args;
        ...
        if (this.mHaveAssistData) {
            if (this.mAssistData.isEmpty()) {
                ...
                while (!this.mAssistData.isEmpty()) {
                    ...
                    this.mPendingAssistDataCount--;
                    AssistDataForActivity assistData = this.mAssistData.remove(0);
                    if (assistData.data == null) {
                        try {
                            this.mSession.handleAssist((Bundle) null, (AssistStructure) null, (AssistContent) null, assistData.activityIndex, assistData.activityCount);
                        } catch (RemoteException e2) {
                        }
                    } else {
    ...
    private void deliverSessionDataLocked(AssistDataForActivity assistDataForActivity) {
        ...
        if (uid >= 0 && content != null) {
            ...
            ClipData data2 = content.getClipData();
            if (data2 != null) {
                grantClipDataPermissions(data2, 1, uid, this.mCallingUid, this.mSessionComponentName.getPackageName());
            }
        }
        try {
