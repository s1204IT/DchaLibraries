...
public final class TwilightService extends SystemService {
            ...
            public void unregisterListener(TwilightListener listener) {
                synchronized (TwilightService.this.mLock) {
                    for (int i = 0; i < TwilightService.this.mListeners.size(); i++) {
                        if (TwilightService.this.mListeners.get(i).mListener == listener) {
    ...
    public void onStart() {
        ...
        this.mLocationHandler = new LocationHandler(this, null);
    ...
    public void onBootPhase(int phase) {
        ...
        getContext().getContentResolver().registerContentObserver(Settings.Secure.getUriFor("twilight_mode"), false, this.mContentObserver, this.mCurrentUser);
    ...
    private final class LocationHandler extends Handler {
        ...
        LocationHandler(TwilightService this$0, LocationHandler locationHandler) {
            this();
        }
        ...
        public void handleMessage(Message msg) {
            ...
            try {
                zIsProviderEnabled = TwilightService.this.mLocationManager.isProviderEnabled("network");
            } catch (Exception e) {
                zIsProviderEnabled = false;
            }
            if (!this.mNetworkListenerEnabled && zIsProviderEnabled) {
            ...
            try {
                zIsProviderEnabled2 = TwilightService.this.mLocationManager.isProviderEnabled("passive");
            } catch (Exception e2) {
                zIsProviderEnabled2 = false;
            }
            if (!this.mPassiveListenerEnabled && zIsProviderEnabled2) {
        ...
        private void updateTwilightState() {
            ...
            if (isNight) {
                if (todaySunrise == -1 || todaySunset == -1) {
                    ...
                    amount = Math.min(1.0f, (now - todaySunset) / 7200000.0f);
                } else {
                    amount = Math.max(0.0f, 1.0f - ((todaySunrise - now) / 7200000.0f));
                }
            }
