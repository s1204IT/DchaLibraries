...
public class Searchables {
    ...
    private boolean isInstalled(ComponentName globalSearch) {
        ...
        List<ResolveInfo> activities = queryIntentActivities(intent, PackageManagerService.DumpState.DUMP_INSTALLS);
        if (activities != null && !activities.isEmpty()) {
            return true;
        }
        return false;
    ...
    private List<ResolveInfo> queryIntentActivities(Intent intent, int flags) {
        try {
            List<ResolveInfo> activities = this.mPm.queryIntentActivities(intent, intent.resolveTypeIfNeeded(this.mContext.getContentResolver()), flags, this.mUserId).getList();
            return activities;
