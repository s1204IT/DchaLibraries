...
class LockSettingsStorage {
    ...
    private SparseArray<Integer> mStoredCredentialType = new SparseArray<>();
    ...
    public int getStoredCredentialType(int userId) throws Throwable {
        int storedCredentialType;
        Integer cachedStoredCredentialType = this.mStoredCredentialType.get(userId);
        ...
        if (pattern == null) {
            ...
            if (password != null && password.version == 1) {
                storedCredentialType = 2;
            } else {
                storedCredentialType = 1;
            }
    ...
    public CredentialHash readPasswordHash(int userId) throws Throwable {
        byte[] stored = readFile(getLockPasswordFilename(userId));
        if (stored != null && stored.length > 0) {
            return new CredentialHash(stored, 1);
        }
        byte[] stored2 = readFile(getLegacyLockPasswordFilename(userId));
        if (stored2 == null || stored2.length <= 0) {
            return null;
        }
        return new CredentialHash(stored2, 0);
    }

    public CredentialHash readPatternHash(int userId) throws Throwable {
        ...
        if (stored2 != null && stored2.length > 0) {
            return new CredentialHash(stored2, true);
        }
        byte[] stored3 = readFile(getLegacyLockPatternFilename(userId));
        if (stored3 == null || stored3.length <= 0) {
            return null;
        }
        return new CredentialHash(stored3, 0);
    }
    ...
    public void writeChildProfileLock(int userId, byte[] lock) throws Throwable {
    ...
    public void writePatternHash(byte[] hash, int userId) throws Throwable {
        this.mStoredCredentialType.put(userId, Integer.valueOf(hash == null ? -1 : 1));
        writeFile(getLockPatternFilename(userId), hash);
        clearPasswordHash(userId);
    }

    private void clearPatternHash(int userId) throws Throwable {
        writeFile(getLockPatternFilename(userId), null);
    }

    public void writePasswordHash(byte[] hash, int userId) throws Throwable {
        this.mStoredCredentialType.put(userId, Integer.valueOf(hash == null ? -1 : 2));
        writeFile(getLockPasswordFilename(userId), hash);
        clearPatternHash(userId);
    }

    private void clearPasswordHash(int userId) throws Throwable {
    ...
    private static class Cache {
        ...
        private synchronized void put(int type, String key, Object value, int userId) {
            this.mCache.put(new CacheKey(null).set(type, key, userId), value);
            this.mVersion++;
        }
