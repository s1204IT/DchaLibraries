...
final class ColorFade {
    ...
    public boolean prepare(Context context, int mode) {
        ...
        if (!((createSurface() && createEglContext() && createEglSurface()) ? captureScreenshotTextureAndSetViewport() : false)) {
            dismiss();
            return false;
        }
        if (!attachEglContext()) {
    ...
    public boolean draw(float level) {
        ...
        try {
            ...
            GLES20.glClear(PackageManagerService.DumpState.DUMP_KEYSETS);
            double one_minus_level = 1.0f - level;
            double cos = Math.cos(3.141592653589793d * one_minus_level);
            double sign = cos < 0.0d ? -1 : 1;
            float opacity = ((float) (-Math.pow(one_minus_level, 2.0d))) + 1.0f;
            float saturation = (float) Math.pow(level, 4.0d);
            float scale = (float) ((((-Math.pow(one_minus_level, 2.0d)) + 1.0d) * 0.1d) + 0.9d);
            float gamma = (float) ((((0.5d * sign * Math.pow(cos, 2.0d)) + 0.5d) * 0.9d) + 0.1d);
