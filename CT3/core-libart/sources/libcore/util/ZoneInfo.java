...
public final class ZoneInfo extends TimeZone {
    ...
    public static ZoneInfo makeTimeZone(String id, BufferIterator it, long currentTimeMillis) {
        ...
        for (int i2 = 0; i2 < tzh_typecnt; i2++) {
            gmtOffsets[i2] = it.readInt();
            byte b = it.readByte();
            if (b != 0 && b != 1) {
                throw new IllegalStateException(id + " dst at " + i2 + " is not 0 or 1, is " + ((int) b));
            }
            isDsts[i2] = b;
            it.skip(1);
        }
    ...
    private ZoneInfo(String name, long[] transitions, byte[] types, int[] gmtOffsets, byte[] isDsts, long currentTimeMillis) {
        ...
        if (this.mTransitions.length == 0) {
            ...
            this.mRawOffset = gmtOffsets[this.mTypes[lastStd] & UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED];
        }
        if (lastDst != -1) {
            long lastDSTTransitionTime = this.mTransitions[lastDst];
        ...
        if (lastDst == -1) {
            ...
            int lastGmtOffset = gmtOffsets[this.mTypes[lastStd] & UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED];
            int lastDstOffset = gmtOffsets[this.mTypes[lastDst] & UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED];
        ...
        while (true) {
            ...
            if (this.mIsDsts[this.mTypes[i2] & UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED] != 0) {
                i2++;
            } else {
                firstStd = i2;
                break;
        ...
        int earliestRawOffset = firstStd != -1 ? gmtOffsets[this.mTypes[firstStd] & UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED] : this.mRawOffset;
    ...
    public int getOffset(int era, int year, int month, int day, int dayOfWeek, int millis) {
        long calc = ((long) (year / 400)) * MILLISECONDS_PER_400_YEARS;
        int year2 = year % 400;
        long calc2 = calc + (((long) year2) * 31536000000L) + (((long) ((year2 + 3) / 4)) * 86400000);
        if (year2 > 0) {
            calc2 -= ((long) ((year2 - 1) / 100)) * 86400000;
        }
        boolean isLeap = year2 == 0 || (year2 % 4 == 0 && year2 % 100 != 0);
        int[] mlen = isLeap ? LEAP : NORMAL;
        return getOffset(((((calc2 + (((long) mlen[month]) * 86400000)) + (((long) (day - 1)) * 86400000)) + ((long) millis)) - ((long) this.mRawOffset)) - UNIX_OFFSET);
    }
    ...
    public int getOffsetsByUtcTime(long utcTimeInMillis, int[] offsets) {
        ...
        if (transitionIndex == -1) {
            ...
            if (this.mIsDsts[type] == 0) {
                ...
                rawOffset = -1;
                int transitionIndex2 = transitionIndex - 1;
                while (true) {
                    if (transitionIndex2 < 0) {
                        break;
                    }
                    int type2 = this.mTypes[transitionIndex2] & 255;
                    if (this.mIsDsts[type2] != 0) {
                        transitionIndex2--;
                    } else {
                        rawOffset = this.mRawOffset + (this.mOffsets[type2] * 1000);
                        break;
                    }
                }
                if (rawOffset == -1) {
    ...
    public boolean inDaylightTime(Date time) {
        ...
        int offsetIndex = findOffsetIndexForTimeInMilliseconds(when);
        return offsetIndex != -1 && this.mIsDsts[offsetIndex] == 1;
    }
    ...
    public boolean hasSameRules(TimeZone timeZone) {
        ...
        if (this.mRawOffset == other.mRawOffset && Arrays.equals(this.mOffsets, other.mOffsets) && Arrays.equals(this.mIsDsts, other.mIsDsts) && Arrays.equals(this.mTypes, other.mTypes)) {
            return Arrays.equals(this.mTransitions, other.mTransitions);
        }
        return false;
    }
    ...
    public static class WallTime {
        ...
        public void localtime(int timeSeconds, ZoneInfo zoneInfo) {
            ...
            try {
                ...
                if (zoneInfo.mTransitions.length == 0 || (offsetIndex = zoneInfo.findOffsetIndexForTimeInSeconds(timeSeconds)) == -1) {
                    b = 0;
                } else {
                    offsetSeconds += zoneInfo.mOffsets[offsetIndex];
                    b = zoneInfo.mIsDsts[offsetIndex];
                }
        ...
        private static int[] getOffsetsOfType(ZoneInfo zoneInfo, int startIndex, int isDst) {
            ...
            } while (!(clampTop ? clampBottom : false));
                ...
                if ((transitionIndexDelta <= 0 || !clampTop) && (transitionIndexDelta >= 0 || !clampBottom)) {
                    ...
                    if (offsetInterval == null) {
                        ...
                        if (mustMatchDst) {
                            ...
                            Integer returnValue2 = tryOffsetAdjustments(zoneInfo, wallTimeSeconds, offsetInterval, currentTransitionIndex, isDstToFind);
    ...
    private static class CheckedArithmeticException extends Exception {
        CheckedArithmeticException(CheckedArithmeticException checkedArithmeticException) {
            this();
        }
    ...
    private static int checkedAdd(long a, int b) throws CheckedArithmeticException {
        ...
        if (result != ((int) result)) {
            throw new CheckedArithmeticException(null);
        }
        return (int) result;
    }
