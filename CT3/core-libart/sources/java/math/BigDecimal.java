...
public class BigDecimal extends Number implements Comparable<BigDecimal>, Serializable {
    ...
    private static final int[] f124javamathRoundingModeSwitchesValues = null;
    ...
    private static int[] m336getjavamathRoundingModeSwitchesValues() {
        if (f124javamathRoundingModeSwitchesValues != null) {
            return f124javamathRoundingModeSwitchesValues;
        }
        ...
        f124javamathRoundingModeSwitchesValues = iArr;
        return iArr;
    }
    ...
    public BigDecimal(char[] in, int offset, int len) {
        ...
        if (offset > last || in[offset] != '.') {
            ...
            this.scale = offset3 - offset2;
            bufLength += this.scale;
            unscaledBuffer.append(in, offset2, this.scale);
            offset = offset3;
        }
        if (offset <= last && (in[offset] == 'e' || in[offset] == 'E')) {
            int offset4 = offset + 1;
            int begin2 = offset4;
            if (offset4 <= last && in[offset4] == '+') {
                int offset5 = offset4 + 1;
                if (offset5 <= last && in[offset5] != '-') {
                    begin2 = offset4 + 1;
                }
            }
            String scaleString = String.valueOf(in, begin2, (last + 1) - begin2);
    ...
    public BigDecimal(double val) {
        ...
        long mantissa = this.scale == 1075 ? (4503599627370495L & bits) << 1 : (4503599627370495L & bits) | 4503599627370496L;
        if (mantissa == 0) {
            this.scale = 0;
            this.precision = 1;
        }
        ...
        if (this.scale > 0) {
            if (this.scale < LONG_FIVE_POW.length && LONG_FIVE_POW_BIT_LENGTH[this.scale] + mantissaBits < 64) {
    ...
    public BigDecimal add(BigDecimal augend, MathContext mc) {
        ...
        if (largerSignum == smaller.signum()) {
            ...
            BigInteger tempBI2 = larger.getUnscaledValue().subtract(BigInteger.valueOf(largerSignum));
            tempBI = Multiplication.multiplyByPositiveInt(tempBI2, 10).add(BigInteger.valueOf(largerSignum * 9));
        }
    ...
    public BigDecimal subtract(BigDecimal subtrahend) {
        ...
        if (diffScale > 0) {
            ...
            return new BigDecimal(getUnscaledValue().subtract(Multiplication.multiplyByTenPow(subtrahend.getUnscaledValue(), diffScale)), this.scale);
        }
        ...
        return new BigDecimal(Multiplication.multiplyByTenPow(getUnscaledValue(), diffScale2).subtract(subtrahend.getUnscaledValue()), subtrahend.scale);
    }
    ...
    public BigDecimal subtract(BigDecimal subtrahend, MathContext mc) {
        ...
        if (subtrahend.approxPrecision() < diffScale - 1 && mc.getPrecision() < approxPrecision()) {
            ...
            if (thisSignum != subtrahend.signum()) {
                ...
                BigInteger tempBI2 = getUnscaledValue().subtract(BigInteger.valueOf(thisSignum));
                tempBI = Multiplication.multiplyByPositiveInt(tempBI2, 10).add(BigInteger.valueOf(thisSignum * 9));
            }
    ...
    public BigDecimal divide(BigDecimal divisor, int scale, RoundingMode roundingMode) {
        ...
        if (this.bitLength < 64 && divisor.bitLength < 64) {
            ...
            } else if ((-diffScale) < MathUtils.LONG_POWERS_OF_TEN.length && this.bitLength + LONG_POWERS_OF_TEN_BIT_LENGTH[(int) (-diffScale)] < 64) {
    ...
    private static BigDecimal divideBigIntegers(BigInteger scaledDividend, BigInteger scaledDivisor, int scale, RoundingMode roundingMode) {
        ...
        if (scaledDivisor.bitLength() < 63) {
            ...
            int compRem2 = longCompareTo(Math.abs(rem) * 2, Math.abs(divisor));
        ...
        if (compRem != 0) {
            ...
            return new BigDecimal(quotient.add(BigInteger.valueOf(compRem)), scale);
        }
        return new BigDecimal(quotient, scale);
    }
    ...
    private static BigDecimal dividePrimitiveLongs(long scaledDividend, long scaledDivisor, int scale, RoundingMode roundingMode) {
        ...
        if (remainder != 0) {
            int compRem = longCompareTo(Math.abs(remainder) * 2, Math.abs(scaledDivisor));
            quotient += (long) roundingBehavior(((int) quotient) & 1, (compRem + 5) * sign, roundingMode);
        }
    ...
    public BigDecimal divide(BigDecimal divisor) {
        ...
        int i2 = k - l;
        return new BigDecimal(i2 > 0 ? Multiplication.multiplyByFivePow(p2, i2) : p2.shiftLeft(-i2), newScale);
    }
    ...
    public BigDecimal divide(BigDecimal divisor, MathContext mc) {
        ...
        if (quotAndRem2[1].signum() != 0) {
            int compRem = quotAndRem2[1].shiftLeftOneBit().compareTo(divisor.getUnscaledValue());
            integerQuot = integerQuot.multiply(BigInteger.TEN).add(BigInteger.valueOf(quotAndRem2[0].signum() * (compRem + 5)));
            newScale++;
        } else {
    ...
    public BigDecimal divideToIntegralValue(BigDecimal divisor) {
        ...
        if (((long) divisor.approxPrecision()) + newScale > ((long) approxPrecision()) + 1 || isZero()) {
            ...
            BigInteger integralValue2 = getUnscaledValue().divide(divisor.getUnscaledValue().multiply(powerOfTen));
            integralValue = integralValue2.multiply(powerOfTen);
        } else {
    ...
    public BigDecimal divideToIntegralValue(BigDecimal divisor, MathContext mc) {
        ...
        if (quotPrecision <= 0) {
            ...
            newScale = Math.min(diffScale, Math.max((((long) mcPrecision) - quotPrecision) + 1, 0L));
        ...
        while (!strippedBI.testBit(0)) {
            BigInteger[] quotAndRem2 = strippedBI.divideAndRemainder(TEN_POW[i]);
            if (quotAndRem2[1].signum() == 0 && (resultPrecision - ((long) i) >= mcPrecision || newScale - ((long) i) >= diffScale)) {
                resultPrecision -= (long) i;
                newScale -= (long) i;
                if (i < lastPow) {
                    i++;
                }
                strippedBI = quotAndRem2[0];
            } else {
                if (i == 1) {
                    break;
                }
    ...
    private BigDecimal movePoint(long newScale) {
        ...
        if ((-newScale) < MathUtils.LONG_POWERS_OF_TEN.length && this.bitLength + LONG_POWERS_OF_TEN_BIT_LENGTH[(int) (-newScale)] < 64) {
    ...
    public boolean equals(Object x) {
        ...
        if (x1.scale == this.scale && x1.bitLength == this.bitLength) {
            return this.bitLength < 64 ? x1.smallValue == this.smallValue : x1.intVal.equals(this.intVal);
        }
        return false;
    }
    ...
    public String toString() {
        ...
        long exponent = ((-this.scale) + ((long) end)) - ((long) begin);
    ...
    public BigInteger toBigIntegerExact() {
        ...
        BigInteger[] integerAndFraction = getUnscaledValue().divideAndRemainder(Multiplication.powerOf10(this.scale));
    ...
    public double doubleValue() {
        ...
        if (this.scale <= 0) {
            mantissa = mantissa2.multiply(Multiplication.powerOf10(-this.scale));
        } else {
            ...
            mantissa = quotAndRem[0].shiftLeft(2).add(BigInteger.valueOf((((compRem + 3) * compRem) / 2) + 1));
            exponent2 -= 2;
        }
        ...
        if (exponent <= 0) {
            ...
            if ((3 & bits4) == 3 || ((1 & bits4) == 1 && tempBits2 != 0 && lowestSetBit < discardedSize)) {
                bits4++;
            }
            exponent = 0;
            bits2 = bits4 >> 1;
    ...
    private void inplaceRound(MathContext mc) {
        ...
        if (integerAndFraction[1].signum() != 0) {
            ...
            BigDecimal tempBD = new BigDecimal(integerAndFraction[0]);
    ...
    private void smallRound(MathContext mc, int discardedPrecision) {
        ...
        if (fraction != 0) {
            int compRem = longCompareTo(Math.abs(fraction) * 2, sizeOfFraction);
            integer += (long) roundingBehavior(((int) integer) & 1, Long.signum(fraction) * (compRem + 5), mc.getRoundingMode());
    ...
    private static int roundingBehavior(int parityBit, int fraction, RoundingMode roundingMode) {
        switch (m336getjavamathRoundingModeSwitchesValues()[roundingMode.ordinal()]) {
            case 1:
                int increment = Math.max(Integer.signum(fraction), 0);
                return increment;
            case 2:
            default:
                return 0;
            case 3:
                int increment2 = Math.min(Integer.signum(fraction), 0);
                return increment2;
            case 4:
                if (Math.abs(fraction) <= 5) {
                    return 0;
                }
                int increment3 = Integer.signum(fraction);
                return increment3;
            case 5:
                if (Math.abs(fraction) + parityBit <= 5) {
                    return 0;
                }
                int increment4 = Integer.signum(fraction);
                return increment4;
            case 6:
                if (Math.abs(fraction) < 5) {
                    return 0;
                }
                int increment5 = Integer.signum(fraction);
                return increment5;
            case 7:
                if (fraction == 0) {
                    return 0;
                ...
                throw new ArithmeticException("Rounding necessary");
            case 8:
                int increment6 = Integer.signum(fraction);
                return increment6;
        }
    }
