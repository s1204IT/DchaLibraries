...
public final class UCharacter implements UCharacterEnums.ECharacterCategory, UCharacterEnums.ECharacterDirection {
    ...
    public static final class UnicodeBlock extends Character.Subset {
        ...
        static {
            for (int blockId = 0; blockId < 263; blockId++) {
        ...
        public static final UnicodeBlock forName(String blockName) {
            ...
            if (mref != null) {
                Map<String, UnicodeBlock> m2 = mref.get();
                m = m2;
            }
            if (m == null) {
    ...
    public static boolean isLegal(int ch) {
        ...
        return ch > 57343 && !UCharacterUtility.isNonCharacter(ch) && ch <= 1114111;
    }
    ...
    private static class StringContextIterator implements UCaseProps.ContextIterator {
        ...
        public int next() {
            ...
            if (this.dir < 0 && this.index > 0) {
                int c2 = this.s.codePointBefore(this.index);
    ...
    public static String toUpperCase(ULocale locale, String str) {
        ...
        while (true) {
            ...
            if (c >= 0) {
                int c2 = UCaseProps.INSTANCE.toFullUpper(c, iter, result, locale, locCache);
                if (c2 < 0) {
                    c2 = ~c2;
                } else if (c2 <= 31) {
                }
    ...
    public static String toLowerCase(ULocale locale, String str) {
        ...
        while (true) {
            int c = iter.nextCaseMapCP();
            if (c >= 0) {
                int c2 = UCaseProps.INSTANCE.toFullLower(c, iter, result, locale, locCache);
                if (c2 < 0) {
                    c2 = ~c2;
                } else if (c2 <= 31) {
                }
        ...
        for (int prev = 0; prev < srcLength; prev = index) {
            ...
            if (prev < index) {
                ...
                if ((options & 512) == 0 && UCaseProps.INSTANCE.getType(c) == 0) {
                    ...
                    if (prev < titleStart) {
                        result.append((CharSequence) str, prev, titleStart);
                    }
                } else {
                    titleStart = prev;
                }
                if (titleStart < index) {
                    ...
                    int c2 = UCaseProps.INSTANCE.toFullTitle(c, iter, result, locale, locCache);
                    while (true) {
                        if (c2 < 0) {
                            c2 = ~c2;
                            result.appendCodePoint(c2);
                        ...
                        if ((options & 256) != 0) {
                            ...
                            if (titleLimit < index) {
                                if (isDutch && c2 == 73 && str.charAt(titleLimit) == 'j') {
                                    result.append('J').append((CharSequence) str, titleLimit + 1, index);
                                } else {
                                    result.append((CharSequence) str, titleLimit, index);
                                }
                            }
                            iter.moveToLimit();
                        } else {
    ...
    public static String toTitleFirst(ULocale locale, String str) {
        ...
        while (true) {
            ...
            if ((propertyMask & BREAK_MASK) != 0) {
                break;
            }
            if (UCaseProps.INSTANCE.getType(c) == 0) {
                i += charCount(c);
            } else {
                String substring = str.substring(i, charCount(c) + i);
                String titled = toTitleCase(locale, substring, BreakIterator.getSentenceInstance(locale), 0);
                if (titled.codePointAt(0) != c) {
                    StringBuilder result = new StringBuilder(str.length()).append((CharSequence) str, 0, i);
                    ...
                    return result.append((CharSequence) str, startOfSuffix, str.length()).toString();
                }
            }
        }
    }
    ...
    public static final String foldCase(String str, int options) {
        ...
        while (i < length) {
            ...
            int c2 = UCaseProps.INSTANCE.toFullFolding(c, result, options);
            if (c2 < 0) {
                c2 = ~c2;
            } else if (c2 <= 31) {
            }
    ...
    private static final class DummyValueIterator implements ValueIterator {
        DummyValueIterator(DummyValueIterator dummyValueIterator) {
            this();
        }
    ...
    public static String getStringPropertyValue(int propertyEnum, int codepoint, int nameChoice) {
        if ((propertyEnum >= 0 && propertyEnum < 57) || (propertyEnum >= 4096 && propertyEnum < 4118)) {
        ...
        if (codePointOffset < 0) {
            while (true) {
                ...
                if (codePointOffset <= 0) {
                    ...
                    while (ch >= 56320 && ch <= 57343 && index > 0) {
                        index--;
            ...
            while (true) {
                ...
                index2++;
                while (ch2 >= 55296 && ch2 <= 56319 && index2 < limit) {
                    int index3 = index2 + 1;
                    ch2 = text.charAt(index2);
                    if ((ch2 < 56320 || ch2 > 57343) && codePointOffset - 1 < 0) {
                        return index3 - 1;
                    }
                    index2 = index3;
                }
    ...
    public static int offsetByCodePoints(char[] text, int start, int count, int index, int codePointOffset) {
        ...
        if (codePointOffset < 0) {
            while (true) {
                ...
                while (ch >= 56320 && ch <= 57343 && index > start) {
                    index--;
                    ch = text[index];
                    if (ch < 55296 || ch > 56319) {
                        codePointOffset++;
            ...
            while (true) {
                ...
                int index3 = index2 + 1;
                char ch2 = text[index2];
                if (index3 > limit) {
                    throw new IndexOutOfBoundsException("index ( " + index3 + ") > limit (" + limit + ")");
                }
                while (true) {
                    index2 = index3;
                    if (ch2 < 55296 || ch2 > 56319 || index2 >= limit) {
                        break;
                    }
                    index3 = index2 + 1;
                    ch2 = text[index2];
