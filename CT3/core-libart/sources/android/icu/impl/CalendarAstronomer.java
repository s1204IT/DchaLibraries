...
public class CalendarAstronomer {
    ...
    public Horizon eclipticToHorizon(double eclipLong) {
        ...
        double azimuth = Math.atan2((-cosD) * cosL * sinH, sinD - (Math.sin(altitude) * sinL));
    ...
    public long getSunRiseSet(boolean rise) {
        ...
        setTime(((rise ? -6L : 6L) * RelativeDateTimeFormatter.HOUR_IN_MILLIS) + noon);
    ...
    public Equatorial getMoonPosition() {
        if (this.moonPosition == null) {
            ...
            double a4 = 0.0037350045992678655d * Math.sin(2.0d * meanAnomalyMoon2);
    ...
    private long timeOfAngle(AngleFunc func, double desired, double periodDays, long epsilon, boolean next) {
        ...
        double deltaT = (((next ? 0.0d : -6.283185307179586d) + deltaAngle) * (8.64E7d * periodDays)) / PI2;
    ...
    private long riseOrSet(CoordFunc func, boolean rise, double diameter, double refraction, long epsilon) {
        ...
        do {
            ...
            double lst = ((pos.ascension + angle) * 24.0d) / PI2;
    ...
    private static String radToHms(double angle) {
        ...
        int sec = (int) ((((angle * RAD_HOUR) - ((double) hrs)) - (((double) min) / 60.0d)) * 3600.0d);
        return Integer.toString(hrs) + "h" + min + DateFormat.MINUTE + sec + DateFormat.SECOND;
    }

    private static String radToDms(double angle) {
        int deg = (int) (angle * RAD_DEG);
        int min = (int) (((angle * RAD_DEG) - ((double) deg)) * 60.0d);
        int sec = (int) ((((angle * RAD_DEG) - ((double) deg)) - (((double) min) / 60.0d)) * 3600.0d);
