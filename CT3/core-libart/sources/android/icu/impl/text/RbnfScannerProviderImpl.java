...
public class RbnfScannerProviderImpl implements RbnfLenientScannerProvider {
    ...
    protected RbnfLenientScanner createScanner(ULocale locale, String extras) {
        ...
        RbnfLenientScannerImpl rbnfLenientScannerImpl = null;
        try {
            collator = (RuleBasedCollator) Collator.getInstance(locale.toLocale());
            if (extras != null) {
                String rules = collator.getRules() + extras;
                RuleBasedCollator collator2 = new RuleBasedCollator(rules);
                collator = collator2;
            }
        ...
        return new RbnfLenientScannerImpl(collator, rbnfLenientScannerImpl);
    }

    private static class RbnfLenientScannerImpl implements RbnfLenientScanner {
        private final RuleBasedCollator collator;

        RbnfLenientScannerImpl(RuleBasedCollator rbc, RbnfLenientScannerImpl rbnfLenientScannerImpl) {
            this(rbc);
        }
        ...
        public int prefixLength(String str, String prefix) {
            ...
            while (oPrefix != -1) {
                ...
                if (oStr == -1 || CollationElementIterator.primaryOrder(oStr) != CollationElementIterator.primaryOrder(oPrefix)) {
                    return 0;
                }
                oStr = strIter.next();
                oPrefix = prefixIter.next();
