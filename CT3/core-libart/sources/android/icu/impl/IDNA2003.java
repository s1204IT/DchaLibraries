...
public final class IDNA2003 {
    ...
    public static StringBuffer convertToASCII(UCharacterIterator src, int options) throws StringPrepParseException {
        ...
        if (useSTD3ASCIIRules && (!srcIsLDH || processOut.charAt(0) == '-' || processOut.charAt(processOut.length() - 1) == '-')) {
            if (!srcIsLDH) {
                String string = processOut.toString();
                if (failPos > 0) {
                    failPos--;
                }
                throw new StringPrepParseException("The input does not conform to the STD 3 ASCII rules", 5, string, failPos);
            }
            ...
            String string2 = processOut.toString();
            if (poLen > 0) {
                poLen--;
            }
            throw new StringPrepParseException("The input does not conform to the STD 3 ASCII rules", 5, string2, poLen);
        }
        if (srcIsASCII2) {
            dest = processOut;
        } else if (!startsWithPrefix(processOut)) {
            boolean[] caseFlags = new boolean[poLen];
    ...
    public static StringBuffer convertToUnicode(UCharacterIterator src, int options) throws StringPrepParseException {
        ...
        if (startsWithPrefix(processOut)) {
            ...
            try {
                stringBuffer = new StringBuffer(Punycode.decode(temp, null));
            } catch (StringPrepParseException e2) {
                stringBuffer = null;
            }
            if (stringBuffer != null) {
                StringBuffer toASCIIOut = convertToASCII(UCharacterIterator.getInstance(stringBuffer), options);
