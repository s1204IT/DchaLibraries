...
public class BOCSU {
    ...
    private static final int writeDiff(int diff, byte[] buffer, int offset) {
        if (diff >= SLOPE_REACH_NEG_1_) {
            ...
            if (diff <= SLOPE_REACH_POS_2_) {
                int offset3 = offset + 1;
                buffer[offset] = (byte) ((diff / 253) + 210);
                int offset4 = offset3 + 1;
                buffer[offset3] = (byte) ((diff % 253) + 3);
                return offset4;
            }
        ...
        if (diff >= SLOPE_REACH_NEG_2_) {
            ...
            buffer[offset5] = (byte) (modulo + 3);
            return offset6;
        }
        if (diff >= SLOPE_REACH_NEG_3_) {
