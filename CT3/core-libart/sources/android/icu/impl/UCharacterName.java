...
public final class UCharacterName {
    ...
    public int getGroupLengths(int index, char[] offsets, char[] lengths) {
        byte length = UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED;
        int index2 = index * this.m_groupsize_;
        int stringoffset = UCharacterUtility.toInt(this.m_groupinfo_[index2 + 1], this.m_groupinfo_[index2 + 2]);
        offsets[0] = 0;
        int i = 0;
        while (i < 32) {
            ...
            for (int shift = 4; shift >= 0; shift -= 4) {
                ...
                if (length == 65535 && n > 11) {
                    ...
                    length = UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED;
                    i++;
                }
            }
            stringoffset++;
        ...
        synchronized (this.m_utilStringBuffer_) {
            ...
            while (i < length) {
                ...
                if (b >= this.m_tokentable_.length) {
                    ...
                    if (token == 65534) {
                        token = this.m_tokentable_[(b << 8) | (this.m_groupstring_[index + i] & UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED)];
                        i++;
                    }
                    if (token == 65535) {
                        if (b == 59) {
    ...
    public synchronized String getGroupName(int ch, int choice) {
        ...
        if (msb == this.m_groupinfo_[this.m_groupsize_ * group]) {
            ...
            int offset = ch & 31;
            return getGroupName(this.m_groupoffsets_[offset] + index, this.m_grouplengths_[offset], choice);
        }
        return null;
    }
    ...
    private int getGroupChar(int index, char[] cArr, String name, int choice) {
        int nindex;
        ...
        for (int result = 0; result <= 32; result++) {
            ...
            if (choice != 0 && choice != 2) {
                ...
                do {
                    int oldindex = index;
                    index += UCharacterUtility.skipByteSubString(this.m_groupstring_, index, c, (byte) 59);
                    c -= index - oldindex;
                    fieldIndex--;
                } while (fieldIndex > 0);
            }
            int count = 0;
            int nindex2 = 0;
            while (count < c && nindex2 != -1 && nindex2 < namelen) {
                byte b = this.m_groupstring_[index + count];
                count++;
                if (b >= this.m_tokentable_.length) {
                    nindex = nindex2 + 1;
                    if (name.charAt(nindex2) != (b & UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED)) {
                        nindex = -1;
                    }
                } else {
                    char token = this.m_tokentable_[b & UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED];
                    if (token == 65534) {
                        token = this.m_tokentable_[(b << 8) | (this.m_groupstring_[index + count] & UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED)];
                        count++;
                    }
                    if (token == 65535) {
                        nindex = nindex2 + 1;
                        ...
                        nindex = UCharacterUtility.compareNullTermByteSubString(name, this.m_tokenstring_, nindex2, token);
                    }
                }
                nindex2 = nindex;
            }
            if (namelen == nindex2 && (count == c || this.m_groupstring_[index + count] == 59)) {
                return result;
            }
            index += c;
        }
    ...
    private int[] addGroupName(int offset, int length, byte[] tokenlength, int[] set) {
        ...
        while (resultplength < length) {
            ...
            if (b >= this.m_tokentable_.length) {
                ...
                if (token == 65534) {
                    b = (char) ((b << '\b') | (this.m_groupstring_[offset + resultplength] & UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED));
