...
final class NFRule {
    ...
    static final boolean f75assertionsDisabled;
    ...
    static {
        f75assertionsDisabled = !NFRule.class.desiredAssertionStatus();
        ZERO = 0L;
    ...
    public static void makeRules(String description, NFRuleSet owner, NFRule predecessor, RuleBasedNumberFormat ownersOwner, List<NFRule> returnList) {
        ...
        if (brack2 < 0 || brack1 > brack2 || rule1.baseValue == -3 || rule1.baseValue == -1 || rule1.baseValue == -5 || rule1.baseValue == -6) {
            ...
            if ((rule1.baseValue > 0 && rule1.baseValue % Math.pow(rule1.radix, rule1.exponent) == 0.0d) || rule1.baseValue == -2 || rule1.baseValue == -4) {
    ...
    private String parseRuleDescriptor(String description) {
        ...
        if (p != -1) {
            ...
            } while (PatternProps.isWhiteSpace(description.charAt(p)));
            ...
            if (firstChar >= '0' && firstChar <= '9' && lastChar != 'x') {
                ...
                while (p2 < descriptorLength) {
                    c = descriptor.charAt(p2);
                    if (c >= '0' && c <= '9') {
                        tempValue = (10 * tempValue) + ((long) (c - '0'));
                    } else {
                        if (c == '/' || c == '>') {
                            break;
                        }
                ...
                if (c == '/') {
                    ...
                    while (p2 < descriptorLength) {
                        c = descriptor.charAt(p2);
                        if (c >= '0' && c <= '9') {
                            tempValue2 = (10 * tempValue2) + ((long) (c - '0'));
                        } else {
                            if (c == '>') {
                                break;
                            }
    ...
    public boolean equals(Object obj) {
        if ((obj instanceof NFRule) && this.baseValue == obj.baseValue && this.radix == obj.radix && this.exponent == obj.exponent && this.ruleText.equals(obj.ruleText) && Utility.objectEquals(this.sub1, obj.sub1)) {
            return Utility.objectEquals(this.sub2, obj.sub2);
        }
        return false;
    }
    ...
    public String toString() {
        ...
        if (this.baseValue == -1) {
            ...
            result.append(ULocale.PRIVATE_USE_EXTENSION).append(this.decimalPoint != 0 ? this.decimalPoint : '.').append("x: ");
        } else if (this.baseValue == -3) {
            result.append('0').append(this.decimalPoint != 0 ? this.decimalPoint : '.').append("x: ");
        } else if (this.baseValue == -4) {
            result.append(ULocale.PRIVATE_USE_EXTENSION).append(this.decimalPoint != 0 ? this.decimalPoint : '.').append("0: ");
    ...
    public void doFormat(long number, StringBuffer toInsertInto, int pos, int recursionCount) {
        ...
        if (this.rulePatternFormat == null) {
            ...
            toInsertInto.insert(pos, this.rulePatternFormat.format((long) (number / Math.pow(this.radix, this.exponent))));
    ...
    public void doFormat(double number, StringBuffer toInsertInto, int pos, int recursionCount) {
        ...
        if (this.rulePatternFormat == null) {
            ...
            if (0.0d <= number && number < 1.0d) {
                pluralVal = Math.round(Math.pow(this.radix, this.exponent) * number);
            } else {
                pluralVal = number / Math.pow(this.radix, this.exponent);
            }
    ...
    public boolean shouldRollBack(double number) {
        return ((this.sub1 != null && this.sub1.isModulusSubstitution()) || (this.sub2 != null && this.sub2.isModulusSubstitution())) && number % Math.pow((double) this.radix, (double) this.exponent) == 0.0d && ((double) this.baseValue) % Math.pow((double) this.radix, (double) this.exponent) != 0.0d;
    }
    ...
    public Number doParse(String text, ParsePosition parsePosition, boolean isFractionRule, double upperBound) {
        ...
        do {
            ...
            if (pp.getIndex() != 0 || this.sub1 == null) {
                ...
                double partialResult2 = matchToDelimiter(workText2, 0, partialResult, this.ruleText.substring(sub2Pos), this.rulePatternFormat, pp2, this.sub2, upperBound).doubleValue();
                if ((pp2.getIndex() != 0 || this.sub2 == null) && pp.getIndex() + prefixLength + pp2.getIndex() > highWaterMark) {
    ...
    private Number matchToDelimiter(String text, int startPos, double baseVal, String delimiter, PluralFormat pluralFormatDelimiter, ParsePosition pp, NFSubstitution sub, double upperBound) {
        ...
        Number result = ZERO;
        Number tempResult2 = sub.doParse(text, tempPP2, baseVal, upperBound, this.formatter.lenientParseEnabled());
