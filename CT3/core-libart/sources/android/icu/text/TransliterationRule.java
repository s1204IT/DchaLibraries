...
class TransliterationRule {
    ...
    public int getAnteContextLength() {
        return ((this.flags & 1) != 0 ? 1 : 0) + this.anteContextLength;
    }
    ...
    public boolean masks(TransliterationRule r2) {
        ...
        if (left == left2 && right == right2 && this.keyLength <= r2.keyLength && r2.pattern.regionMatches(0, this.pattern, 0, len)) {
            ...
            return ((r2.flags & 1) == 0 || (r2.flags & 2) == 0) ? false : true;
        }
        if (left > left2) {
            return false;
        }
        if (right < right2 || (right == right2 && this.keyLength <= r2.keyLength)) {
            return r2.pattern.regionMatches(left2 - left, this.pattern, 0, len);
        }
        return false;
    }
    ...
    public int matchAndReplace(Replaceable text, Transliterator.Position pos, boolean incremental) {
        ...
        if (this.postContext != null) {
            ...
            int match2 = this.postContext.matches(text, intRef, pos.contextLimit, incremental);
            if (match2 != 2) {
                return match2;
            }
        }
