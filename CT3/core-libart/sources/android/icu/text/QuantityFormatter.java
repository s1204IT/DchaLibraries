...
class QuantityFormatter {
    ...
    static final boolean f85assertionsDisabled;
    private final SimplePatternFormatter[] templates = new SimplePatternFormatter[StandardPlural.COUNT];

    static {
        f85assertionsDisabled = !QuantityFormatter.class.desiredAssertionStatus();
    }
    ...
    public String format(double number, NumberFormat numberFormat, PluralRules pluralRules) {
        ...
        SimplePatternFormatter formatter = this.templates[p.ordinal()];
        if (formatter == null) {
    ...
    public SimplePatternFormatter getByVariant(CharSequence variant) {
        if (!f85assertionsDisabled && !isValid()) {
            throw new AssertionError();
        }
