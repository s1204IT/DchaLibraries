...
class RBBIRuleBuilder {
    ...
    void flattenData(OutputStream os) throws IOException {
        ...
        int totalSize = forwardTableSize + 96 + reverseTableSize + safeFwdTableSize + safeRevTableSize + statusTableSize + trieSize + rulesSize;
        int outputPos = 0;
        ...
        for (short s : tableData) {
            dos.writeShort(s);
            outputPos += 2;
        }
        short[] tableData2 = this.fReverseTables.exportTable();
        Assert.assrt(outputPos == header[6]);
        for (short s2 : tableData2) {
            dos.writeShort(s2);
            outputPos += 2;
        }
        Assert.assrt(outputPos == header[8]);
        short[] tableData3 = this.fSafeFwdTables.exportTable();
        for (short s3 : tableData3) {
            dos.writeShort(s3);
            outputPos += 2;
        }
        Assert.assrt(outputPos == header[10]);
        short[] tableData4 = this.fSafeRevTables.exportTable();
        for (short s4 : tableData4) {
            dos.writeShort(s4);
            outputPos += 2;
        }
