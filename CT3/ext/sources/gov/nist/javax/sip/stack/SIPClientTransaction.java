...
public class SIPClientTransaction extends SIPTransaction implements ServerResponseInterface, ClientTransaction, ClientTransactionExt {
    ...
    public boolean isMessagePartOfTransaction(SIPMessage messageToTest) {
        ...
        if (getBranch() == null || messageBranch == null || !getBranch().toLowerCase().startsWith(SIPConstants.BRANCH_MAGIC_COOKIE_LOWER_CASE)) {
            ...
            zStartsWith = messageBranch.toLowerCase().startsWith(SIPConstants.BRANCH_MAGIC_COOKIE_LOWER_CASE);
        }
        if (TransactionState.COMPLETED == getState()) {
            if (zStartsWith) {
                ...
                boolean transactionMatches = getMethod().equals(messageToTest.getCSeq().getMethod());
                return transactionMatches;
            }
            boolean transactionMatches2 = getBranch().equals(messageToTest.getTransactionId());
        ...
        if (zStartsWith) {
            ...
            boolean transactionMatches3 = getOriginalRequest().getCSeq().getMethod().equals(messageToTest.getCSeq().getMethod());
            return transactionMatches3;
        }
        if (getBranch() != null) {
            boolean transactionMatches4 = getBranch().equalsIgnoreCase(messageToTest.getTransactionId());
            return transactionMatches4;
        }
        boolean transactionMatches5 = getOriginalRequest().getTransactionId().equalsIgnoreCase(messageToTest.getTransactionId());
        return transactionMatches5;
    }
