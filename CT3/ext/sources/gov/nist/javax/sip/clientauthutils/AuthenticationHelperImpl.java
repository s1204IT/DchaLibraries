...
public class AuthenticationHelperImpl implements AuthenticationHelper {
    ...
    private AuthorizationHeader getAuthorization(String method, String uri, String requestBody, WWWAuthenticateHeader authHeader, UserCredentials userCredentials) {
        ...
        String str = qopList != null ? "auth" : null;
        String response = MessageDigestAlgorithm.calculateResponse(authHeader.getAlgorithm(), userCredentials.getUserName(), authHeader.getRealm(), userCredentials.getPassword(), authHeader.getNonce(), "00000001", "xyz", method, uri, requestBody, str, this.sipStack.getStackLogger());
        try {
    ...
    private AuthorizationHeader getAuthorization(String method, String uri, String requestBody, WWWAuthenticateHeader authHeader, UserCredentialHash userCredentials) {
        ...
        String qopList = authHeader.getQop();
        String str = qopList != null ? "auth" : null;
        String response = MessageDigestAlgorithm.calculateResponse(authHeader.getAlgorithm(), userCredentials.getHashUserDomainPassword(), authHeader.getNonce(), "00000001", "xyz", method, uri, requestBody, str, this.sipStack.getStackLogger());
        try {
            if (authHeader instanceof ProxyAuthenticateHeader) {
                authorization = this.headerFactory.createProxyAuthorizationHeader(authHeader.getScheme());
            } else {
