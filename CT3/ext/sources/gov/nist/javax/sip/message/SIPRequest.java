...
public final class SIPRequest extends SIPMessage implements Request, RequestExt {
    ...
    protected void setDefaults() {
        String method;
        ?? uri;
        if (this.requestLine == null || (method = this.requestLine.getMethod()) == null || (uri = this.requestLine.getUri()) == 0) {
            return;
        }
        if ((method.compareTo("REGISTER") != 0 && method.compareTo("INVITE") != 0) || !(uri instanceof SipUri)) {
            return;
        }
        uri.setUserParam(DEFAULT_USER);
        try {
    ...
    public boolean equals(Object other) {
        ...
        if (this.requestLine.equals(that.requestLine)) {
        ...
        while (headers.hasNext()) {
            ...
            if (!(next instanceof RouteList) && !(next instanceof ProxyAuthorization)) {
                if (next instanceof ContentLength) {
                    nextHeader = (SIPHeader) next.clone();
                    try {
                    ...
                    if (next instanceof CSeq) {
                        ...
                        nextHeader = responseToHeader != null ? responseToHeader : (SIPHeader) next.clone();
                    } else if (!(next instanceof ContactList) && !(next instanceof Expires)) {
                        nextHeader = next instanceof ViaList ? (SIPHeader) next.getFirst().clone() : (SIPHeader) next.clone();
                    }
                }
                try {
    ...
    public SIPRequest createSIPRequest(RequestLine requestLine, boolean switchHeaders) {
        ...
        while (headerIterator.hasNext()) {
            SIPHeader next = headerIterator.next();
            if (next instanceof CSeq) {
                CSeq newCseq = (CSeq) next.clone();
                next = newCseq;
                try {
                ...
                Via via = (Via) next.getFirst().clone();
                ...
                ContentLength cl = (ContentLength) next.clone();
                try {
