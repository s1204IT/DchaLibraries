...
public class SipProviderImpl implements SipProvider, SipProviderExt, SIPTransactionEventListener, SIPDialogEventListener {
    ...
    public void handleEvent(EventObject eventObject, SIPTransaction transaction) {
        if (this.sipStack.isLoggingEnabled()) {
            this.sipStack.getStackLogger().logDebug("handleEvent " + eventObject + "currentTransaction = " + transaction + "this.sipListener = " + getSipListener() + "sipEvent.source = " + eventObject.getSource());
            if (eventObject instanceof RequestEvent) {
                Dialog dialog = eventObject.getDialog();
                if (this.sipStack.isLoggingEnabled()) {
                    this.sipStack.getStackLogger().logDebug("Dialog = " + dialog);
                }
            } else if (eventObject instanceof ResponseEvent) {
                Dialog dialog2 = eventObject.getDialog();
                if (this.sipStack.isLoggingEnabled()) {
        ...
        EventWrapper eventWrapper = new EventWrapper(eventObject, transaction);
    ...
    public ClientTransaction getNewClientTransaction(Request request) throws TransactionUnavailableException {
        ...
        try {
            ...
            if (request.getMethod().equalsIgnoreCase(Request.CANCEL) && (ct = (SIPClientTransaction) this.sipStack.findCancelTransaction((SIPRequest) request, false)) != null) {
                SIPClientTransaction sIPClientTransactionCreateClientTransaction = this.sipStack.createClientTransaction((SIPRequest) request, ct.getMessageChannel());
                ...
                if (ct.getDialog() != null) {
                    sIPClientTransactionCreateClientTransaction.setDialog((SIPDialog) ct.getDialog(), sipRequest.getDialogId(false));
                }
                ...
                if (SipStackImpl.isDialogCreated(sipRequest.getMethod())) {
                    ...
                    if (!isAutomaticDialogSupportEnabled()) {
                        ...
                        if (transaction2 != null) {
                            ...
                            if (dialog2 != null) {
                                dialog2.addTransaction(transaction2);
                                dialog2.addRoute(sipRequest);
                                transaction2.setDialog(dialog2, sipRequest.getDialogId(true));
                            }
                            return transaction2;
                        }
                        ...
                        if (dialog3 != null) {
                            dialog3.addTransaction(transaction3);
                            dialog3.addRoute(sipRequest);
                            transaction3.setDialog(dialog3, sipRequest.getDialogId(true));
                        }
                        return transaction3;
                    }
                    if (((SIPServerTransaction) this.sipStack.findTransaction((SIPRequest) request, true)) != null) {
                    ...
                    try {
                        ...
                        if (dialog4 != null) {
                            dialog4.addTransaction(transaction);
                            dialog4.addRoute(sipRequest);
                            transaction.setDialog(dialog4, sipRequest.getDialogId(true));
                        }
    ...
    public void sendRequest(Request request) throws SipException {
        ...
        try {
            try {
                ...
                MessageChannel messageChannel = this.listeningPoints.containsKey(hop.getTransport().toUpperCase()) ? this.sipStack.createRawMessageChannel(getListeningPoint(hop.getTransport()).getIPAddress(), getListeningPoint(hop.getTransport()).getPort(), hop) : null;
    ...
    public void transactionErrorEvent(SIPTransactionErrorEvent transactionErrorEvent) {
        ...
        if (transactionErrorEvent.getErrorID() == 2) {
            ...
            ?? source = transactionErrorEvent.getSource();
            Timeout timeout = Timeout.TRANSACTION;
            if (source instanceof SIPServerTransaction) {
                ev3 = new TimeoutEvent(this, (ServerTransaction) source, timeout);
            } else {
                SIPClientTransaction clientTx = (SIPClientTransaction) source;
                ...
                ev3 = new TimeoutEvent(this, (ClientTransaction) source, timeout);
            }
            handleEvent(ev3, (SIPTransaction) source);
            return;
        }
        if (transactionErrorEvent.getErrorID() == 1) {
            ?? source2 = transactionErrorEvent.getSource();
            Timeout timeout2 = Timeout.TRANSACTION;
            if (source2 instanceof SIPServerTransaction) {
                ev2 = new TimeoutEvent(this, (ServerTransaction) source2, timeout2);
            } else {
                SIPClientTransaction clientTx2 = (SIPClientTransaction) source2;
                Hop hop2 = clientTx2.getNextHop();
                if (this.sipStack.getRouter() instanceof RouterExt) {
                    ((RouterExt) this.sipStack.getRouter()).transactionTimeout(hop2);
                }
                ev2 = new TimeoutEvent(this, (ClientTransaction) source2, timeout2);
            }
            handleEvent(ev2, (SIPTransaction) source2);
            return;
        }
        if (transactionErrorEvent.getErrorID() != 3) {
            return;
        }
        ?? source3 = transactionErrorEvent.getSource();
        Transaction tx = (Transaction) source3;
        ...
        if (source3 instanceof SIPServerTransaction) {
            ev = new TimeoutEvent(this, (ServerTransaction) source3, timeout3);
        } else {
            ev = new TimeoutEvent(this, (ClientTransaction) source3, timeout3);
        }
        handleEvent(ev, (SIPTransaction) source3);
    }
