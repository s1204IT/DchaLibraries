...
public class StringMsgParser {
    ...
    public SIPMessage parseSIPMessage(byte[] msgBuffer) throws InvalidArgumentException, ParseException, UnsupportedEncodingException {
        String currentLine;
        ...
        do {
            ...
            try {
                ...
                if (currentLine.length() == 0) {
                    if (currentHeader != null && sIPMessageProcessFirstLine != null) {
                        processHeader(currentHeader, sIPMessageProcessFirstLine);
                    }
                } else if (isFirstLine) {
                    sIPMessageProcessFirstLine = processFirstLine(currentLine);
                } else {
        ...
        if (this.readBody && sIPMessageProcessFirstLine.getContentLength() != null && sIPMessageProcessFirstLine.getContentLength().getContentLength() != 0) {
            ...
            sIPMessageProcessFirstLine.setMessageContent(body, computeContentLengthFromMessage, sIPMessageProcessFirstLine.getContentLength().getContentLength());
        }
    ...
    public SIPMessage parseSIPMessage(String msgString) throws ParseException {
        ...
        if (this.readBody && sIPMessageProcessFirstLine.getContentLength() != null) {
            if (sIPMessageProcessFirstLine.getContentLength().getContentLength() != 0) {
                ...
                sIPMessageProcessFirstLine.setMessageContent(body, this.strict, computeContentLengthFromMessage, sIPMessageProcessFirstLine.getContentLength().getContentLength());
            } else if (!computeContentLengthFromMessage && sIPMessageProcessFirstLine.getContentLength().getContentLength() == 0 && !msgString.endsWith("\r\n\r\n") && this.strict) {
        ...
        if (!str.startsWith(SIPConstants.SIP_VERSION_STRING)) {
            ...
            try {
                ...
                if (this.parseExceptionListener != null) {
                    this.parseExceptionListener.handleException(e, sIPRequest, RequestLine.class, str, this.rawStringMessage);
                    sIPMessage = sIPRequest;
                } else {
                    throw e;
                }
            ...
            try {
                sIPResponse.setStatusLine(new StatusLineParser(str + Separators.RETURN).parse());
    ...
    private void processHeader(String header, SIPMessage message) throws ParseException {
        ...
        try {
            ...
            try {
                ...
                Class<ExtensionHeaderImpl> classFromName = NameMap.getClassFromName(headerName);
                if (classFromName == null) {
                    classFromName = ExtensionHeaderImpl.class;
                }
                this.parseExceptionListener.handleException(ex, message, classFromName, header, this.rawStringMessage);
            }
