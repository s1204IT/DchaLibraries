    ...
    static final class Reader implements FrameReader {
        ...
        public boolean nextFrame(FrameReader.Handler handler) throws IOException {
            try {
                ...
                boolean control = (Integer.MIN_VALUE & w1) != 0;
        ...
        private void readSynStream(FrameReader.Handler handler, int flags, int length) throws IOException {
            ...
            List<Header> nameValueBlock = this.headerBlockReader.readNameValueBlock(length - 10);
            boolean inFinished = (flags & 1) != 0;
            boolean outFinished = (flags & 2) != 0;
            handler.headers(outFinished, inFinished, streamId, associatedStreamId, nameValueBlock, HeadersMode.SPDY_SYN_STREAM);
        }

        private void readSynReply(FrameReader.Handler handler, int flags, int length) throws IOException {
            int w1 = this.source.readInt();
            int streamId = w1 & Integer.MAX_VALUE;
            List<Header> nameValueBlock = this.headerBlockReader.readNameValueBlock(length - 4);
            boolean inFinished = (flags & 1) != 0;
            handler.headers(false, inFinished, streamId, -1, nameValueBlock, HeadersMode.SPDY_REPLY);
        }
    ...
    static final class Writer implements FrameWriter {
        ...
        public synchronized void synStream(boolean outFinished, boolean inFinished, int streamId, int associatedStreamId, List<Header> list) throws IOException {
            ...
            synchronized (this) {
                ...
                int i2 = outFinished ? 1 : Spdy3.TYPE_DATA;
                if (inFinished) {
                    i = 2;
                }
                int flags = i2 | i;
        ...
        private void writeNameValueBlockToBuffer(List<Header> list) throws IOException {
            ...
            for (int i = Spdy3.TYPE_DATA; i < size; i++) {
                ...
                ByteString value = list.get(i).value;
