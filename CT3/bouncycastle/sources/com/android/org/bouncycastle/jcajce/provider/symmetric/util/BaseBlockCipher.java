...
public class BaseBlockCipher extends BaseWrapCipher implements PBE {
    ...
    private boolean isBCPBEKeyWithoutIV(Key key) {
        return (key instanceof BCPBEKey) && !(key.getParam() instanceof ParametersWithIV);
    }
    ...
    protected void engineInit(int i, Key key, AlgorithmParameterSpec algorithmParameterSpec, SecureRandom secureRandom) throws InvalidKeyException, InvalidAlgorithmParameterException {
        ...
        if (algorithmParameterSpec == 0 && this.baseEngine.getAlgorithmName().startsWith("RC5-64")) {
        ...
        if ((this.scheme == 2 || (key instanceof PKCS12Key)) && !isBCPBEKeyWithoutIV(key)) {
            try {
                ...
                if ((secretKey instanceof PBEKey) && this.pbeSpec == null) {
                    if (((PBEKey) secretKey).getSalt() == null) {
                        throw new InvalidAlgorithmParameterException("Parameters for the algorithm are null and the PBEKey has null salt");
                    }
                    this.pbeSpec = new PBEParameterSpec(((PBEKey) secretKey).getSalt(), ((PBEKey) secretKey).getIterationCount());
                }
                ...
                if (!(key instanceof BCPBEKey)) {
                    ...
                    if (key.getParam() == null || !(key.getParam() instanceof ParametersWithIV)) {
                        throw new IllegalStateException("Unreachable code");
                    }
                    cipherParametersMakePBEParameters = key.getParam();
                }
                boolean z = cipherParametersMakePBEParameters instanceof ParametersWithIV;
                aEADParameters = cipherParametersMakePBEParameters;
                if (z) {
                    this.ivParam = cipherParametersMakePBEParameters;
                    aEADParameters = cipherParametersMakePBEParameters;
                }
            } catch (Exception e) {
            ...
            if (key.getParam() != null) {
                cipherParametersMakePBEParameters2 = adjustParameters(algorithmParameterSpec, key.getParam());
            } else if (algorithmParameterSpec instanceof PBEParameterSpec) {
                this.pbeSpec = algorithmParameterSpec;
                cipherParametersMakePBEParameters2 = PBE.Util.makePBEParameters(key, algorithmParameterSpec, this.cipher.getUnderlyingCipher().getAlgorithmName());
            } else {
                throw new InvalidAlgorithmParameterException("PBE requires PBE parameters to be set.");
            }
            boolean z2 = cipherParametersMakePBEParameters2 instanceof ParametersWithIV;
            aEADParameters = cipherParametersMakePBEParameters2;
            if (z2) {
                this.ivParam = cipherParametersMakePBEParameters2;
                aEADParameters = cipherParametersMakePBEParameters2;
            }
            ...
            ?? MakePBEParameters = PBE.Util.makePBEParameters(pBEKey.getEncoded(), this.scheme, this.digest, this.keySizeInBits, this.ivLength * 8, this.pbeSpec, this.cipher.getAlgorithmName());
        ...
        if (algorithmParameterSpec instanceof IvParameterSpec) {
            if (this.ivLength != 0) {
                if (algorithmParameterSpec.getIV().length != this.ivLength && !(this.cipher instanceof AEADGenericBlockCipher) && this.fixedIv) {
                    throw new InvalidAlgorithmParameterException("IV must be " + this.ivLength + " bytes long.");
                }
                if (aEADParameters instanceof ParametersWithIV) {
                    aEADParameters = new ParametersWithIV(aEADParameters.getParameters(), algorithmParameterSpec.getIV());
                } else {
                    aEADParameters = new ParametersWithIV(aEADParameters, algorithmParameterSpec.getIV());
                }
                this.ivParam = aEADParameters;
            ...
            try {
                aEADParameters = new AEADParameters(aEADParameters instanceof ParametersWithIV ? (KeyParameter) aEADParameters.getParameters() : (KeyParameter) aEADParameters, ((Integer) gcmSpecClass.getDeclaredMethod("getTLen", new Class[0]).invoke(algorithmParameterSpec, new Object[0])).intValue(), (byte[]) gcmSpecClass.getDeclaredMethod("getIV", new Class[0]).invoke(algorithmParameterSpec, new Object[0]));
                this.aeadParams = aEADParameters;
            } catch (Exception e2) {
                throw new InvalidAlgorithmParameterException("Cannot process GCMParameterSpec.");
            }
        } else if (algorithmParameterSpec != 0 && !(algorithmParameterSpec instanceof PBEParameterSpec)) {
        ...
        if (this.ivLength == 0 || (aEADParameters instanceof ParametersWithIV) || (aEADParameters instanceof AEADParameters)) {
            ...
            if (i == 1 || i == 3) {
                ...
                ParametersWithIV parametersWithIV = new ParametersWithIV(aEADParameters, bArr);
                this.ivParam = parametersWithIV;
                r20 = parametersWithIV;
                ...
                ParametersWithIV parametersWithIV2 = new ParametersWithIV(aEADParameters, new byte[this.ivLength]);
                this.ivParam = parametersWithIV2;
                r20 = parametersWithIV2;
            }
        }
        ?? parametersWithRandom = (secureRandom == null || !this.padded) ? r20 : new ParametersWithRandom(r20, secureRandom);
        try {
            switch (i) {
                case 1:
                case 3:
    ...
    private CipherParameters adjustParameters(AlgorithmParameterSpec algorithmParameterSpec, CipherParameters cipherParameters) {
        if (cipherParameters instanceof ParametersWithIV) {
            CipherParameters key = cipherParameters.getParameters();
            if (algorithmParameterSpec instanceof IvParameterSpec) {
                this.ivParam = new ParametersWithIV(key, algorithmParameterSpec.getIV());
                CipherParameters param = this.ivParam;
                return param;
            }
            return cipherParameters;
        }
        if (algorithmParameterSpec instanceof IvParameterSpec) {
            this.ivParam = new ParametersWithIV(cipherParameters, algorithmParameterSpec.getIV());
            CipherParameters param2 = this.ivParam;
            return param2;
        }
        return cipherParameters;
    }
    ...
    private boolean isAEADModeName(String modeName) {
        if ("CCM".equals(modeName)) {
            return true;
        }
        return "GCM".equals(modeName);
    }
