        ...
        protected SecretKey engineGenerateSecret(KeySpec keySpec) throws InvalidKeySpecException {
            if (keySpec instanceof PBEKeySpec) {
                ...
                if (keySpec.getIterationCount() <= 0) {
                    throw new InvalidKeySpecException("positive iteration count required: " + keySpec.getIterationCount());
                }
                if (keySpec.getKeyLength() <= 0) {
                    throw new InvalidKeySpecException("positive key length required: " + keySpec.getKeyLength());
                }
                if (keySpec.getPassword().length == 0) {
                    throw new IllegalArgumentException("password empty");
                }
                int keySize = keySpec.getKeyLength();
                CipherParameters param = PBE.Util.makePBEMacParameters(keySpec, this.scheme, 1, keySize);
                return new BCPBEKey(this.algName, this.algOid, this.scheme, 1, keySize, -1, keySpec, param);
            }
