        ...
        protected SecretKey engineGenerateSecret(KeySpec keySpec) throws InvalidKeySpecException {
            ...
            if (keySpec instanceof PBEKeySpec) {
                if (keySpec.getSalt() == null) {
                    return new BCPBEKey(this.algName, this.algOid, this.scheme, this.digest, this.keySize, this.ivSize, keySpec, null);
                }
                if (this.forCipher) {
                    param = PBE.Util.makePBEParameters(keySpec, this.scheme, this.digest, this.keySize, this.ivSize);
                } else {
                    param = PBE.Util.makePBEMacParameters(keySpec, this.scheme, this.digest, this.keySize);
                }
                KeyParameter kParam = param instanceof ParametersWithIV ? (KeyParameter) param.getParameters() : (KeyParameter) param;
                DESParameters.setOddParity(kParam.getKey());
                return new BCPBEKey(this.algName, this.algOid, this.scheme, this.digest, this.keySize, this.ivSize, keySpec, param);
            }
            throw new InvalidKeySpecException("Invalid KeySpec");
        }
    }
