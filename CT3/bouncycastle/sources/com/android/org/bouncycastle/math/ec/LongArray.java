...
class LongArray implements Cloneable {
    ...
    public LongArray modMultiplyLD(LongArray other, int m, int[] ks) {
        ...
        for (int k = 56; k >= 0; k -= 8) {
            ...
            while (true) {
                ...
                if (j2 < aLen) {
                    ...
                    int u = aVal & 15;
                    int v = (aVal >>> 4) & 15;
                    addBoth(c, j2 - 1, T0, ti[u], T1, ti[v], bMax);
                    j = j2 + 2;
                }
            }
            shiftUp(c, 0, cLen, 8);
        ...
        for (int k2 = 56; k2 >= 0; k2 -= 8) {
            for (int j3 = 0; j3 < aLen; j3 += 2) {
                ...
                int u2 = aVal2 & 15;
                int v2 = (aVal2 >>> 4) & 15;
                addBoth(c, j3, T0, ti[u2], T1, ti[v2], bMax);
            }
            if (k2 > 0) {
                shiftUp(c, 0, cLen, 8);
            }
    ...
    private static long interleave2_n(long x, int rounds) {
        ...
        return rounds > 0 ? interleave2_32to64((int) x) | (interleave2_32to64((int) (x >>> 32)) << 1) : x;
    }
    ...
    public boolean equals(Object obj) {
        int usedLen;
        if (!(obj instanceof LongArray) || obj.getUsedLength() != (usedLen = getUsedLength())) {
            return false;
        }
        for (int i = 0; i < usedLen; i++) {
            if (this.m_ints[i] != obj.m_ints[i]) {
                return false;
            }
        }
        return true;
