...
public class AttributeTable {
    ...
    public Attribute get(ASN1ObjectIdentifier oid) {
        ?? r0 = this.attributes.get(oid);
        return r0 instanceof Vector ? (Attribute) r0.elementAt(0) : (Attribute) r0;
    }

    public ASN1EncodableVector getAll(ASN1ObjectIdentifier oid) {
        ASN1EncodableVector v = new ASN1EncodableVector();
        ?? r2 = this.attributes.get(oid);
        if (r2 instanceof Vector) {
            Enumeration e = r2.elements();
    ...
    public int size() {
        ...
        while (en.hasMoreElements()) {
            ...
            size = NextElement instanceof Vector ? size + NextElement.size() : size + 1;
        }
        return size;
    }
    ...
    public ASN1EncodableVector toASN1EncodableVector() {
        ...
        while (e.hasMoreElements()) {
            ...
            if (NextElement instanceof Vector) {
                Enumeration en = NextElement.elements();
                while (en.hasMoreElements()) {
                    v.add(Attribute.getInstance(en.nextElement()));
                }
            } else {
                v.add(Attribute.getInstance(NextElement));
            }
        }
        return v;
    }
