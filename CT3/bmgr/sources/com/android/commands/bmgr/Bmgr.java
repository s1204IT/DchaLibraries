...
public final class Bmgr {
    ...
    private void backupNowPackages(List<String> packages) {
        try {
            ...
            int err = this.mBmgr.requestBackup((String[]) packages.toArray(new String[packages.size()]), observer);
            if (err == 0) {
    ...
    private void doRestorePackage(String pkg) {
        try {
            this.mRestore = this.mBmgr.beginRestoreSession(pkg, (String) null);
            if (this.mRestore == null) {
                System.err.println(BMGR_NOT_RUNNING_ERR);
                return;
            }
