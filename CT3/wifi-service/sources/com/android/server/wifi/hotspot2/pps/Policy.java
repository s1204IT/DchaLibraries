...
public class Policy {
    ...
    public Policy(OMANode node) throws OMAException {
        ...
        if (sxNode == null) {
            ...
            for (OMANode instance3 : sxNode.getChildren()) {
                if (instance3.isLeaf()) {
                    throw new OMAException("Not expecting leaf node in SPExclusionList");
                }
                this.mSPExclusionList.add(PasspointManagementObjectManager.getString(instance3, PasspointManagementObjectManager.TAG_SSID));
            }
        }
        ...
        if (rptNode == null) {
            ...
            for (OMANode instance4 : rptNode.getChildren()) {
                if (instance4.isLeaf()) {
                    throw new OMAException("Not expecting leaf node in RequiredProtoPortTuple");
                }
                int protocol = (int) PasspointManagementObjectManager.getLong(instance4, PasspointManagementObjectManager.TAG_IPProtocol, null);
                String[] portSegments = PasspointManagementObjectManager.getString(instance4, PasspointManagementObjectManager.TAG_PortNumber).split(",");
        ...
        this.mMaxBSSLoad = (int) PasspointManagementObjectManager.getLong(node, PasspointManagementObjectManager.TAG_MaximumBSSLoadValue, Long.MAX_VALUE);
    }
    ...
    private static class PreferredRoamingPartner {
        ...
        PreferredRoamingPartner(OMANode node, PreferredRoamingPartner preferredRoamingPartner) {
            this(node);
        }
    ...
    private static class MinBackhaul {
        ...
        MinBackhaul(OMANode node, MinBackhaul minBackhaul) {
            this(node);
        }
        ...
        private MinBackhaul(OMANode node) throws OMAException {
            ...
            this.mDL = PasspointManagementObjectManager.getLong(node, PasspointManagementObjectManager.TAG_DLBandwidth, Long.MAX_VALUE);
            this.mUL = PasspointManagementObjectManager.getLong(node, PasspointManagementObjectManager.TAG_ULBandwidth, Long.MAX_VALUE);
        }
