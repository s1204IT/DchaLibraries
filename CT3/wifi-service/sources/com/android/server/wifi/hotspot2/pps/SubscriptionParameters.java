...
public class SubscriptionParameters {
    ...
    public SubscriptionParameters(OMANode node) throws OMAException {
        ...
        for (OMANode instance : ulNode.getChildren()) {
            ...
            this.mLimits.add(new Limit(instance, limit));
        }
    }
    ...
    private static class Limit {
        ...
        Limit(OMANode node, Limit limit) {
            this(node);
        }

        private Limit(OMANode node) throws OMAException {
            this.mDataLimit = PasspointManagementObjectManager.getLong(node, PasspointManagementObjectManager.TAG_DataLimit, Long.MAX_VALUE);
            this.mStartDate = PasspointManagementObjectManager.getTime(node.getChild(PasspointManagementObjectManager.TAG_StartDate));
            this.mTimeLimit = PasspointManagementObjectManager.getLong(node, PasspointManagementObjectManager.TAG_TimeLimit, Long.MAX_VALUE) * PasspointManagementObjectManager.IntervalFactor;
