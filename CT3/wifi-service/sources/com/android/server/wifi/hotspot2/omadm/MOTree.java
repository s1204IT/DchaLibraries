...
public class MOTree {
    ...
    public MOTree(XMLNode node, String urn) throws SAXException, IOException {
        ...
        Iterator child$iterator = node.getChildren().iterator();
        while (child$iterator.hasNext()) {
            buildNode(this.mRoot, (XMLNode) child$iterator.next());
        }
    }
    ...
    private static class NodeData {
        ...
        NodeData(String name, NodeData nodeData) {
            this(name);
        }
    ...
    private static void buildNode(OMANode parent, XMLNode node) throws IOException {
        ...
        if (values.isEmpty()) {
            ...
            while (child$iterator.hasNext()) {
                buildNode(subNode, (XMLNode) child$iterator.next());
            }
            return;
        }
        if (!children.isEmpty()) {
        ...
        while (true) {
            ...
            if (octet > 32) {
                ...
                if (!tree.toString().equals("tree")) {
                    throw new IOException("Not a tree: " + ((Object) tree));
                }
