...
public class VenueNameElement extends ANQPElement {
    ...
    static {
        ...
        int index = 0;
        while (i < length) {
            VenueType venue = venueTypeArr[i];
            sGroupBases.put(VenueGroup.valuesCustom()[index], Integer.valueOf(venue.ordinal()));
            i++;
            index++;
        }
    }

    public VenueNameElement(Constants.ANQPElementType infoID, ByteBuffer payload) throws ProtocolException {
        super(infoID);
        ...
        if (group >= VenueGroup.Reserved.ordinal()) {
            ...
            this.mGroup = VenueGroup.valuesCustom()[group];
            int type2 = type + sGroupBases.get(this.mGroup).intValue();
            if (type2 >= VenueType.Reserved.ordinal()) {
                this.mType = VenueType.Reserved;
            } else {
                this.mType = VenueType.valuesCustom()[type2];
            }
        }
