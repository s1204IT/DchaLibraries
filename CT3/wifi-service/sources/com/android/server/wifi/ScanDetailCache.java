...
public class ScanDetailCache {
    ...
    public WifiConfiguration.Visibility getVisibilityByPasspointMatch(long age) {
        ...
        for (PasspointMatchInfo pmi : this.mPasspointMatches.values()) {
            ...
            if (scanDetail != null && (result = scanDetail.getScanResult()) != null && scanDetail.getSeen() != 0 && now_ms - result.seen <= age) {
                if (result.is5GHz()) {
        ...
        if (pmiBest5 != null) {
            ScanResult result2 = pmiBest5.getScanDetail().getScanResult();
            status.rssi5 = result2.level;
            status.age5 = result2.seen;
            status.BSSID5 = result2.BSSID;
            logMsg = "Visiblity by passpoint match returned 5 GHz BSSID of " + result2.BSSID;
        }
        if (pmiBest24 != null) {
            ScanResult result3 = pmiBest24.getScanDetail().getScanResult();
            status.rssi24 = result3.level;
            status.age24 = result3.seen;
            status.BSSID24 = result3.BSSID;
            logMsg = logMsg + "2.4 GHz BSSID of " + result3.BSSID;
        }
        Log.d(TAG, logMsg);
        return status;
    }
