...
public class WifiQualifiedNetworkSelector {
    ...
    private static class BssidBlacklistStatus {
        ...
        BssidBlacklistStatus(BssidBlacklistStatus bssidBlacklistStatus) {
            this();
        }

        private BssidBlacklistStatus() {
            this.mBlacklistedTimeStamp = -1L;
        }
    }
    ...
    WifiQualifiedNetworkSelector(WifiConfigManager configureStore, Context context, WifiInfo wifiInfo, Clock clock) {
        this.mRssiScoreSlope = 4;
        this.mRssiScoreOffset = 85;
        this.mSameBssidAward = 24;
        this.mLastSelectionAward = LAST_SELECTION_AWARD;
        this.mPasspointSecurityAward = 40;
        this.mSecurityAward = 80;
        ...
        this.mRssiScoreSlope = context.getResources().getInteger(R.integer.config_allowedUnprivilegedKeepalivePerUid);
        this.mRssiScoreOffset = context.getResources().getInteger(R.integer.config_alertDialogController);
        this.mSameBssidAward = context.getResources().getInteger(R.integer.config_am_tieredCachedAdjUiTierSize);
        this.mLastSelectionAward = context.getResources().getInteger(R.integer.config_app_exit_info_history_list_size);
        this.mPasspointSecurityAward = context.getResources().getInteger(R.integer.config_attentionMaximumExtension);
        this.mSecurityAward = context.getResources().getInteger(R.integer.config_attentiveTimeout);
    ...
    private boolean isNetworkQualified(WifiConfiguration currentNetwork) {
        ...
        if (this.mWifiConfigManager.isOpenNetwork(currentNetwork)) {
            localLog("Current network is open network");
            return false;
        }
        if (this.mWifiInfo.is24GHz() && this.mUserPreferedBand != 2) {
            localLog("Current band dose not match user preference. Start Qualified Network Selection Current band = " + (this.mWifiInfo.is24GHz() ? "2.4GHz band" : "5GHz band") + "UserPreference band = " + this.mUserPreferedBand);
            return false;
        }
    ...
    private boolean needQualifiedNetworkSelection(boolean isLinkDebouncing, boolean isConnected, boolean isDisconnected, boolean isSupplicantTransientState) {
        ...
        if (isConnected) {
            ...
            WifiConfiguration currentNetwork = this.mWifiConfigManager.getWifiConfiguration(this.mWifiInfo.getNetworkId());
            if (currentNetwork == null || isNetworkQualified(this.mCurrentConnectedNetwork)) {
                return false;
            }
    ...
    int calculateBssidScore(ScanResult scanResult, WifiConfiguration network, WifiConfiguration currentNetwork, boolean sameBssid, boolean sameSelect, StringBuffer sbuf) {
        int rssi = scanResult.level <= this.mWifiConfigManager.mThresholdSaturatedRssi24.get() ? scanResult.level : this.mWifiConfigManager.mThresholdSaturatedRssi24.get();
    ...
    private void updateSavedNetworkSelectionStatus() {
        ...
        for (WifiConfiguration network : savedNetworks) {
            ...
            status.setCandidate((ScanResult) null);
    ...
    public boolean userSelectNetwork(int netId, boolean persist) {
        ...
        if (this.mWifiFwkExt != null && this.mWifiFwkExt.hasNetworkSelection() == 3) {
            if (this.mLastUserSelectSecuredNetworkForOP03 == null || compareSecurity(selected, this.mLastUserSelectSecuredNetworkForOP03) <= 0) {
    ...
    public boolean enableBssidForQualityNetworkSelection(String bssid, boolean enable) {
        BssidBlacklistStatus bssidBlacklistStatus = null;
        if (enable) {
        ...
        if (bssid != null) {
            ...
            if (status == null) {
                BssidBlacklistStatus newStatus = new BssidBlacklistStatus(bssidBlacklistStatus);
                newStatus.mCounter++;
    ...
    public boolean isBssidDisabled(String bssid) {
        ...
        return status.mIsBlacklisted;
    }

    public WifiConfiguration selectQualifiedNetwork(boolean forceSelectNetwork, boolean isUntrustedConnectionsAllowed, List<ScanDetail> scanDetails, boolean isLinkDebouncing, boolean isConnected, boolean isDisconnected, boolean isSupplicantTransient) throws Throwable {
        Integer netScore;
        localLog("==========start qualified Network Selection==========");
        this.mScanDetails = scanDetails;
        if (this.mDbg) {
            StringBuffer dumpScanResult = new StringBuffer();
            Iterator scanDetail$iterator = this.mScanDetails.iterator();
            while (scanDetail$iterator.hasNext()) {
                dumpScanResult.append(toScanId(((ScanDetail) scanDetail$iterator.next()).getScanResult()) + " / ");
            }
            localLog(((Object) dumpScanResult) + " dump for scanDetails at begin of selectQualifiedNetwork()\n");
        }
        ...
        String lastUserSelectedNetWorkKey = this.mWifiConfigManager.getLastSelectedConfiguration();
        WifiConfiguration lastUserSelectedNetwork = this.mWifiConfigManager.getWifiConfiguration(lastUserSelectedNetWorkKey);
        ...
        for (ScanDetail scanDetail : this.mScanDetails) {
            ...
            if (scanResult.SSID != null && !TextUtils.isEmpty(scanResult.SSID)) {
                ...
                if (this.mWifiConfigManager.isBssidBlacklisted(scanResult.BSSID) || isBssidDisabled(scanResult.BSSID)) {
                    ...
                    List<WifiConfiguration> associatedWifiConfigurations = this.mWifiConfigManager.updateSavedNetworkWithNewScanDetail(scanDetail, (isSupplicantTransient || isConnected) ? true : isLinkDebouncing);
                    ...
                    if (!potentiallyEphemeral) {
                        ...
                        for (WifiConfiguration network2 : associatedWifiConfigurations) {
                            WifiConfiguration.NetworkSelectionStatus status = network2.getNetworkSelectionStatus();
                            ...
                            if (!status.isNetworkEnabled()) {
                                ...
                                if (this.mWifiFwkExt != null && this.mWifiFwkExt.hasNetworkSelection() != 0 && this.mWifiFwkExt.hasNetworkSelection() != 3) {
                                    int order = this.mWifiFwkExt.hasNetworkSelection() == 1 ? 0 : 0;
                                ...
                                if (network2.useExternalScores) {
                                    ...
                                    int score = calculateBssidScore(scanResult, network2, this.mCurrentConnectedNetwork, this.mCurrentBssid == null ? false : this.mCurrentBssid.equals(scanResult.BSSID), lastUserSelectedNetwork != null && lastUserSelectedNetwork.networkId == network2.networkId, scoreHistory);
                                    if (this.mWifiFwkExt != null && this.mWifiFwkExt.hasNetworkSelection() == 3 && networkCandidate != null) {
                                        if (this.mLastUserSelectSecuredNetworkForOP03 == null || !this.mLastUserSelectSecuredNetworkForOP03.SSID.equals(networkCandidate.SSID)) {
                                            int order2 = compareSecurity(network2, networkCandidate);
                                            localLog("OP_03:compareSecurity(" + network2.SSID + "," + networkCandidate.SSID + "):" + order2);
                                            if (order2 < 0) {
                                ...
                                localLog("Network: " + getNetworkString(network2) + " has specifiedBSSID:" + network2.BSSID + ". Skip " + scanResult.BSSID);
                            }
                        }
                    ...
                    } else if (isUntrustedConnectionsAllowed && (netScore = getNetworkScore(scanResult, false)) != null && !this.mWifiConfigManager.wasEphemeralNetworkDeleted(scanResult.SSID)) {
        ...
        if (this.mScoreManager != null && unscoredNetworks.size() != 0) {
            NetworkKey[] unscoredNetworkKeys = (NetworkKey[]) unscoredNetworks.toArray(new NetworkKey[unscoredNetworks.size()]);
            this.mScoreManager.requestScores(unscoredNetworkKeys);
        }
        if (this.mDbg) {
            localLog(((Object) lowSignalScan) + " skipped due to low signal\n");
            localLog(((Object) notSavedScan) + " skipped due to not saved\n ");
            localLog(((Object) noValidSsid) + " skipped due to not valid SSID\n");
        ...
        String currentAssociationId = this.mCurrentConnectedNetwork == null ? "Disconnected" : getNetworkString(this.mCurrentConnectedNetwork);
    ...
    WifiConfiguration getExternalScoreCandidate(ExternalScoreEvaluator scoreEvaluator) {
        switch (scoreEvaluator.getBestCandidateType()) {
            ...
            case 1:
                ...
                localLog(String.format("new scored candidate %s network ID:%d", toScanId(scanResultCandidate), Integer.valueOf(networkCandidate.networkId)));
                return networkCandidate;
            case 2:
                ...
                localLog(String.format("new ephemeral candidate %s network ID:%d, meteredHint=%b", toScanId(untrustedScanResultCandidate), Integer.valueOf(unTrustedNetworkCandidate.networkId), Boolean.valueOf(unTrustedNetworkCandidate.meteredHint)));
