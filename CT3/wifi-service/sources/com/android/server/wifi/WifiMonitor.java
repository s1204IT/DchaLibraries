...
public class WifiMonitor {
    ...
    private static final Pattern p2pErrorExPattern1 = Pattern.compile("status=([0-9]{1,2})");
    private static final Pattern p2pErrorExPattern2 = Pattern.compile("status=([0-9]{1,2}) peer_oper_freq=(-?[0-9]{1,4})");
    private static final Pattern p2pErrorExPattern3 = Pattern.compile("reason=FREQ_CONFLICT peer_oper_freq=(-?[0-9]{1,4})");
    ...
    private Map<String, Long> mLastConnectBSSIDs = new HashMap<String, Long>() {
        public Long get(String iface) {
            Long value = (Long) super.get((Object) iface);
            if (value != null) {
                return value;
            }
            return 0L;
    ...
    private class MonitorThread extends Thread {
        ...
        public MonitorThread() {
            ...
            WifiNative unused = WifiMonitor.this.mWifiNative;
    ...
    private boolean dispatchEvent(String eventStr, String iface) {
        ...
        if (!eventStr.startsWith(EVENT_PREFIX_STR)) {
            ...
            } else if (eventStr.startsWith(HS20_SUB_REM_STR) || eventStr.startsWith(HS20_DEAUTH_STR)) {
                handleWnmFrame(String.format("%012x %s", this.mLastConnectBSSIDs.get(iface), eventStr), iface);
            ...
            } else if (eventStr.startsWith("Authentication with") && eventStr.endsWith(AUTH_TIMEOUT_STR)) {
        ...
        if (eventName.equals(CONNECTED_STR)) {
            ...
            if (prefix >= 0 && (suffix = eventStr.indexOf(ConnectSuffix)) > prefix) {
                try {
            ...
            event = eventName.equals(STATE_CHANGE_STR) ? 3 : eventName.equals(SCAN_RESULTS_STR) ? 4 : eventName.equals(SCAN_FAILED_STR) ? 15 : eventName.equals(LINK_SPEED_STR) ? 5 : eventName.equals(TERMINATING_STR) ? 6 : eventName.equals(DRIVER_STATE_STR) ? 7 : eventName.equals(EAP_FAILURE_STR) ? 8 : eventName.equals(ASSOC_REJECT_STR) ? 9 : eventName.equals(TEMP_DISABLED_STR) ? 10 : eventName.equals(REENABLED_STR) ? 11 : eventName.equals(BSS_ADDED_STR) ? 12 : eventName.equals(BSS_REMOVED_STR) ? 13 : eventName.equals(WAPI_NO_CERTIFICATION_STRING) ? 16 : 14;
        }
    ...
    private WifiP2pServiceImpl.P2pStatus p2pError(String dataString) {
        ...
        try {
            return WifiP2pServiceImpl.P2pStatus.valueOf(Integer.parseInt(nameValue[1]));
    ...
    private WifiP2pServiceImpl.P2pStatus p2pError_GroupRemove(String dataString) {
        WifiP2pServiceImpl.P2pStatus err = WifiP2pServiceImpl.P2pStatus.UNKNOWN;
        String[] tokens = dataString.split(" ");
        if (tokens.length < 3) {
            return err;
        ...
        if (err == WifiP2pServiceImpl.P2pStatus.NO_COMMON_CHANNEL && tokens.length > 4) {
            String[] freqNameValue = tokens[4].split("=");
            if (!tokens[4].startsWith("peer_oper_freq") || freqNameValue.length != 2 || freqNameValue[1] == null) {
    ...
    private WifiP2pServiceImpl.P2pStatus p2pErrorEx(String dataString) {
        WifiP2pServiceImpl.P2pStatus status = WifiP2pServiceImpl.P2pStatus.UNKNOWN;
        Matcher match = p2pErrorExPattern2.matcher(dataString);
        if (match.find()) {
            status = WifiP2pServiceImpl.P2pStatus.valueOf(Integer.valueOf(match.group(1)).intValue());
            sP2pOperFreq = Integer.valueOf(match.group(2)).intValue();
        } else {
            Matcher match2 = p2pErrorExPattern1.matcher(dataString);
            if (match2.find()) {
                status = WifiP2pServiceImpl.P2pStatus.valueOf(Integer.valueOf(match2.group(1)).intValue());
                sP2pOperFreq = -1;
            }
        }
        Matcher match3 = p2pErrorExPattern3.matcher(dataString);
        if (match3.find()) {
            status = WifiP2pServiceImpl.P2pStatus.NO_COMMON_CHANNEL;
            sP2pOperFreq = Integer.valueOf(match3.group(1)).intValue();
        }
    ...
    private void handleP2pEvents(String dataString, String iface) {
        ...
        if (dataString.startsWith(P2P_DEVICE_LOST_STR)) {
            WifiP2pDevice device2 = getWifiP2pDevice(dataString);
            if (device2 != null) {
                sendMessage(iface, P2P_DEVICE_LOST_EVENT, device2);
                return;
            }
            return;
        }
        ...
        if (dataString.startsWith(P2P_GO_NEG_FAILURE_STR)) {
            WifiP2pServiceImpl.P2pStatus p2pStatus = p2pErrorEx(dataString);
        ...
        if (dataString.startsWith(P2P_GROUP_FORMATION_FAILURE_STR)) {
            WifiP2pServiceImpl.P2pStatus p2pStatus2 = p2pErrorEx(dataString);
            sendMessage(iface, P2P_GROUP_FORMATION_FAILURE_EVENT, sP2pOperFreq, 0, p2pStatus2);
            sP2pOperFreq = -1;
            return;
        }
        ...
        if (dataString.startsWith(P2P_GROUP_REMOVED_STR)) {
            WifiP2pServiceImpl.P2pStatus p2pStatus3 = p2pError_GroupRemove(dataString);
            sendMessage(iface, P2P_GROUP_REMOVED_EVENT, sP2pOperFreq, 0, p2pStatus3);
            sP2pOperFreq = -1;
            return;
        }
        ...
        if (dataString.startsWith(P2P_INVITATION_RESULT_STR)) {
            WifiP2pServiceImpl.P2pStatus p2pStatus4 = p2pErrorEx(dataString);
            sendMessage(iface, P2P_INVITATION_RESULT_EVENT, sP2pOperFreq, 0, p2pStatus4);
            sP2pOperFreq = -1;
            return;
        }
        ...
        if (tokens.length > 3 && tokens[3].startsWith("peer_oper_freq")) {
            String[] freqNameValue2 = tokens[3].split("=");
            if (freqNameValue2[1] != null) {
                sP2pOperFreq = Integer.valueOf(freqNameValue2[1]).intValue();
    ...
    private void handleAnqpResult(String eventStr, String iface) {
        ...
        try {
            long bssid = Utils.parseMac(eventStr.substring(ADDR_STRING.length() + addrPos, eoaddr));
            int result = eventStr.substring(RESULT_STRING.length() + resPos, eoresult).equalsIgnoreCase("success") ? 1 : 0;
            sendMessage(iface, ANQP_DONE_EVENT, result, 0, Long.valueOf(bssid));
        } catch (IllegalArgumentException iae) {
    ...
    private void handleSupplicantStateChange(String dataString, String iface) {
        ...
        while (true) {
            ...
            SupplicantState state = supplicantStateArrValues[i];
    ...
    private void handleNetworkStateChange(NetworkInfo.DetailedState newState, String data, String iface) {
        ...
        if (!match2.find()) {
            ...
            try {
                local = Integer.parseInt(match2.group(4));
