...
public final class AppCompatDrawableManager {
    ...
    private static void installDefaultInflateDelegates(@NonNull AppCompatDrawableManager appCompatDrawableManager) {
        VdcInflateDelegate vdcInflateDelegate = null;
        Object[] objArr = 0;
        int i = Build.VERSION.SDK_INT;
        if (i >= 23) {
            return;
        }
        appCompatDrawableManager.addDelegate("vector", new VdcInflateDelegate(vdcInflateDelegate));
        if (i < 11) {
            return;
        }
        appCompatDrawableManager.addDelegate("animated-vector", new AvdcInflateDelegate(objArr == true ? 1 : 0));
    }
    ...
    private Drawable tintDrawable(@NonNull Context context, @DrawableRes int resId, boolean failIfNotKnown, @NonNull Drawable drawable) {
        ...
        if (resId == R$drawable.abc_seekbar_track_material) {
            ...
            setPorterDuffColorFilter(ld.findDrawableByLayerId(R.id.background), ThemeUtils.getThemeAttrColor(context, R$attr.colorControlNormal), DEFAULT_MODE);
            setPorterDuffColorFilter(ld.findDrawableByLayerId(R.id.secondaryProgress), ThemeUtils.getThemeAttrColor(context, R$attr.colorControlNormal), DEFAULT_MODE);
            setPorterDuffColorFilter(ld.findDrawableByLayerId(R.id.progress), ThemeUtils.getThemeAttrColor(context, R$attr.colorControlActivated), DEFAULT_MODE);
            return drawable;
        }
        if (resId == R$drawable.abc_ratingbar_material || resId == R$drawable.abc_ratingbar_indicator_material || resId == R$drawable.abc_ratingbar_small_material) {
            ...
            setPorterDuffColorFilter(ld2.findDrawableByLayerId(R.id.background), ThemeUtils.getDisabledThemeAttrColor(context, R$attr.colorControlNormal), DEFAULT_MODE);
            setPorterDuffColorFilter(ld2.findDrawableByLayerId(R.id.secondaryProgress), ThemeUtils.getThemeAttrColor(context, R$attr.colorControlActivated), DEFAULT_MODE);
            setPorterDuffColorFilter(ld2.findDrawableByLayerId(R.id.progress), ThemeUtils.getThemeAttrColor(context, R$attr.colorControlActivated), DEFAULT_MODE);
            return drawable;
        }
    ...
    private static class VdcInflateDelegate implements InflateDelegate {
        VdcInflateDelegate(VdcInflateDelegate vdcInflateDelegate) {
            this();
        }
    ...
    private static class AvdcInflateDelegate implements InflateDelegate {
        AvdcInflateDelegate(AvdcInflateDelegate avdcInflateDelegate) {
            this();
        }
