...
public class StorageMeasurement {
    ...
    private class MainHandler extends Handler {
        MainHandler(StorageMeasurement this$0, MainHandler mainHandler) {
            this();
        }
    ...
    public void measureExactStorage(IMediaContainerService imcs) {
        ...
        if (this.mVolume.getType() == 1) {
            ...
            for (UserInfo user2 : users) {
                Iterator app$iterator = volumeApps.iterator();
                while (app$iterator.hasNext()) {
                    packageManager.getPackageSizeInfoAsUser(((ApplicationInfo) app$iterator.next()).packageName, user2.id, observer);
                }
            }
            return;
        }
