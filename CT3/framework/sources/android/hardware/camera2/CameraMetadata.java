...
public abstract class CameraMetadata<TKey> {
    ...
    static <TKey> ArrayList<TKey> getKeysStatic(Class<?> cls, Class<TKey> cls2, CameraMetadata<TKey> cameraMetadata, int[] iArr) {
        ...
        for (Field field : cls.getDeclaredFields()) {
            if (field.getType().isAssignableFrom(cls2) && (field.getModifiers() & 1) != 0) {
                try {
                    ...
                    if ((cameraMetadata == 0 || cameraMetadata.getProtected(obj) != null) && shouldKeyBeAdded(obj, field, iArr)) {
                        arrayList.add(obj);
                    }
        ...
        if (allVendorKeys != null) {
            for (Object obj2 : allVendorKeys) {
                if (obj2 instanceof CaptureRequest.Key) {
                    name = ((CaptureRequest.Key) obj2).getName();
                } else if (obj2 instanceof CaptureResult.Key) {
                    name = ((CaptureResult.Key) obj2).getName();
                } else if (obj2 instanceof CameraCharacteristics.Key) {
                    name = ((CameraCharacteristics.Key) obj2).getName();
                }
                if (iArr == null || Arrays.binarySearch(iArr, CameraMetadataNative.getTag(name)) >= 0) {
                    arrayList.add(obj2);
                }
            }
        }
