...
public class ContextHubService extends IContextHubService.Stub {
    ...
    public ContextHubService(Context context) {
        ...
        if (!context.getPackageManager().hasSystemFeature(PackageManager.FEATURE_VR_MODE) || (vrManager = IVrManager.Stub.asInterface(ServiceManager.getService("vrmanager"))) == null) {
            return;
        }
        try {
    ...
    public int[] getContextHubHandles() throws RemoteException {
        ...
        for (int i = 0; i < returnArray.length; i++) {
            returnArray[i] = i;
            Log.d(TAG, String.format("Hub %s is mapped to %d", this.mContextHubInfo[i].getName(), Integer.valueOf(returnArray[i])));
        }
        return returnArray;
    }
    ...
    public int loadNanoApp(int contextHubHandle, NanoApp app) throws RemoteException {
        ...
        int[] msgHeader = {3, 0, contextHubHandle, -1};
        return nativeSendMessage(msgHeader, app.getAppBinary()) != 0 ? -1 : 0;
    }

    @Override
    public int unloadNanoApp(int nanoAppInstanceHandle) throws RemoteException {
        checkPermissions();
        NanoAppInstanceInfo info = this.mNanoAppHash.get(Integer.valueOf(nanoAppInstanceHandle));
        if (info == null) {
            return -1;
        }
        int[] msgHeader = {4, 0, -1, -1};
        return nativeSendMessage(msgHeader, null) != 0 ? -1 : 0;
    }
    ...
    public NanoAppInstanceInfo getNanoAppInstanceInfo(int nanoAppInstanceHandle) throws RemoteException {
        ...
        if (this.mNanoAppHash.containsKey(Integer.valueOf(nanoAppInstanceHandle))) {
            return this.mNanoAppHash.get(Integer.valueOf(nanoAppInstanceHandle));
        }
        return null;
    }
    ...
    public int sendMessage(int hubHandle, int nanoAppHandle, ContextHubMessage msg) throws RemoteException {
        checkPermissions();
        int[] msgHeader = {msg.getMsgType(), msg.getVersion(), hubHandle, nanoAppHandle};
    ...
    private void sendVrStateChangeMessageToApp(NanoAppInstanceInfo app, boolean vrModeEnabled) {
        int[] msgHeader = {0, 0, -1, app.getHandle()};
