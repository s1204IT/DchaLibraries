...
class GLLogWrapper extends GLWrapperBase {
    ...
    private String toString(int n, ShortBuffer buf) {
        ...
        builder.append("{\n");
        for (int i = 0; i < n; i++) {
            builder.append(" [").append(i).append("] = ").append((int) buf.get(i)).append('\n');
        }
        builder.append("}");
        return builder.toString();
    }
    ...
    private String getTexturePName(int pname) {
        switch (pname) {
            ...
            case GLES11.GL_GENERATE_MIPMAP:
                return "GL_GENERATE_MIPMAP";
            case GLES11Ext.GL_TEXTURE_CROP_RECT_OES:
        ...
        switch (i2) {
            case 5121:
                ...
                for (int i3 = 0; i3 < i; i3++) {
                    cArr[i3] = (char) (bArrArray[iArrayOffset + i3] & BatteryStats.HistoryItem.CMD_NULL);
                }
                return cArr;
            case 5122:
            default:
                return cArr;
            case 5123:
                CharBuffer charBufferAsCharBuffer = buffer instanceof CharBuffer ? buffer : toByteBuffer(i * 2, buffer).asCharBuffer();
        ...
        for (int i = 0; i < size; i++) {
            ...
            switch (type) {
                ...
                case 5121:
                    ...
                    builder.append(Integer.toString(d2 & BatteryStats.HistoryItem.CMD_NULL));
                    break;
                case 5122:
    ...
    public void glCompressedTexSubImage2D(int target, int level, int xoffset, int yoffset, int width, int height, int format, int imageSize, Buffer data) {
        ...
        arg("width", width);
        arg("height", height);
        arg(MediaStore.Files.FileColumns.FORMAT, format);
        arg("imageSize", imageSize);
        arg("data", data.toString());
        end();
    ...
    private class PointerInfo {
        ...
        public void bindByteBuffer() {
            this.mTempByteBuffer = this.mPointer != null ? GLLogWrapper.this.toByteBuffer(-1, this.mPointer) : null;
        }
