...
public final class Condition implements Parcelable {
    ...
    public Condition(Uri id, String summary, int state) {
        this(id, summary, ProxyInfo.LOCAL_EXCL_LIST, ProxyInfo.LOCAL_EXCL_LIST, -1, state, 2);
    }
    ...
    public static String relevanceToString(int flags) {
        ...
        if (now || always) {
            return (now && always) ? "NOW, ALWAYS" : now ? "NOW" : "ALWAYS";
        }
    ...
    public static boolean isValidId(Uri id, String pkg) {
        if (id == null || !SCHEME.equals(id.getScheme())) {
            return false;
        }
        return pkg.equals(id.getAuthority());
    }
}
