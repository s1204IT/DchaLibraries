...
public class Paint {
    ...
    private void syncTextLocalesWithMinikin() {
        ...
        synchronized (sCacheLock) {
            Integer minikinLangListId = sMinikinLangListIdCache.get(languageTags);
            if (minikinLangListId == null) {
                int newID = nSetTextLocales(this.mNativePaint, languageTags);
                sMinikinLangListIdCache.put(languageTags, Integer.valueOf(newID));
            } else {
    ...
    public float measureText(char[] text, int index, int count) {
        ...
        if (!this.mHasCompatScaling) {
            return (float) Math.ceil(nGetTextAdvances(this.mNativePaint, this.mNativeTypeface, text, index, count, index, count, this.mBidiFlags, (float[]) null, 0));
        }
        float oldSize = getTextSize();
        setTextSize(this.mCompatScaling * oldSize);
        float w = nGetTextAdvances(this.mNativePaint, this.mNativeTypeface, text, index, count, index, count, this.mBidiFlags, (float[]) null, 0);
        setTextSize(oldSize);
    ...
    public float measureText(String text, int start, int end) {
        ...
        if (!this.mHasCompatScaling) {
            return (float) Math.ceil(nGetTextAdvances(this.mNativePaint, this.mNativeTypeface, text, start, end, start, end, this.mBidiFlags, (float[]) null, 0));
        }
        float oldSize = getTextSize();
        setTextSize(this.mCompatScaling * oldSize);
        float w = nGetTextAdvances(this.mNativePaint, this.mNativeTypeface, text, start, end, start, end, this.mBidiFlags, (float[]) null, 0);
        setTextSize(oldSize);
        return (float) Math.ceil(this.mInvCompatScaling * w);
    }
