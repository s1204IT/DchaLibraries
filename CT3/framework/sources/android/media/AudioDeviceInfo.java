...
public final class AudioDeviceInfo {
    ...
    public int[] getChannelCounts() {
        ...
        for (int mask : getChannelMasks()) {
            if (isSink()) {
                iChannelCountFromInChannelMask = AudioFormat.channelCountFromOutChannelMask(mask);
            } else {
                iChannelCountFromInChannelMask = AudioFormat.channelCountFromInChannelMask(mask);
            }
            countSet.add(Integer.valueOf(iChannelCountFromInChannelMask));
        }
        ...
        int index = 0;
        Iterator count$iterator = countSet.iterator();
        while (count$iterator.hasNext()) {
            int count = ((Integer) count$iterator.next()).intValue();
            counts[index] = count;
            index++;
        }
        return counts;
