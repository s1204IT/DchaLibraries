...
final class TtmlUtils {
    ...
    public static long parseTimeExpression(String time, int frameRate, int subframeRate, int tickRate) throws NumberFormatException {
        ...
        if (matcher.matches()) {
            ...
            double durationSeconds4 = durationSeconds3 + (frames != null ? Long.parseLong(frames) / ((double) frameRate) : 0.0d);
            String subframes = matcher.group(6);
            if (subframes != null) {
                d = (Long.parseLong(subframes) / ((double) subframeRate)) / ((double) frameRate);
            } else {
                d = 0.0d;
            }
            return (long) (1000.0d * (durationSeconds4 + d));
        }
    ...
    private static void extractText(TtmlNode node, long startUs, long endUs, StringBuilder out, boolean inPTag) {
        ...
        for (int i = 0; i < node.mChildren.size(); i++) {
            extractText(node.mChildren.get(i), startUs, endUs, out, !pTag ? inPTag : true);
        }
