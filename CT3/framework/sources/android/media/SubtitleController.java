...
public class SubtitleController {
    ...
    static {
        f15assertionsDisabled = !SubtitleController.class.desiredAssertionStatus();
    }
    ...
    public SubtitleTrack getDefaultTrack() {
        ...
        synchronized (this.mTracks) {
            for (SubtitleTrack track : this.mTracks) {
                ...
                boolean zEquals = (locale == null || locale.getLanguage().equals(ProxyInfo.LOCAL_EXCL_LIST) || locale.getISO3Language().equals(language)) ? true : locale.getLanguage().equals(language);
                int score = (forced ? 0 : 8) + ((selectedLocale == null && is_default) ? 4 : 0) + (autoselect ? 0 : 2) + (zEquals ? 1 : 0);
                if (!selectForced || forced) {
                    if ((selectedLocale == null && is_default) || (zEquals && (autoselect || forced || selectedLocale != null))) {
                        if (score > bestScore) {
                            bestScore = score;
                            bestTrack = track;
                        }
