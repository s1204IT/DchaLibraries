...
public class Equalizer extends AudioEffect {
    ...
    public Equalizer(int priority, int audioSession) throws RuntimeException {
    ...
    public void setBandLevel(short band, short level) throws IllegalStateException, UnsupportedOperationException, IllegalArgumentException {
        int[] param = {2, band};
        short[] value = {level};
        checkStatus(setParameter(param, value));
    }

    public short getBandLevel(short band) throws IllegalStateException, UnsupportedOperationException, IllegalArgumentException {
        short[] result = new short[1];
        int[] param = {2, band};
        checkStatus(getParameter(param, result));
        return result[0];
    ...
    public int getCenterFreq(short band) throws IllegalStateException, UnsupportedOperationException, IllegalArgumentException {
    ...
    public int[] getBandFreqRange(short band) throws IllegalStateException, UnsupportedOperationException, IllegalArgumentException {
    ...
    public short getBand(int frequency) throws IllegalStateException, UnsupportedOperationException, IllegalArgumentException {
        short[] result = new short[1];
    ...
    public void usePreset(short preset) throws IllegalStateException, UnsupportedOperationException, IllegalArgumentException {
    ...
    private class BaseParameterListener implements AudioEffect.OnParameterChangeListener {
        BaseParameterListener(Equalizer this$0, BaseParameterListener baseParameterListener) {
            this();
        }
        ...
        public void onParameterChange(AudioEffect effect, int status, byte[] param, byte[] value) {
            ...
            int v = -1;
            if (param.length >= 4) {
                p1 = Equalizer.byteArrayToInt(param, 0);
                if (param.length >= 8) {
                    p2 = Equalizer.byteArrayToInt(param, 4);
                }
            }
            if (value.length == 2) {
                v = Equalizer.byteArrayToShort(value, 0);
            } else if (value.length == 4) {
                v = Equalizer.byteArrayToInt(value, 0);
            }
            if (p1 == -1 || v == -1) {
                return;
            }
