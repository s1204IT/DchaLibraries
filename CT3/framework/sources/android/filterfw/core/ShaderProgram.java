...
public class ShaderProgram extends Program {
    ...
    @Override
    public void process(Frame[] inputs, Frame frame) {
        ...
        if (this.mMaxTileSize > 0) {
            int xTiles = ((frame.getFormat().getWidth() + this.mMaxTileSize) - 1) / this.mMaxTileSize;
            int yTiles = ((frame.getFormat().getHeight() + this.mMaxTileSize) - 1) / this.mMaxTileSize;
            setShaderTileCounts(xTiles, yTiles);
        }
        if (!shaderProcess(glInputs, frame)) {
    ...
    public void setHostValue(String variableName, Object value) {
        if (this.mLogVerbose) {
            Log.v(TAG, "setHostValue(" + variableName + ", " + explainObject(value) + ")");
        }
    ...
    private void wait3DReady() {
        if (!this.mLogVerbose || SystemProperties.getInt("debug.effect.wait", 0) != 1) {
            return;
        }
        Frame.wait3DReady();
    }
