...
final class SystemServiceRegistry {
    ...
    static {
        ...
        registerService(Context.SEARCH_ENGINE_SERVICE, SearchEngineManager.class, new CachedServiceFetcher<SearchEngineManager>() {
            @Override
        ...
        registerService(Context.MTK_PERF_SERVICE, IPerfServiceWrapper.class, new CachedServiceFetcher<IPerfServiceWrapper>() {
            @Override
            public IPerfServiceWrapper createService(ContextImpl ctx) {
                try {
                    return new PerfServiceWrapper(ctx);
                } catch (Exception e) {
                    e.printStackTrace();
                    return null;
                }
        ...
        if (WifiEnterpriseConfig.ENGINE_ENABLE.equals(SystemProperties.get("ro.mtk_sensorhub_support"))) {
            registerService("sensorhubservice", ISensorHubManager.class, new CachedServiceFetcher<ISensorHubManager>() {
                @Override
                public ISensorHubManager createService(ContextImpl ctx) {
                    try {
                        IBinder b = ServiceManager.getService("sensorhubservice");
                        ISensorHubService service = ISensorHubService.Stub.asInterface(b);
                        return new SensorHubManager(service);
                    } catch (Exception e) {
                        e.printStackTrace();
                        return null;
                    }
        ...
        if (!"no".equals(SystemProperties.get("ro.mtk_carrierexpress_pack", "no"))) {
            registerService(Context.USP_SERVICE, UspManager.class, new CachedServiceFetcher<UspManager>() {
                @Override
        ...
        if (!WifiEnterpriseConfig.ENGINE_ENABLE.equals(SystemProperties.get("persist.runningbooster.support"))) {
            return;
        }
        registerService(Context.RUNNING_BOOSTER_SERVICE, RunningBoosterManager.class, new CachedServiceFetcher<RunningBoosterManager>() {
            @Override
    ...
    static abstract class CachedServiceFetcher<T> implements ServiceFetcher<T> {
        ...
        public final T getService(ContextImpl contextImpl) {
            ...
            synchronized (objArr) {
                tCreateService = (T) objArr[this.mCacheIndex];
                if (tCreateService == null) {
