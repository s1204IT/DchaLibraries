...
public abstract class RegisteredServicesCache<V> {
    ...
    private UserServices<V> findOrCreateUserLocked(int userId, boolean loadFromFileIfNew) {
        ...
        if (services == null) {
            services = new UserServices<>(userServices);
    ...
    private final void handlePackageEvent(Intent intent, int userId) {
        ...
        if (Intent.ACTION_PACKAGE_REMOVED.equals(action)) {
            ...
            zEquals = Intent.ACTION_EXTERNAL_APPLICATIONS_UNAVAILABLE.equals(action);
        }
        boolean replacing = intent.getBooleanExtra(Intent.EXTRA_REPLACING, false);
        if (zEquals && replacing) {
            return;
        }
        int[] uids = null;
    ...
    public void dump(FileDescriptor fd, PrintWriter fout, String[] args, int userId) {
        synchronized (this.mServicesLock) {
            ...
            if (user.services != null) {
                ...
                Iterator info$iterator = user.services.values().iterator();
                while (info$iterator.hasNext()) {
                    ServiceInfo<?> info = (ServiceInfo) info$iterator.next();
                    fout.println("  " + info);
                }
            } else {
    ...
    public Collection<ServiceInfo<V>> getAllServices(int userId) {
        ...
        synchronized (this.mServicesLock) {
            ...
            collectionUnmodifiableCollection = Collections.unmodifiableCollection(new ArrayList(user.services.values()));
        }
    ...
    private void generateServicesMap(int[] changedUids, int userId) {
        ...
        synchronized (this.mServicesLock) {
            ...
            for (Object obj : user.persistentServices.keySet()) {
                if (!containsType(serviceInfos, obj) && containsUid(changedUids, user.persistentServices.get(obj).intValue())) {
                    arrayListNewArrayList.add(obj);
                }
            }
            for (Object obj2 : arrayListNewArrayList) {
                changed = true;
                user.persistentServices.remove(obj2);
                user.services.remove(obj2);
                notifyListener(obj2, userId, true);
            }
            if (changed) {
                onServicesChangedLocked(userId);
    ...
    private boolean containsUid(int[] changedUids, int uid) {
        if (changedUids != null) {
            return ArrayUtils.contains(changedUids, uid);
        }
        return true;
    }
    ...
    private void writePersistentServicesLocked(UserServices<V> userServices, int i) {
        ...
        try {
            ...
            Iterator<T> it = userServices.persistentServices.entrySet().iterator();
            while (it.hasNext()) {
                Map.Entry entry = (Map.Entry) it.next();
                fastXmlSerializer.startTag(null, "service");
                fastXmlSerializer.attribute(null, GrantCredentialsPermissionActivity.EXTRAS_REQUESTING_UID, Integer.toString(((Integer) entry.getValue()).intValue()));
