...
public class NetworkStats implements Parcelable {
    ...
    public static class Entry {
        ...
        public boolean equals(Object o) {
            ...
            Entry e = (Entry) o;
            if (this.uid == e.uid && this.set == e.set && this.tag == e.tag && this.roaming == e.roaming && this.rxBytes == e.rxBytes && this.rxPackets == e.rxPackets && this.txBytes == e.txBytes && this.txPackets == e.txPackets && this.operations == e.operations) {
                return this.iface.equals(e.iface);
            }
            return false;
        }
    }
    ...
    public NetworkStats m1664clone() {
    ...
    private Entry getTotal(Entry recycle, HashSet<String> limitIface, int limitUid, boolean includeTags) {
        ...
        for (int i = 0; i < this.size; i++) {
            ...
            boolean zContains = limitIface != null ? limitIface.contains(this.iface[i]) : true;
            if (matchesUid && zContains && (this.tag[i] == 0 || includeTags)) {
                entry.rxBytes += this.rxBytes[i];
                entry.rxPackets += this.rxPackets[i];
                entry.txBytes += this.txBytes[i];
                entry.txPackets += this.txPackets[i];
    ...
    public static <C> NetworkStats subtract(NetworkStats left, NetworkStats right, NonMonotonicObserver<C> observer, C cookie, NetworkStats recycle) {
        ...
        for (int i = 0; i < left.size; i++) {
            ...
            int j = right.findIndexHinted(entry.iface, entry.uid, entry.set, entry.tag, entry.roaming, i);
            if (j == -1) {
