...
public class WifiManager {
    ...
    private ArrayList<WifiOffListener> mWifiOffListenerList = new ArrayList<>();
    ...
    public boolean enableNetwork(int netId, boolean disableOthers) {
        ...
        if (pin) {
            NetworkRequest request = new NetworkRequest.Builder().clearCapabilities().addTransportType(1).build();
    ...
    public class WifiLock {
        ...
        WifiLock(WifiManager this$0, int lockType, String tag, WifiLock wifiLock) {
            this(lockType, tag);
        }
    ...
    public class MulticastLock {
        ...
        MulticastLock(WifiManager this$0, String tag, MulticastLock multicastLock) {
            this(tag);
        }
                ...
                for (int i = 2; i < lines.length; i++) {
                    ...
                    if (nameValue.length == 2) {
                        try {
                            ...
                            result.put(Integer.valueOf(ch), Integer.valueOf(num));
    ...
    public void addWifiOffListener(WifiOffListener listener) {
        if (this.mWifiOffListenerList.size() == 0) {
            this.mWifiOffNotifyReceiver = new BroadcastReceiver() {
                @Override
                public void onReceive(Context context, Intent intent) {
                    if (WifiManager.this.mWifiOffListenerList.size() <= 0) {
                        return;
                    }
                    String action = intent.getAction();
                    Log.d(WifiManager.TAG, "onReceive, action: " + action);
                    int reason = intent.getIntExtra(WifiManager.EXTRA_WIFI_OFF_REASON, 0);
                    Log.d(WifiManager.TAG, "wifi_off_reason: " + reason);
                    for (WifiOffListener listener2 : WifiManager.this.mWifiOffListenerList) {
                        listener2.onWifiOff(reason);
                    }
                }
            };
