...
public class ConnectivityManager {
    ...
    public static boolean isNetworkTypeValid(int networkType) {
        if (networkType < 0 || networkType > 17) {
            return networkType >= 34 && networkType <= 49;
        }
        return true;
    }
    ...
    public int startUsingNetworkFeature(int networkType, String feature) {
        ...
        synchronized (sLegacyRequests) {
            ...
            if (l != null) {
                ...
                return l.currentNetwork != null ? 0 : 1;
            }
    ...
    private NetworkCapabilities networkCapabilitiesForFeature(int networkType, String feature) {
        ...
        netCap2.maybeMarkCapabilitiesRestricted();
        return netCap2;
    }
    ...
    private NetworkRequest requestNetworkForFeatureLocked(NetworkCapabilities netCap) {
        LegacyRequest legacyRequest = null;
        int type = legacyTypeForNetworkCapabilities(netCap);
        try {
            int delay = this.mService.getRestoreDefaultNetworkDelay(type);
            LegacyRequest l = new LegacyRequest(legacyRequest);
    ...
    public class PacketKeepalive {
        ...
        PacketKeepalive(ConnectivityManager this$0, Network network, PacketKeepaliveCallback callback, PacketKeepalive packetKeepalive) {
        ...
        public void handleMessage(Message message) {
            ...
            switch (message.what) {
                case ConnectivityManager.CALLBACK_PRECHECK:
                    NetworkCallback callback2 = getCallback(request, "PRECHECK");
                    if (callback2 == null) {
                        return;
                    }
                    callback2.onPreCheck(network);
                    return;
                case ConnectivityManager.CALLBACK_AVAILABLE:
                    NetworkCallback callback3 = getCallback(request, "AVAILABLE");
                    if (callback3 == null) {
                        return;
                    }
                    callback3.onAvailable(network);
                    return;
                case ConnectivityManager.CALLBACK_LOSING:
                    NetworkCallback callback4 = getCallback(request, "LOSING");
                    if (callback4 == null) {
                        return;
                    }
                    callback4.onLosing(network, message.arg1);
                    return;
                case ConnectivityManager.CALLBACK_LOST:
                    NetworkCallback callback5 = getCallback(request, "LOST");
                    if (callback5 == null) {
                        return;
                    }
                    callback5.onLost(network);
                    return;
                case ConnectivityManager.CALLBACK_UNAVAIL:
                    NetworkCallback callback6 = getCallback(request, "UNAVAIL");
                    if (callback6 == null) {
                        return;
                    }
                    callback6.onUnavailable();
                    return;
                case ConnectivityManager.CALLBACK_CAP_CHANGED:
                    NetworkCallback callback7 = getCallback(request, "CAP_CHANGED");
                    if (callback7 == null) {
                        return;
                    }
                    NetworkCapabilities cap = (NetworkCapabilities) getObject(message, NetworkCapabilities.class);
                    callback7.onCapabilitiesChanged(network, cap);
                    return;
                case ConnectivityManager.CALLBACK_IP_CHANGED:
                    NetworkCallback callback8 = getCallback(request, "IP_CHANGED");
                    if (callback8 == null) {
                        return;
                    }
                    LinkProperties lp = (LinkProperties) getObject(message, LinkProperties.class);
                    callback8.onLinkPropertiesChanged(network, lp);
                    return;
                case ConnectivityManager.CALLBACK_RELEASED:
                    synchronized (this.mCallbackMap) {
                        NetworkCallback callback9 = this.mCallbackMap.remove(request);
                        callback = callback9;
                    }
                    if (callback != null) {
                ...
                case ConnectivityManager.CALLBACK_SUSPENDED:
                    NetworkCallback callback10 = getCallback(request, "SUSPENDED");
                    if (callback10 == null) {
                        return;
                    }
                    callback10.onNetworkSuspended(network);
                    return;
                case ConnectivityManager.CALLBACK_RESUMED:
                    NetworkCallback callback11 = getCallback(request, "RESUMED");
                    if (callback11 == null) {
                        return;
                    }
                    callback11.onNetworkResumed(network);
                    return;
                default:
                    return;
            }
