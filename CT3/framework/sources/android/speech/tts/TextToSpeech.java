...
public class TextToSpeech {
    ...
    public void shutdown() {
        synchronized (this.mStartLock) {
            if (this.mConnectingServiceConnection != null) {
                ...
                }, null, PowerManager.WAKE_UP_REASON_SHUTDOWN, false);
            }
        }
    }
    ...
    public int speak(String text, int queueMode, HashMap<String, String> params) {
        return speak(text, queueMode, convertParamsHashMaptoBundle(params), params != null ? params.get(Engine.KEY_PARAM_UTTERANCE_ID) : null);
    }
    ...
    public int playEarcon(String earcon, int queueMode, HashMap<String, String> params) {
        return playEarcon(earcon, queueMode, convertParamsHashMaptoBundle(params), params != null ? params.get(Engine.KEY_PARAM_UTTERANCE_ID) : null);
    }
    ...
    public int playSilence(long durationInMs, int queueMode, HashMap<String, String> params) {
        return playSilentUtterance(durationInMs, queueMode, params != null ? params.get(Engine.KEY_PARAM_UTTERANCE_ID) : null);
    }
                    ...
                    try {
                        ...
                        if (result >= 0) {
                            ...
                            if (TextUtils.isEmpty(voiceName)) {
                                Log.w(TextToSpeech.TAG, "Couldn't find the default voice for " + language + ContactsContract.Aas.ENCODE_SYMBOL + country + ContactsContract.Aas.ENCODE_SYMBOL + variant);
                                return -2;
                            }
                            if (service.loadVoice(TextToSpeech.this.getCallerIdentity(), voiceName) == -1) {
                                Log.w(TextToSpeech.TAG, "The service claimed " + language + ContactsContract.Aas.ENCODE_SYMBOL + country + ContactsContract.Aas.ENCODE_SYMBOL + variant + " was available with voice name " + voiceName + " but loadVoice returned ERROR");
                                return -2;
                            }
    ...
    private static boolean verifyIntegerBundleParam(Bundle bundle, String key) {
        if (bundle.containsKey(key)) {
            if (!(!(bundle.get(key) instanceof Integer) ? bundle.get(key) instanceof Long : true)) {
                bundle.remove(key);
    ...
    private static boolean verifyBooleanBundleParam(Bundle bundle, String key) {
        if (bundle.containsKey(key)) {
            if (!(!(bundle.get(key) instanceof Boolean) ? bundle.get(key) instanceof String : true)) {
                bundle.remove(key);
    ...
    private static boolean verifyFloatBundleParam(Bundle bundle, String key) {
        if (bundle.containsKey(key)) {
            if (!(!(bundle.get(key) instanceof Float) ? bundle.get(key) instanceof Double : true)) {
                bundle.remove(key);
    ...
    private class Connection implements ServiceConnection {
        ...
        Connection(TextToSpeech this$0, Connection connection) {
            this();
        }
