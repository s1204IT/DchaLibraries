...
public abstract class BatteryStats implements Parcelable {
    ...
    private static final int[] f23comandroidinternalosBatterySipper$DrainTypeSwitchesValues = null;
    ...
    private static int[] m1757xe0235176() {
        if (f23comandroidinternalosBatterySipper$DrainTypeSwitchesValues != null) {
            return f23comandroidinternalosBatterySipper$DrainTypeSwitchesValues;
        }
        ...
        f23comandroidinternalosBatterySipper$DrainTypeSwitchesValues = iArr;
        return iArr;
    }
    ...
    public static abstract class Uid {
        ...
        static final String[] USER_ACTIVITY_TYPES = {CardEmulation.CATEGORY_OTHER, "button", "touch", Context.ACCESSIBILITY_SERVICE};
        ...
        public void decodeEntryAt(int index, String value) {
            ...
            while (i3 < N) {
                ...
                if (c != '-') {
                    ...
                    i2 = i + 1;
                    long out22 = out | ((level << 40) & BatteryStats.STEP_LEVEL_LEVEL_MASK);
                    long duration2 = 0;
                    while (i2 < N) {
                    }
                    this.mStepDurations[index] = (BatteryStats.STEP_LEVEL_TIME_MASK & duration2) | out22;
                }
            }
            i = i3 + 1;
            long level2 = 0;
            ...
            i2 = i + 1;
            long out222 = out | ((level2 << 40) & BatteryStats.STEP_LEVEL_LEVEL_MASK);
            long duration22 = 0;
            while (i2 < N) {
            }
            this.mStepDurations[index] = (BatteryStats.STEP_LEVEL_TIME_MASK & duration22) | out222;
        }
    ...
    public static final class HistoryItem implements Parcelable {
        ...
        public void writeToParcel(Parcel dest, int flags) {
            ...
            int bat = (this.wakeReasonTag != null ? 536870912 : 0) | ((this.batteryPlugType << 24) & 251658240) | (this.cmd & CMD_NULL) | ((this.batteryLevel << 8) & 65280) | ((this.batteryStatus << WifiScanner.PnoSettings.PnoNetwork.FLAG_SAME_NETWORK) & 983040) | ((this.batteryHealth << 20) & 15728640) | (this.wakelockTag != null ? 268435456 : 0) | (this.eventCode != 0 ? 1073741824 : 0);
            dest.writeInt(bat);
            int bat2 = (this.batteryTemperature & 65535) | ((this.batteryVoltage << 16) & Color.RED);
            dest.writeInt(bat2);
    ...
    private final void printControllerActivity(PrintWriter pw, StringBuilder sb, String prefix, String controllerName, ControllerActivityCounter counter, int which) {
        ...
        sb.append("  ");
        sb.append(controllerName);
        sb.append(" Power drain: ").append(BatteryStatsHelper.makemAh(powerDrainMaMs / 3600000.0d));
        sb.append("mAh");
        pw.println(sb.toString());
    }
    ...
    public final void dumpCheckinLocked(Context context, PrintWriter pw, int which, int reqUid, boolean wifiOnly) {
        ...
        for (int iu = 0; iu < NU; iu++) {
            ArrayMap<String, ? extends Uid.Wakelock> wakelocks = uidStats.valueAt(iu).getWakelockStats();
        ...
        dumpLine(pw, 0, category, GLOBAL_WIFI_DATA, Long.valueOf(wifiOnTime / 1000), Long.valueOf(wifiRunningTime / 1000), 0, 0, 0);
        ...
        if (reqUid < 0) {
            ...
            if (wakeupReasons.size() > 0) {
                for (Map.Entry<String, ? extends Timer> ent2 : wakeupReasons.entrySet()) {
                    ...
                    dumpLine(pw, 0, category, WAKEUP_REASON_DATA, "\"" + ent2.getKey() + "\"", Long.valueOf((500 + totalTimeMicros) / 1000), Integer.valueOf(((Timer) ent2.getValue()).getCountLocked(which)));
                }
            }
        }
        ...
        if (sippers != null && sippers.size() > 0) {
            ...
            for (int i12 = 0; i12 < sippers.size(); i12++) {
                BatterySipper bs = sippers.get(i12);
                int uid = 0;
                switch (m1757xe0235176()[bs.drainType.ordinal()]) {
                    case 1:
        ...
        for (int iu2 = 0; iu2 < NU; iu2++) {
            ...
            if (reqUid < 0 || uid2 == reqUid) {
                ...
                if (mobileBytesRx > 0 || mobileBytesTx > 0 || wifiBytesRx > 0 || wifiBytesTx > 0 || mobilePacketsRx > 0 || mobilePacketsTx > 0 || wifiPacketsRx > 0 || wifiPacketsTx > 0 || mobileActiveTime > 0 || mobileActiveCount > 0 || btBytesRx > 0 || btBytesTx > 0) {
                    dumpLine(pw, uid2, category, NETWORK_DATA, Long.valueOf(mobileBytesRx), Long.valueOf(mobileBytesTx), Long.valueOf(wifiBytesRx), Long.valueOf(wifiBytesTx), Long.valueOf(mobilePacketsRx), Long.valueOf(mobilePacketsTx), Long.valueOf(wifiPacketsRx), Long.valueOf(wifiPacketsTx), Long.valueOf(mobileActiveTime), Integer.valueOf(mobileActiveCount), Long.valueOf(btBytesRx), Long.valueOf(btBytesTx));
                }
                dumpControllerActivityLine(pw, uid2, category, MODEM_CONTROLLER_DATA, u.getModemControllerActivity(), which);
                ...
                if (fullWifiLockOnTime != 0 || wifiScanTime != 0 || wifiScanCount != 0 || uidWifiRunningTime != 0) {
                    dumpLine(pw, uid2, category, WIFI_DATA, Long.valueOf(fullWifiLockOnTime), Long.valueOf(wifiScanTime), Long.valueOf(uidWifiRunningTime), Integer.valueOf(wifiScanCount), 0, 0, 0);
                }
                dumpControllerActivityLine(pw, uid2, category, WIFI_CONTROLLER_DATA, u.getWifiControllerActivity(), which);
                dumpTimer(pw, uid2, category, BLUETOOTH_MISC_DATA, u.getBluetoothScanTimer(), rawRealtime, which);
                dumpControllerActivityLine(pw, uid2, category, BLUETOOTH_CONTROLLER_DATA, u.getBluetoothControllerActivity(), which);
                if (u.hasUserActivity()) {
                    Object[] args7 = new Object[4];
                    boolean hasData = false;
                    for (int i13 = 0; i13 < 4; i13++) {
                        int val = u.getUserActivityCount(i13, which);
                        args7[i13] = Integer.valueOf(val);
                        if (val != 0) {
                            hasData = true;
                        }
                    }
                    if (hasData) {
                        dumpLine(pw, uid2, category, USER_ACTIVITY_DATA, args7);
                    }
                }
                ArrayMap<String, ? extends Uid.Wakelock> wakelocks2 = u.getWakelockStats();
                for (int iw2 = wakelocks2.size() - 1; iw2 >= 0; iw2--) {
                    Uid.Wakelock wl2 = wakelocks2.valueAt(iw2);
                    sb.setLength(0);
                    String linePrefix = printWakeLockCheckin(sb, wl2.getWakeTime(1), rawRealtime, FullBackup.FILES_TREE_TOKEN, which, ProxyInfo.LOCAL_EXCL_LIST);
                    ...
                    if (sb.length() > 0) {
                        String name = wakelocks2.keyAt(iw2);
                ...
                for (int isy = syncs.size() - 1; isy >= 0; isy--) {
                    ...
                    int count = timer.getCountLocked(which);
                    if (totalTime != 0) {
                        dumpLine(pw, uid2, category, SYNC_DATA, syncs.keyAt(isy), Long.valueOf(totalTime), Integer.valueOf(count));
                    }
                }
                ...
                for (int ij = jobs.size() - 1; ij >= 0; ij--) {
                    ...
                    int count2 = timer2.getCountLocked(which);
                    if (totalTime2 != 0) {
                        dumpLine(pw, uid2, category, JOB_DATA, jobs.keyAt(ij), Long.valueOf(totalTime2), Integer.valueOf(count2));
                    }
                }
                dumpTimer(pw, uid2, category, FLASHLIGHT_DATA, u.getFlashlightTurnedOnTimer(), rawRealtime, which);
                dumpTimer(pw, uid2, category, CAMERA_DATA, u.getCameraTurnedOnTimer(), rawRealtime, which);
                dumpTimer(pw, uid2, category, VIDEO_DATA, u.getVideoTurnedOnTimer(), rawRealtime, which);
                dumpTimer(pw, uid2, category, AUDIO_DATA, u.getAudioTurnedOnTimer(), rawRealtime, which);
                ...
                for (int ise = 0; ise < NSE; ise++) {
                    ...
                    if (timer3 != null) {
                        ...
                        int count3 = timer3.getCountLocked(which);
                        if (totalTime3 != 0) {
                            dumpLine(pw, uid2, category, SENSOR_DATA, Integer.valueOf(sensorNumber), Long.valueOf(totalTime3), Integer.valueOf(count3));
                        }
                    }
                }
                dumpTimer(pw, uid2, category, VIBRATOR_DATA, u.getVibratorOnTimer(), rawRealtime, which);
                dumpTimer(pw, uid2, category, FOREGROUND_DATA, u.getForegroundActivityTimer(), rawRealtime, which);
                ...
                if (userCpuTimeUs > 0 || systemCpuTimeUs > 0 || powerCpuMaUs > 0) {
                    dumpLine(pw, uid2, category, CPU_DATA, Long.valueOf(userCpuTimeUs / 1000), Long.valueOf(systemCpuTimeUs / 1000), Long.valueOf(powerCpuMaUs / 1000));
                }
                ...
                for (int ipr = processStats.size() - 1; ipr >= 0; ipr--) {
                    ...
                    if (userMillis != 0 || systemMillis != 0 || foregroundMillis != 0 || starts != 0 || numAnrs != 0 || numCrashes != 0) {
                        dumpLine(pw, uid2, category, PROCESS_DATA, processStats.keyAt(ipr), Long.valueOf(userMillis), Long.valueOf(systemMillis), Long.valueOf(foregroundMillis), Integer.valueOf(starts), Integer.valueOf(numAnrs), Integer.valueOf(numCrashes));
                    }
                }
                ArrayMap<String, ? extends Uid.Pkg> packageStats = u.getPackageStats();
                for (int ipkg = packageStats.size() - 1; ipkg >= 0; ipkg--) {
                    Uid.Pkg ps2 = packageStats.valueAt(ipkg);
                    int wakeups = 0;
                    ArrayMap<String, ? extends Counter> alarms = ps2.getWakeupAlarmStats();
                    for (int iwa = alarms.size() - 1; iwa >= 0; iwa--) {
                        int count4 = alarms.valueAt(iwa).getCountLocked(which);
                        wakeups += count4;
                        dumpLine(pw, uid2, category, WAKEUP_ALARM_DATA, alarms.keyAt(iwa).replace(',', '_'), Integer.valueOf(count4));
                    }
                    ArrayMap<String, ? extends Uid.Pkg.Serv> serviceStats = ps2.getServiceStats();
                    for (int isvc = serviceStats.size() - 1; isvc >= 0; isvc--) {
                        ...
                        if (startTime != 0 || starts2 != 0 || launches != 0) {
                            dumpLine(pw, uid2, category, APK_DATA, Integer.valueOf(wakeups), packageStats.keyAt(ipkg), serviceStats.keyAt(isvc), Long.valueOf(startTime / 1000), Integer.valueOf(starts2), Integer.valueOf(launches));
                        }
                    }
                }
            }
    ...
    public final void dumpLocked(Context context, PrintWriter pw, String prefix, int which, int reqUid, boolean wifiOnly) {
        ...
        LongCounter dischargeCounter = getDischargeCoulombCounter();
        long dischargeCount = dischargeCounter.getCountLocked(which);
        if (dischargeCount >= 0) {
            sb.setLength(0);
            sb.append(prefix);
            sb.append("  Discharge: ");
            sb.append(BatteryStatsHelper.makemAh(dischargeCount / 1000.0d));
            sb.append(" mAh");
            pw.println(sb.toString());
        }
        LongCounter dischargeScreenOffCounter = getDischargeScreenOffCoulombCounter();
        long dischargeScreenOffCount = dischargeScreenOffCounter.getCountLocked(which);
        if (dischargeScreenOffCount >= 0) {
            sb.setLength(0);
            sb.append(prefix);
            sb.append("  Screen off discharge: ");
            sb.append(BatteryStatsHelper.makemAh(dischargeScreenOffCount / 1000.0d));
            sb.append(" mAh");
            pw.println(sb.toString());
        }
        ...
        if (dischargeScreenOnCount >= 0) {
            ...
            sb.append(BatteryStatsHelper.makemAh(dischargeScreenOnCount / 1000.0d));
            sb.append(" mAh");
            pw.println(sb.toString());
        }
        ...
        if (sippers != null && sippers.size() > 0) {
            ...
            if (helper.getMinDrainedPower() != helper.getMaxDrainedPower()) {
                pw.print(ContactsContract.Aas.ENCODE_SYMBOL);
                printmAh(pw, helper.getMaxDrainedPower());
            }
            pw.println();
            for (int i7 = 0; i7 < sippers.size(); i7++) {
        ...
        if (sippers2 != null && sippers2.size() > 0) {
            ...
            long totalTime = 0;
            for (int i8 = 0; i8 < sippers2.size(); i8++) {
                BatterySipper bs2 = sippers2.get(i8);
                sb.setLength(0);
                sb.append(prefix);
                sb.append("    Uid ");
        ...
        if (reqUid < 0) {
            ...
            if (kernelWakelocks.size() > 0) {
                ...
                if (ktimers.size() > 0) {
                    ...
                    for (int i9 = 0; i9 < ktimers.size(); i9++) {
                        TimerEntry timer2 = ktimers.get(i9);
                        sb.setLength(0);
                        sb.append(prefix);
            ...
            if (timers.size() > 0) {
                ...
                for (int i10 = 0; i10 < timers.size(); i10++) {
                    TimerEntry timer3 = timers.get(i10);
                    sb.setLength(0);
                    sb.append("  Wake lock ");
            ...
            if (wakeupReasons.size() > 0) {
                ...
                for (Map.Entry<String, ? extends Timer> ent2 : wakeupReasons.entrySet()) {
                    reasons.add(new TimerEntry(ent2.getKey(), 0, (Timer) ent2.getValue(), r19.getCountLocked(which)));
                }
                Collections.sort(reasons, timerComparator);
                for (int i11 = 0; i11 < reasons.size(); i11++) {
                    TimerEntry timer4 = reasons.get(i11);
                    sb.setLength(0);
                    sb.append(prefix);
        ...
        for (int iu2 = 0; iu2 < NU; iu2++) {
            ...
            if (reqUid < 0 || uid == reqUid || uid == 1000) {
                ...
                long btRxBytes = u2.getNetworkActivityBytes(4, which);
                long btTxBytes = u2.getNetworkActivityBytes(5, which);
                ...
                int wifiScanCount = u2.getWifiScanCount(which);
                ...
                printControllerActivityIfInteresting(pw, sb, prefix + "  ", "Modem", u2.getModemControllerActivity(), which);
                ...
                printControllerActivityIfInteresting(pw, sb, prefix + "  ", "WiFi", u2.getWifiControllerActivity(), which);
                ...
                boolean uidActivity = printTimer(pw, sb, u2.getBluetoothScanTimer(), rawRealtime, which, prefix, "Bluetooth Scan");
                if (u2.hasUserActivity()) {
                    boolean hasData = false;
                    for (int i12 = 0; i12 < 4; i12++) {
                        int val = u2.getUserActivityCount(i12, which);
                        if (val != 0) {
                            if (!hasData) {
                                sb.setLength(0);
                            ...
                            sb.append(val);
                            sb.append(WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER);
                            sb.append(Uid.USER_ACTIVITY_TYPES[i12]);
                        }
                    }
                    if (hasData) {
                        pw.println(sb.toString());
                ...
                for (int iw2 = wakelocks2.size() - 1; iw2 >= 0; iw2--) {
                    ...
                    sb.append(prefix);
                    sb.append("    Wake lock ");
                    sb.append(wakelocks2.keyAt(iw2));
                    String linePrefix2 = printWakeLock(sb, wl2.getWakeTime(1), rawRealtime, "full", which, ": ");
                    printWakeLock(sb, wl2.getWakeTime(18), rawRealtime, "draw", which, printWakeLock(sb, wl2.getWakeTime(2), rawRealtime, Context.WINDOW_SERVICE, which, printWakeLock(sb, wl2.getWakeTime(0), rawRealtime, "partial", which, linePrefix2)));
                    sb.append(" realtime");
                    pw.println(sb.toString());
                    uidActivity = true;
                    countWakelock++;
                    totalFullWakelock += computeWakeLock(wl2.getWakeTime(1), rawRealtime, which);
                    totalPartialWakelock += computeWakeLock(wl2.getWakeTime(0), rawRealtime, which);
                    totalWindowWakelock += computeWakeLock(wl2.getWakeTime(2), rawRealtime, which);
                    totalDrawWakelock += computeWakeLock(wl2.getWakeTime(18), rawRealtime, which);
                }
                ...
                ArrayMap<String, ? extends Timer> syncs = u2.getSyncStats();
                for (int isy = syncs.size() - 1; isy >= 0; isy--) {
                    Timer timer5 = syncs.valueAt(isy);
                    long totalTime2 = (timer5.getTotalTimeLocked(rawRealtime, which) + 500) / 1000;
                    int count = timer5.getCountLocked(which);
                    sb.setLength(0);
                    sb.append(prefix);
                    sb.append("    Sync ");
                ...
                boolean uidActivity2 = uidActivity | printTimer(pw, sb, u2.getFlashlightTurnedOnTimer(), rawRealtime, which, prefix, "Flashlight") | printTimer(pw, sb, u2.getCameraTurnedOnTimer(), rawRealtime, which, prefix, "Camera") | printTimer(pw, sb, u2.getVideoTurnedOnTimer(), rawRealtime, which, prefix, "Video") | printTimer(pw, sb, u2.getAudioTurnedOnTimer(), rawRealtime, which, prefix, "Audio");
                ...
                for (int ise = 0; ise < NSE; ise++) {
                    ...
                    if (timer7 != null) {
                        ...
                        int count3 = timer7.getCountLocked(which);
                        if (totalTime4 != 0) {
                ...
                boolean uidActivity3 = uidActivity2 | printTimer(pw, sb, u2.getVibratorOnTimer(), rawRealtime, which, prefix, "Vibrator") | printTimer(pw, sb, u2.getForegroundActivityTimer(), rawRealtime, which, prefix, "Foreground activities");
                long totalStateTime = 0;
                for (int ips = 0; ips < 6; ips++) {
                    long time7 = u2.getProcessStateTime(ips, rawRealtime, which);
                    if (time7 > 0) {
                        ...
                        sb.append(" for: ");
                        formatTimeMs(sb, (500 + time7) / 1000);
                        pw.println(sb.toString());
                        uidActivity3 = true;
                    }
                }
                ...
                long userCpuTimeUs = u2.getUserCpuTimeUs(which);
                long systemCpuTimeUs = u2.getSystemCpuTimeUs(which);
                long powerCpuMaUs = u2.getCpuPowerMaUs(which);
                if (userCpuTimeUs > 0 || systemCpuTimeUs > 0 || powerCpuMaUs > 0) {
                    sb.setLength(0);
                    sb.append(prefix);
                    ...
                    sb.append("p=");
                    printmAh(sb, powerCpuMaUs / 3.6E9d);
                    sb.append("mAh");
                    pw.println(sb.toString());
                }
                ArrayMap<String, ? extends Uid.Proc> processStats = u2.getProcessStats();
                for (int ipr = processStats.size() - 1; ipr >= 0; ipr--) {
                    Uid.Proc ps = processStats.valueAt(ipr);
                ...
                ArrayMap<String, ? extends Uid.Pkg> packageStats = u2.getPackageStats();
                for (int ipkg = packageStats.size() - 1; ipkg >= 0; ipkg--) {
                    pw.print(prefix);
                    pw.print("    Apk ");
    ...
    static void printBitDescriptions(PrintWriter pw, int oldval, int newval, HistoryTag wakelockTag, BitDescription[] descriptions, boolean longNames) {
        ...
        for (BitDescription bd : descriptions) {
            if ((bd.mask & diff) != 0) {
                ...
                if (bd.shift < 0) {
                    pw.print((bd.mask & newval) != 0 ? "+" : ContactsContract.Aas.ENCODE_SYMBOL);
            ...
            if (rec.stepDetails != null) {
                if (checkin) {
                    ...
                    if (total > 0) {
                        ...
                        pw.print(String.format("%.1f%%", Float.valueOf(perc)));
                        pw.print(" of ");
