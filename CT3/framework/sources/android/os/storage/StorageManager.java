...
public class StorageManager {
    ...
    private class ObbActionListener extends IObbActionListener.Stub {
        ...
        ObbActionListener(StorageManager this$0, ObbActionListener obbActionListener) {
            this();
        }
    ...
    public String getBestVolumeDescription(VolumeInfo vol) {
        ...
        if (vol.isPhoneStorage()) {
            String volumeName = this.mContext.getResources().getString(134545634);
    ...
    public List<StorageVolume> getStorageVolumes() {
        ...
        Collections.addAll(res, getVolumeList(UserHandle.myUserId(), BluetoothClass.Device.Major.IMAGING));
        return res;
    }

    public StorageVolume getPrimaryStorageVolume() {
        return getVolumeList(UserHandle.myUserId(), BluetoothClass.Device.Major.IMAGING)[0];
    }
    ...
    public static boolean isEncrypted() {
        String state = SystemProperties.get("ro.crypto.state", ProxyInfo.LOCAL_EXCL_LIST);
    ...
    public static boolean isFileEncryptedNativeOnly() {
        ...
        String status = SystemProperties.get("ro.crypto.type", ProxyInfo.LOCAL_EXCL_LIST);
    ...
    public static boolean isBlockEncrypting() {
        String state = SystemProperties.get("vold.encrypt_progress", ProxyInfo.LOCAL_EXCL_LIST);
        return !ProxyInfo.LOCAL_EXCL_LIST.equalsIgnoreCase(state);
    }
    ...
    public static File maybeTranslateEmulatedPathToInternal(File path) {
        ...
        IMountService mountService = IMountService.Stub.asInterface(ServiceManager.getService("mount"));
        try {
            VolumeInfo[] vols = mountService.getVolumes(0);
            for (VolumeInfo vol : vols) {
                if ((vol.getType() == 2 || vol.getType() == 0) && vol.isMountedReadable() && (internalPath = FileUtils.rewriteAfterRename(vol.getPath(), vol.getInternalPath(), path)) != null && internalPath.exists()) {
                    return internalPath;
                }
            }
            return path;
