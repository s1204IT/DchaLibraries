...
public class DiskInfo implements Parcelable {
    ...
    public String getDescription() {
        ...
        if ((this.flags & 4) != 0) {
            if (isPhoneStorage(getId())) {
                return res.getString(134545634);
            }
            return isInteresting(this.label) ? res.getString(17040575, this.label) : res.getString(17040574);
        }
        if ((this.flags & 8) != 0) {
            return isInteresting(this.label) ? res.getString(17040577, this.label) : res.getString(17040576);
        }
        return null;
    }
    ...
    public static boolean isPhoneStorage(String diskId) {
        ...
        boolean isEMMCProject = SystemProperties.get("ro.mtk_emmc_support").equals(WifiEnterpriseConfig.ENGINE_ENABLE);
        if (isEMMCProject) {
