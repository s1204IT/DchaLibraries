...
public class MediaEncoderFilter extends Filter {
    ...
    private void updateMediaRecorderParams() {
        ...
        this.mMediaRecorder.setOnCameraReleasedListener(this.mCameraReleasedListener);
    ...
    private void startRecording(FilterContext context) {
        ...
        try {
            ...
            this.mCameraReleasedListener.onInfo(this.mMediaRecorder, 1999, -1);
        } catch (IOException e) {
    ...
    public boolean skipFrameAndModifyTimestamp(long timestampNs) {
        if (this.mLogVerbose) {
            Log.v(TAG, "skipFrameAndModifyTimestamp(" + timestampNs + "), recorderStartTimestampNs=" + this.recorderStartTimestampNs);
        }
        if (timestampNs < this.recorderStartTimestampNs) {
            return true;
        }
