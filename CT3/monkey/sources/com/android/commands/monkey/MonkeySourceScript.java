    ...
    private void handleEvent(String s, String[] args) {
        ...
        if (s.indexOf(EVENT_KEYWORD_KEY) >= 0 && args.length == 8) {
            try {
                ...
                MonkeyKeyEvent e2 = new MonkeyKeyEvent(downTime, eventTime, action, code, repeat, metaState, device, scancode);
        ...
        if ((s.indexOf(EVENT_KEYWORD_POINTER) >= 0 || s.indexOf(EVENT_KEYWORD_TRACKBALL) >= 0) && args.length == 12) {
            try {
                ...
                MonkeyMotionEvent e4 = s.indexOf("Pointer") > 0 ? new MonkeyTouchEvent(action2) : new MonkeyTrackballEvent(action2);
                e4.setDownTime(downTime2).setEventTime(eventTime2).setMetaState(metaState2).setPrecision(xPrecision, yPrecision).setDeviceId(device2).setEdgeFlags(edgeFlags).addPointer(0, x, y, pressure, size);
        ...
        if ((s.indexOf(EVENT_KEYWORD_POINTER) >= 0 || s.indexOf(EVENT_KEYWORD_TRACKBALL) >= 0) && args.length == 13) {
            try {
                ...
                if (s.indexOf("Pointer") > 0) {
                    e = action3 == 5 ? new MonkeyTouchEvent((pointerId << 8) | 5).setIntermediateNote(true) : new MonkeyTouchEvent(action3);
                ...
                if (pointerId == 1) {
                    e.setDownTime(downTime3).setEventTime(eventTime3).setMetaState(metaState3).setPrecision(xPrecision2, yPrecision2).setDeviceId(device3).setEdgeFlags(edgeFlags2).addPointer(0, this.mLastX[0], this.mLastY[0], pressure2, size2).addPointer(1, x2, y2, pressure2, size2);
                    this.mLastX[1] = x2;
                    this.mLastY[1] = y2;
                } else if (pointerId == 0) {
                    e.setDownTime(downTime3).setEventTime(eventTime3).setMetaState(metaState3).setPrecision(xPrecision2, yPrecision2).setDeviceId(device3).setEdgeFlags(edgeFlags2).addPointer(0, x2, y2, pressure2, size2);
                    if (action3 == 6) {
        ...
        if (s.indexOf(EVENT_KEYWORD_TAP) >= 0 && args.length >= 2) {
            try {
                ...
                long tapDuration = args.length == 3 ? Long.parseLong(args[2]) : 0L;
        ...
        if (s.indexOf(EVENT_KEYWORD_DRAG) >= 0 && args.length == 5) {
            ...
            if (stepCount > 0) {
                ...
                this.mQ.addLast((MonkeyEvent) new MonkeyTouchEvent(0).setDownTime(downTime6).setEventTime(eventTime4).addPointer(0, xStart, yStart, 1.0f, 5.0f));
                for (int i = 0; i < stepCount; i++) {
                    x5 += xStep;
                    y5 += yStep;
                    long eventTime5 = SystemClock.uptimeMillis();
                    this.mQ.addLast((MonkeyEvent) new MonkeyTouchEvent(2).setDownTime(downTime6).setEventTime(eventTime5).addPointer(0, x5, y5, 1.0f, 5.0f));
                }
                long eventTime6 = SystemClock.uptimeMillis();
                this.mQ.addLast((MonkeyEvent) new MonkeyTouchEvent(1).setDownTime(downTime6).setEventTime(eventTime6).addPointer(0, x5, y5, 1.0f, 5.0f));
            }
        }
        if (s.indexOf(EVENT_KEYWORD_PINCH_ZOOM) >= 0 && args.length == 9) {
            ...
            if (stepCount2 > 0) {
                ...
                this.mQ.addLast((MonkeyEvent) new MonkeyTouchEvent(0).setDownTime(downTime7).setEventTime(eventTime7).addPointer(0, pt1xStart, pt1yStart, 1.0f, 5.0f));
                this.mQ.addLast((MonkeyEvent) new MonkeyTouchEvent(261).setDownTime(downTime7).addPointer(0, pt1xStart, pt1yStart).addPointer(1, pt2xStart, pt2yStart).setIntermediateNote(true));
        ...
        if (s.indexOf(EVENT_KEYWORD_FLIP) >= 0 && args.length == 1) {
            ...
            this.mQ.addLast((MonkeyEvent) new MonkeyFlipEvent(keyboardOpen));
        }
        ...
        if (s.indexOf(EVENT_KEYWORD_INSTRUMENTATION) >= 0 && args.length == 2) {
            ...
            this.mQ.addLast((MonkeyEvent) new MonkeyInstrumentationEvent(test_name, runner_name));
            return;
        }
        if (s.indexOf(EVENT_KEYWORD_WAIT) >= 0 && args.length == 1) {
            try {
                long sleeptime = Integer.parseInt(args[0]);
                this.mQ.addLast((MonkeyEvent) new MonkeyWaitEvent(sleeptime));
                return;
        ...
        if (s.indexOf(EVENT_KEYWORD_POWERLOG) >= 0 && args.length > 0) {
            ...
            if (args.length == 1) {
                ...
                this.mQ.addLast((MonkeyEvent) new MonkeyPowerEvent(power_log_type, test_case_status));
            }
        }
        ...
        if (s.indexOf(EVENT_KEYWORD_RUNCMD) >= 0 && args.length == 1) {
            String cmd = args[0];
            this.mQ.addLast((MonkeyEvent) new MonkeyCommandEvent(cmd));
        }
        if (s.indexOf(EVENT_KEYWORD_INPUT_STRING) >= 0 && args.length == 1) {
            String input = args[0];
            String cmd2 = "input text " + input;
            this.mQ.addLast((MonkeyEvent) new MonkeyCommandEvent(cmd2));
            return;
        }
        ...
        if (s.indexOf(EVENT_KEYWORD_END_FRAMERATE_CAPTURE) >= 0 && args.length == 1) {
            ...
            this.mQ.addLast((MonkeyEvent) new MonkeyGetFrameRateEvent("end", input2));
            return;
        }
        if (s.indexOf(EVENT_KEYWORD_START_APP_FRAMERATE_CAPTURE) >= 0 && args.length == 1) {
            String app = args[0];
            this.mQ.addLast((MonkeyEvent) new MonkeyGetAppFrameRateEvent("start", app));
        } else {
            ...
            String app2 = args[0];
            String label = args[1];
            this.mQ.addLast((MonkeyEvent) new MonkeyGetAppFrameRateEvent("end", app2, label));
        }
    }
