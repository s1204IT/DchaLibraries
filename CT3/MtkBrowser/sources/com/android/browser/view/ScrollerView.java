...
public class ScrollerView extends FrameLayout {
    ...
    public boolean executeKeyEvent(KeyEvent event) {
        ...
        if (!canScroll()) {
            ...
            View nextFocused = FocusFinder.getInstance().findNextFocus(this, currentFocused, IBrowserFeatureIndexExt.CUSTOM_PREFERENCE_ACCESSIBILITY);
            if (nextFocused == null || nextFocused == this) {
                return false;
            }
            return nextFocused.requestFocus(IBrowserFeatureIndexExt.CUSTOM_PREFERENCE_ACCESSIBILITY);
        }
        ...
        switch (event.getKeyCode()) {
            ...
            case 20:
                if (!event.isAltPressed()) {
                    boolean handled3 = arrowScroll(IBrowserFeatureIndexExt.CUSTOM_PREFERENCE_ACCESSIBILITY);
                    return handled3;
                }
                boolean handled4 = fullScroll(IBrowserFeatureIndexExt.CUSTOM_PREFERENCE_ACCESSIBILITY);
                return handled4;
            case 62:
        ...
        switch (action & 255) {
            ...
            case 2:
                ...
                if (activePointerId != -1) {
                    ...
                    if (pointerIndex == -1) {
                        Log.e("ScrollerView", "Invalid active pointer index = " + activePointerId + " at onInterceptTouchEvent ACTION_MOVE");
                    } else {
        ...
        switch (action & 255) {
            ...
            case 2:
                if (this.mIsOrthoDragged) {
                    ...
                    float y2 = ev.getY(activePointerIndex);
                    if (isOrthoMove(x - this.mDownCoords.x, y2 - this.mDownCoords.y)) {
                        View view = this.mDownView;
                        if (this.mHorizontal) {
                            f = y2 - this.mDownCoords.y;
                        } else {
                ...
                if (this.mIsBeingDragged) {
                    ...
                    if (activePointerIndex2 == -1) {
                        Log.e("ScrollerView", "Invalid active pointer index = " + this.mActivePointerId + " at onTouchEvent ACTION_MOVE begin dragged");
                        return true;
                    }
                    float y3 = this.mHorizontal ? ev.getX(activePointerIndex2) : ev.getY(activePointerIndex2);
                    int deltaY = (int) (this.mLastMotionY - y3);
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (top < viewBottom && viewTop < bottom) {
                ...
                if (focusCandidate == null) {
                    ...
                    if (!topFocus || viewTop >= ctop) {
                        viewIsCloserToBoundary = !topFocus && viewBottom > cbot;
                    } else {
    ...
    private boolean isOrthoMove(float moveX, float moveY) {
        if (!this.mHorizontal || Math.abs(moveY) <= Math.abs(moveX)) {
            return !this.mHorizontal && Math.abs(moveX) > Math.abs(moveY);
        }
        return true;
    }
    ...
    protected int computeHorizontalScrollRange() {
        ...
        if (scrollX > overscrollBottom) {
            return scrollRange + (scrollX - overscrollBottom);
        }
        return scrollRange;
    }
    ...
    private int computeScrollDeltaToGetChildRectOnScreenHorizontal(Rect rect) {
        ...
        if (rect.right > screenRight && rect.left > screenLeft) {
            if (rect.width() > width) {
                scrollXDelta2 = (rect.left - screenLeft) + 0;
            } else {
                scrollXDelta2 = (rect.right - screenRight) + 0;
            }
            int right = getChildAt(0).getRight();
            int distanceToRight = right - screenRight;
            return Math.min(scrollXDelta2, distanceToRight);
        }
    ...
    protected boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
        ...
        if (this.mHorizontal) {
            ...
            direction = IBrowserFeatureIndexExt.CUSTOM_PREFERENCE_ACCESSIBILITY;
        } else if (direction == 1) {
            direction = 33;
        }
