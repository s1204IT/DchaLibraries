    ...
    class TabLayout extends LinearLayout {
        ...
        protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
            ...
            if (layoutDirection == 1) {
                nextLeft = getChildAt(0).getLeft() + TabScrollView.this.mTabOverlap;
            }
            for (int i = 1; i < getChildCount(); i++) {
                View tab = getChildAt(i);
                int w = tab.getRight() - tab.getLeft();
                if (layoutDirection == 1) {
                    tab.layout(nextLeft - w, tab.getTop(), nextLeft, tab.getBottom());
                    nextLeft = (nextLeft - w) + TabScrollView.this.mTabOverlap;
                } else {
