...
public class SampleList extends AbstractList<ByteBuffer> {
    ...
    private void splitToArrays(Map<Long, Long> offsets2Sizes) {
        ...
        for (int i = 0; i < keys.size(); i++) {
            nuOffsets[i + this.offsets.length] = keys.get(i).longValue();
            nuSizes[i + this.sizes.length] = offsets2Sizes.get(keys.get(i)).longValue();
        }
        this.sizes = nuSizes;
    ...
    public ByteBuffer get(int index) {
        ...
        MediaDataBox[] arr$ = this.mdats;
        for (MediaDataBox mediaDataBox : arr$) {
            long start = this.mdatStartCache.get(mediaDataBox).longValue();
            long end = this.mdatEndCache.get(mediaDataBox).longValue();
                ...
                for (TrackRunBox trun : trackFragmentBox.getBoxes(TrackRunBox.class)) {
                    ...
                    for (TrackRunBox.Entry entry : trun.getEntries()) {
                        if (trun.isSampleSizePresent()) {
                            ...
                            long sampleSize2 = tfhd.getDefaultSampleSize();
                            offsets2Sizes.put(Long.valueOf(offset + sampleBaseOffset), Long.valueOf(sampleSize2));
                            offset += sampleSize2;
                        } else {
                            if (trex == null) {
                                throw new RuntimeException("File doesn't contain trex box but track fragments aren't fully self contained. Cannot determine sample size.");
                            }
                            long sampleSize3 = trex.getDefaultSampleSize();
                            offsets2Sizes.put(Long.valueOf(offset + sampleBaseOffset), Long.valueOf(sampleSize3));
                            offset += sampleSize3;
                        }
                    }
                }
