...
public class OverScroller {
    ...
    static class SplineOverScroller {
        ...
        static void initFromContext(Context context) {
            ...
            PHYSICAL_COEF = 386.0878f * ppi * 0.84f;
        }
        ...
        private double getSplineDeceleration(int velocity) {
            return Math.log((0.35f * Math.abs(velocity)) / (this.mFlingFriction * PHYSICAL_COEF));
        }

        private double getSplineFlingDistance(int velocity) {
            double l = getSplineDeceleration(velocity);
            double decelMinusOne = ((double) DECELERATION_RATE) - 1.0d;
            return ((double) (this.mFlingFriction * PHYSICAL_COEF)) * Math.exp((((double) DECELERATION_RATE) / decelMinusOne) * l);
        }
