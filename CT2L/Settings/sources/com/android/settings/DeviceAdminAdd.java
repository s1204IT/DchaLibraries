...
public class DeviceAdminAdd extends Activity {
    ...
    void updateInterface() {
        ...
        if (!this.mRefreshing && !this.mAddingProfileOwner && this.mDPM.isAdminActive(this.mDeviceAdmin.getComponent())) {
            if (this.mActivePolicies.size() == 0) {
                ArrayList<DeviceAdminInfo.PolicyInfo> policies = this.mDeviceAdmin.getUsedPolicies();
                for (int i = 0; i < policies.size(); i++) {
                    View view = AppSecurityPermissions.getPermissionItemView(this, getText(policies.get(i).label), "", true);
        ...
        if (this.mAddingPolicies.size() == 0) {
            ArrayList<DeviceAdminInfo.PolicyInfo> policies2 = this.mDeviceAdmin.getUsedPolicies();
            for (int i2 = 0; i2 < policies2.size(); i2++) {
                DeviceAdminInfo.PolicyInfo pi = policies2.get(i2);
                View view2 = AppSecurityPermissions.getPermissionItemView(this, getText(pi.label), getText(pi.description), true);
                this.mAddingPolicies.add(view2);
                this.mAdminPolicies.addView(view2);
            }
        }
    ...
    void toggleMessageEllipsis(View v) {
        ...
        this.mAddMsgExpander.setImageResource(this.mAddMsgEllipsized ? android.R.drawable.clock_dial : android.R.drawable.cling_button_pressed);
    }
