...
import com.android.settings.bluetooth.BluetoothEventManager;
...
final class LocalBluetoothProfileManager {
    ...
    public synchronized boolean isManagerReady() {
        boolean zIsProfileReady;
        LocalBluetoothProfile profile = this.mHeadsetProfile;
        if (profile != null) {
            zIsProfileReady = profile.isProfileReady();
        } else {
            LocalBluetoothProfile profile2 = this.mA2dpProfile;
            if (profile2 != null) {
                zIsProfileReady = profile2.isProfileReady();
            } else {
                zIsProfileReady = false;
            }
        }
        return zIsProfileReady;
    }
