...
public final class BluetoothPermissionRequest extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals("android.bluetooth.device.action.CONNECTION_ACCESS_REQUEST")) {
            ...
            if (!com.android.settings.Utils.isManagedProfile(um)) {
                ...
                if (!checkUserChoice()) {
                    ...
                    Notification notification = new Notification.Builder(context).setContentTitle(title).setTicker(message).setContentText(message).setSmallIcon(android.R.drawable.stat_sys_data_bluetooth).setAutoCancel(true).setPriority(2).setOnlyAlertOnce(false).setDefaults(-1).setContentIntent(PendingIntent.getActivity(context, 0, connectionAccessIntent, 0)).setDeleteIntent(PendingIntent.getBroadcast(context, 0, deleteIntent, 0)).setColor(context.getResources().getColor(android.R.color.system_accent3_600)).build();
                    notification.flags |= 32;
                    NotificationManager notificationManager = (NotificationManager) context.getSystemService("notification");
                    notificationManager.notify(getNotificationTag(this.mRequestType), android.R.drawable.stat_sys_data_bluetooth, notification);
                    return;
                }
                return;
            }
        ...
        if (action.equals("android.bluetooth.device.action.CONNECTION_ACCESS_CANCEL")) {
            ...
            manager.cancel(getNotificationTag(this.mRequestType), android.R.drawable.stat_sys_data_bluetooth);
        }
    }
    ...
    private boolean checkUserChoice() {
        ...
        } else if (this.mRequestType == 3 && (messagePermission = cachedDevice.getMessagePermissionChoice()) != 0) {
    ...
    private void playPopupAlert(Context context) {
        ...
        NotificationManager manager = (NotificationManager) context.getSystemService("notification");
        manager.notify(android.R.drawable.stat_sys_data_bluetooth, builder.getNotification());
    }
}
