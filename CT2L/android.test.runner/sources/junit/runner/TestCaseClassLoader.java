...
public class TestCaseClassLoader extends ClassLoader {
    ...
    public synchronized Class loadClass(String name, boolean resolve) throws ClassNotFoundException {
        Class<?> clsFindSystemClass;
        Class<?> clsFindLoadedClass = findLoadedClass(name);
        if (clsFindLoadedClass != null) {
            clsFindSystemClass = clsFindLoadedClass;
        } else if (isExcluded(name)) {
            try {
                clsFindSystemClass = findSystemClass(name);
            ...
            if (clsFindLoadedClass == null) {
                ...
                clsFindLoadedClass = defineClass(name, data, 0, data.length);
            }
            if (resolve) {
    ...
    boolean isJar(String pathEntry) {
        return pathEntry.endsWith(".jar") || pathEntry.endsWith(".apk") || pathEntry.endsWith(".zip");
    }
    ...
    private void readExcludedPackages() {
        ...
        if (is != null) {
            ...
            try {
                ...
                Enumeration<?> enumerationPropertyNames = p.propertyNames();
