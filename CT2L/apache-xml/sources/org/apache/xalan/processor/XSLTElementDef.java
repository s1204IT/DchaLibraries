...
public class XSLTElementDef {
    ...
    XSLTAttributeDef getAttributeDef(String uri, String localName) {
        ...
        for (XSLTAttributeDef attrDef : attrDefs) {
            String uriDef = attrDef.getNamespace();
            String nameDef = attrDef.getName();
            if (nameDef.equals("*")) {
                if (equalsMayBeNullOrZeroLen(uri, uriDef)) {
                    return attrDef;
                }
                if (uriDef != null && uriDef.equals("*") && uri != null && uri.length() > 0) {
                    return attrDef;
                }
            }
        ...
        if (defaultDef == null && uri.length() > 0 && !equalsMayBeNullOrZeroLen(uri, Constants.S_XSLNAMESPACEURL)) {
            XSLTAttributeDef attrDef2 = XSLTAttributeDef.m_foreignAttr;
            return attrDef2;
        }
        XSLTAttributeDef attrDef3 = defaultDef;
        return attrDef3;
    }
