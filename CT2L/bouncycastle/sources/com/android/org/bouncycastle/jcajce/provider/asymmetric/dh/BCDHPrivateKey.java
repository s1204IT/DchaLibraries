...
public class BCDHPrivateKey implements DHPrivateKey, PKCS12BagAttributeCarrier {
    ...
    public byte[] getEncoded() {
        ...
        try {
            if (this.info != null) {
                encoded = this.info.getEncoded(ASN1Encoding.DER);
            } else {
                PrivateKeyInfo info = new PrivateKeyInfo(new AlgorithmIdentifier(PKCSObjectIdentifiers.dhKeyAgreement, (ASN1Encodable) new DHParameter(this.dhSpec.getP(), this.dhSpec.getG(), this.dhSpec.getL()).toASN1Primitive()), new ASN1Integer(getX()));
                encoded = info.getEncoded(ASN1Encoding.DER);
            }
            return encoded;
        } catch (Exception e) {
            return null;
