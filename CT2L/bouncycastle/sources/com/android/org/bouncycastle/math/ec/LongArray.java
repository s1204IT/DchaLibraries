...
class LongArray {
    ...
    private static int bitLength(long w) {
        ...
        if (t == 0) {
            int t2 = u >>> 8;
            i = t2 == 0 ? bitLengths[u] : bitLengths[t2] + 8;
        } else {
            int v = t >>> 8;
            i = v == 0 ? bitLengths[t] + Tnaf.POW_2_WIDTH : bitLengths[v] + 24;
        }
        return b + i;
    }
    ...
    public BigInteger toBigInteger() {
        ...
        int barrLen = ((usedLen - 1) * 8) + barrI2;
        ...
        if (aDeg != 0) {
            ...
            for (int k = 56; k >= 0; k -= 8) {
                ...
                while (true) {
                    int j2 = j;
                    if (j2 < aLen) {
                        int aVal = (int) (a2[j2] >>> k);
                        int u = aVal & 15;
                        int v = (aVal >>> 4) & 15;
                        addBoth(c2, j2 - 1, T0, ti[u], T1, ti[v], bMax);
                        j = j2 + 2;
                    }
                }
                shiftUp(c2, 0, cLen, 8);
            ...
            for (int k2 = 56; k2 >= 0; k2 -= 8) {
                for (int j3 = 0; j3 < aLen; j3 += 2) {
                    int aVal2 = (int) (a2[j3] >>> k2);
                    int u2 = aVal2 & 15;
                    int v2 = (aVal2 >>> 4) & 15;
                    addBoth(c2, j3, T0, ti[u2], T1, ti[v2], bMax);
                }
                if (k2 > 0) {
                    shiftUp(c2, 0, cLen, 8);
                }
            }
            return reduceResult(c2, 0, cLen, m, ks);
        }
        return this;
    }
        ...
        if (aDeg != 0) {
            ...
            for (int aPos = 0; aPos < aLen; aPos++) {
                ...
                while (true) {
                    ...
                    long aVal2 = aVal >>> 4;
                    int v = ((int) aVal2) & 15;
                    addBoth(c2, cOff, T0, ti[u], T1, ti[v], bMax);
                    aVal = aVal2 >>> 4;
                    if (aVal == 0) {
                        break;
                    }
            ...
            while (true) {
                cOff2 -= cLen;
                if (cOff2 != 0) {
                    addShiftedUp(c2, cOff2 - cLen, c2, cOff2, cLen, 8);
                } else {
                    return reduceResult(c2, 0, cLen, m, ks);
                }
            }
    ...
    public LongArray modMultiplyAlt(LongArray other, int m, int[] ks) {
        ...
        if (aDeg != 0) {
            ...
            long[] c2 = new long[cTotal + cLen + 1];
            interleave(A.m_ints, 0, c2, 0, aLen, 4);
            int bOff = aLen;
            System.arraycopy(B.m_ints, 0, c2, bOff, bLen);
            for (int bank = 1; bank < 8; bank++) {
                bOff += bMax;
                shiftUp(c2, aLen, c2, bOff, bMax, bank);
            }
            int MASK = 16 - 1;
            int k = 0;
            while (true) {
                ...
                do {
                    ...
                    while (true) {
                        ...
                        if (index != 0) {
                            add(c2, ci[index] + aPos, c2, bOff2, bMax);
                        }
                        bank2++;
                        if (bank2 == 8) {
                            break;
                ...
                shiftUp(c2, aLen, bTotal, 8);
            }
            int ciPos = ci.length;
            while (true) {
                ciPos--;
                if (ciPos <= 1) {
                    return reduceResult(c2, ci[1], cLen, m, ks);
                }
                if ((((long) ciPos) & 1) == 0) {
                    addShiftedUp(c2, ci[ciPos >>> 1], c2, ci[ciPos], cLen, 16);
                } else {
                    distribute(c2, ci[ciPos], ci[ciPos - 1], ci[1], cLen);
                }
            }
        } else {
            return this;
    ...
    private static long interleave3(long x) {
        ...
        return interleave3_21to63(((int) x) & 2097151) | z | (interleave3_21to63(((int) (x >>> 21)) & 2097151) << 1) | (interleave3_21to63(((int) (x >>> 42)) & 2097151) << 2);
    }
