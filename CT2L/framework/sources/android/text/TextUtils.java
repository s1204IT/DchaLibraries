...
public class TextUtils {
    ...
    public static int indexOf(CharSequence s, char ch, int start) {
        return s.getClass() == String.class ? ((String) s).indexOf(ch, start) : indexOf(s, ch, start, s.length());
    }
    ...
    public static int lastIndexOf(CharSequence s, char ch, int last) {
        return s.getClass() == String.class ? ((String) s).lastIndexOf(ch, last) : lastIndexOf(s, ch, 0, last);
    }
    ...
    public static String[] split(String text, String expression) {
        return text.length() == 0 ? EMPTY_STRING_ARRAY : text.split(expression, -1);
    }

    public static String[] split(String text, Pattern pattern) {
        return text.length() == 0 ? EMPTY_STRING_ARRAY : pattern.split(text, -1);
    }
    ...
    public static int getOffsetBefore(CharSequence text, int offset) {
        ...
        int offset2 = (c >= 56320 && c <= 57343 && (c1 = text.charAt(offset - 2)) >= 55296 && c1 <= 56319) ? offset - 2 : offset - 1;
    ...
    public static int getOffsetAfter(CharSequence text, int offset) {
        ...
        if (offset != len && offset != len - 1) {
            ...
            if (c >= 55296 && c <= 56319 && (c1 = text.charAt(offset + 1)) >= 56320 && c1 <= 57343) {
                offset2 = offset + 2;
            } else {
                offset2 = offset + 1;
            }
    ...
    public static CharSequence ellipsize(CharSequence text, TextPaint paint, float avail, TruncateAt where, boolean preserveLength, EllipsizeCallback callback, TextDirectionHeuristic textDir, String ellipsis) {
        ...
        try {
            ...
            if (avail2 < 0.0f) {
                ...
                int left3 = mt.breakText(len, true, avail2);
                left = left3;
            } else {
                right = len - mt.breakText(len, false, avail2 / 2.0f);
                int left4 = mt.breakText(right, true, avail2 - mt.measure(right, len));
                left = left4;
            }
            if (callback != null) {
    ...
    private static float setPara(MeasuredText mt, TextPaint paint, CharSequence text, int start, int end, TextDirectionHeuristic textDir) {
        ...
        while (spanStart < len) {
            ...
            width2 += mt.addStyleRun(paint, (MetricAffectingSpan[]) removeEmptySpans(spans, sp, MetricAffectingSpan.class), spanEnd - spanStart, null);
            spanStart = spanEnd;
        }
        return width2;
    }
