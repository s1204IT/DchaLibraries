...
public class AudioService extends IAudioService.Stub {
    ...
    private static int[] MAX_STREAM_VOLUME = {5, 7, 7, 15, 7, 7, 15, 7, 15, 15, 15};
    private static int[] DEFAULT_STREAM_VOLUME = {4, 7, 5, 11, 6, 5, 7, 7, 11, 11, 11};
    private static final int[] STEAM_VOLUME_OPS = {34, 36, 35, 36, 37, 38, 39, 36, 36, 36, 36};
    private static final String[] STREAM_NAMES = {"STREAM_VOICE_CALL", "STREAM_SYSTEM", "STREAM_RING", "STREAM_MUSIC", "STREAM_ALARM", "STREAM_NOTIFICATION", "STREAM_BLUETOOTH_SCO", "STREAM_SYSTEM_ENFORCED", "STREAM_DTMF", "STREAM_TTS", "STREAM_FM"};
    ...
    static int access$10408(AudioService x0) {
    ...
    public AudioService(Context context) {
        ...
        int maxVolume2 = SystemProperties.getInt("ro.config.media_vol_steps", MAX_STREAM_VOLUME[3]);
        if (maxVolume2 != MAX_STREAM_VOLUME[3]) {
            MAX_STREAM_VOLUME[3] = maxVolume2;
            DEFAULT_STREAM_VOLUME[3] = (maxVolume2 * 3) / 4;
        }
    ...
    private void readPersistedSettings() {
        ...
        boolean microphoneMute = Settings.System.getIntForUser(cr, Settings.System.MICROPHONE_MUTE, 0, -2) == 1;
    ...
    private void onSetStreamVolume(int streamType, int index, int flags, int device) {
        ...
        if ((flags & 2) != 0 || this.mStreamVolumeAlias[streamType] == getMasterStreamType()) {
            int newRingerMode = (index == 0 && this.mHasVibrator) ? 1 : 2;
            setRingerMode(newRingerMode, "AudioService.onSetStreamVolume", false);
        }
    }
    ...
    private int findVolumeDelta(int direction, int volume) {
        ...
        if (direction == 1) {
            ...
            while (true) {
                ...
                if (volume > this.mMasterVolumeRamp[i2]) {
                    i2 += 2;
                } else {
    ...
    public boolean isStreamMute(int streamType) {
        boolean zIsMuted_syncVSS;
        if (streamType == Integer.MIN_VALUE) {
            streamType = getActiveStreamType(streamType);
        }
        synchronized (VolumeStreamState.class) {
            zIsMuted_syncVSS = this.mStreamStates[streamType].isMuted_syncVSS();
        }
        return zIsMuted_syncVSS;
    }
    ...
    private void setMasterMuteInternal(boolean state, int flags, String callingPackage, IBinder cb, int uid) {
        if (!this.mUseFixedVolume && this.mAppOps.noteOp(33, uid, callingPackage) == 0 && state != AudioSystem.getMasterMute()) {
            ...
            sendMsg(this.mAudioHandler, 11, 0, state ? 1 : 0, UserHandle.getCallingUserId(), null, 500);
        ...
        do {
            if (mode == 0) {
                if (!this.mSetModeDeathHandlers.isEmpty()) {
                    SetModeDeathHandler hdlr2 = this.mSetModeDeathHandlers.get(0);
                    hdlr = hdlr2;
                    cb = hdlr.getBinder();
                    mode = hdlr.getMode();
                    if (DEBUG_MODE) {
                ...
                synchronized (VolumeStreamState.class) {
                    if (streamState.isMuted_syncVSS() && ((!isStreamAffectedByMute(streamType) && !isStreamMutedByRingerMode(streamType)) || this.mUseFixedVolume)) {
                        ...
                        for (int i = 0; i < size; i++) {
                            ...
                            ((VolumeStreamState.VolumeDeathHandler) streamState.mDeathHandlers.get(i)).mute_syncVSS(false);
                        }
                    }
                }
            }
    ...
    public void setSpeakerphoneOn(boolean on) {
        if (checkAudioSettingsPermission("setSpeakerphoneOn()")) {
            boolean need_force_record = false;
            if (on) {
                if (this.mForcedUseForComm == 3) {
                    need_force_record = true;
                }
    ...
    private void onConfigureSafeVolume(boolean force) {
        ...
        synchronized (this.mSafeMediaVolumeState) {
            ...
            if (this.mMcc != mcc || (this.mMcc == 0 && force)) {
                this.mSafeMediaVolumeIndex = this.mContext.getResources().getInteger(R.integer.config_safe_media_volume_index) * 10;
                boolean safeMediaVolumeEnabled = SystemProperties.getBoolean("audio.safemedia.force", false) || this.mContext.getResources().getBoolean(R.bool.config_safe_media_volume_enabled);
    ...
    boolean updateRingerModeAffectedStreams() {
        ...
        int ringerModeAffectedStreams4 = Settings.System.getIntForUser(this.mContentResolver, Settings.System.MODE_RINGER_STREAMS_AFFECTED, 166, -2);
        int ringerModeAffectedStreams5 = ringerModeAffectedStreams4 | 38;
        switch (this.mPlatformType) {
            case 2:
                ringerModeAffectedStreams = 0;
                break;
            default:
                ringerModeAffectedStreams = ringerModeAffectedStreams5 & (-9);
                break;
        }
        ...
        switch (this.mPlatformType) {
            case 1:
                ...
                if (suggestedStreamType == Integer.MIN_VALUE) {
                    ...
                    if (AudioSystem.isStreamActive(10, 0)) {
                        if (DEBUG_VOL) {
    ...
    public class VolumeStreamState {
        ...
        public void readSettings() {
            synchronized (VolumeStreamState.class) {
                ...
                if (this.mStreamType == 1 || this.mStreamType == 7) {
                    int index = AudioService.DEFAULT_STREAM_VOLUME[this.mStreamType] * 10;
                ...
                while (remainingDevices != 0) {
                    ...
                    if ((device & remainingDevices) != 0) {
                        ...
                        int defaultIndex = device == 1073741824 ? AudioService.DEFAULT_STREAM_VOLUME[this.mStreamType] : -1;
        ...
        public void setAllIndexes(VolumeStreamState srcStream) {
            synchronized (VolumeStreamState.class) {
                ...
                while (it.hasNext()) {
                    it.next().setValue(Integer.valueOf(index2));
                }
                for (Map.Entry<Integer, Integer> entry : srcStream.mIndex.entrySet()) {
                    int device = entry.getKey().intValue();
                    int index3 = entry.getValue().intValue();
                ...
                if (state) {
                    if (this.mMuteCount == 0) {
                        try {
                            ...
                            if (VolumeStreamState.this.mStreamType == 7 && AudioService.this.isCameraSoundForced()) {
                                if (AudioService.DEBUG_VOL) {
                                    Log.d(AudioService.TAG, "set stream enforced volume as max because of force shutter sound");
                                }
                                this.mMuteCount--;
                                for (Map.Entry entry : VolumeStreamState.this.mIndex.entrySet()) {
                                    int device = ((Integer) entry.getKey()).intValue();
                                    VolumeStreamState.this.mIndex.put(Integer.valueOf(device), Integer.valueOf(VolumeStreamState.this.mIndexMax));
                                }
                            }
                        } catch (RemoteException e) {
                            binderDied();
        ...
        private void dump(PrintWriter pw) {
            ...
            while (it.hasNext()) {
                ...
                int device = next.getKey().intValue();
                ...
                pw.print(": ");
                int index = (next.getValue().intValue() + 5) / 10;
                pw.print(index);
                if (it.hasNext()) {
                    pw.print(", ");
                }
            ...
            switch (msg.what) {
                ...
                case 12:
                    ...
                    if (N > 0) {
                        ...
                        while (N > 0) {
                            N--;
                            IAudioRoutesObserver obs = (IAudioRoutesObserver) AudioService.this.mRoutesObservers.getBroadcastItem(N);
                            try {
                ...
                case 22:
                    ...
                    Settings.Secure.putIntForUser(AudioService.this.mContentResolver, Settings.Secure.UNSAFE_VOLUME_MUSIC_ACTIVE_MS, musicActiveMs, -2);
                    return;
                case 23:
    ...
    private void onSetWiredDeviceConnectionState(int device, int state, String name) {
        synchronized (this.mConnectedDevices) {
            if (state == 0 && (device == 4 || device == 8 || device == 131072)) {
                setBluetoothA2dpOnInt(true);
                if ((device & (-24577)) != 0) {
                    handleDeviceConnection(state == 1, device, !isUsb ? name : ProxyInfo.LOCAL_EXCL_LIST);
                    if (state == 0) {
                    }
                }
            } else {
                boolean isUsb = (device & (-24577)) != 0 || ((Integer.MIN_VALUE & device) != 0 && (2147477503 & device) == 0);
                handleDeviceConnection(state == 1, device, !isUsb ? name : ProxyInfo.LOCAL_EXCL_LIST);
                if (state == 0) {
                ...
                for (AudioPort port : ports) {
                    if (port instanceof AudioDevicePort) {
                        ...
                        if (devicePort.type() == 1024 || devicePort.type() == 262144) {
                            ...
                            int maxChannels = 0;
                            int[] arr$ = devicePort.channelMasks();
                            for (int mask : arr$) {
    ...
    private class AudioServiceBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(AudioManager.ACTION_USB_AUDIO_ACCESSORY_PLUG)) {
            ...
            if (action.equals(AudioManager.ACTION_USB_AUDIO_DEVICE_PLUG)) {
                int isDisabled = Settings.Secure.getInt(AudioService.this.mContentResolver, Settings.Secure.USB_AUDIO_AUTOMATIC_ROUTING_DISABLED, 0);
                if (isDisabled == 0) {
                    ...
                    String params = (alsaCard2 == -1 && alsaDevice2 == -1) ? ProxyInfo.LOCAL_EXCL_LIST : "card=" + alsaCard2 + ";device=" + alsaDevice2;
                    if (hasPlayback) {
    ...
    private boolean checkSafeMediaVolume(int streamType, int index, int device) {
        boolean z;
        synchronized (this.mSafeMediaVolumeState) {
            z = this.mSafeMediaVolumeState.intValue() != 3 || this.mStreamVolumeAlias[streamType] != 3 || (device & 12) == 0 || index <= this.mSafeMediaVolumeIndex;
        }
        return z;
    }
    ...
    public String registerAudioPolicy(AudioPolicyConfig policyConfig, IAudioPolicyCallback pcb, boolean hasFocusListener) {
        ...
        boolean hasPermissionForPolicy = this.mContext.checkCallingPermission(Manifest.permission.MODIFY_AUDIO_ROUTING) == 0;
    ...
    public class AudioPolicyProxy extends AudioPolicyConfig implements IBinder.DeathRecipient {
        ...
        AudioPolicyProxy(AudioPolicyConfig config, IAudioPolicyCallback token, boolean hasFocusListener) {
            ...
            setRegistration(new String(config.hashCode() + ":ap:" + AudioService.access$10408(AudioService.this)));
