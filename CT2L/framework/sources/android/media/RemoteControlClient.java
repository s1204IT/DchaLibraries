...
public class RemoteControlClient {
    ...
    private void onPositionDriftCheck() {
        synchronized (this.mCacheLock) {
            ...
            if (this.mPlaybackPositionMs >= 0 && this.mPlaybackSpeed != 0.0f) {
                long estPos = this.mPlaybackPositionMs + ((long) ((SystemClock.elapsedRealtime() - this.mPlaybackStateChangeTimeMs) / this.mPlaybackSpeed));
    ...
    private static long getCheckPeriodFromSpeed(float speed) {
        return Math.abs(speed) <= 1.0f ? POSITION_REFRESH_PERIOD_PLAYING_MS : Math.max((long) (15000.0f / Math.abs(speed)), POSITION_REFRESH_PERIOD_MIN_MS);
    }
}
