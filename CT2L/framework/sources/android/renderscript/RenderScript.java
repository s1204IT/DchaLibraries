...
public class RenderScript {
    ...
    synchronized void nAllocationData2D(long id, int xoff, int yoff, int mip, int face, int w, int h, Object d, int sizeBytes, Element.DataType dt) {
        validate();
    ...
    synchronized void nAllocationData3D(long id, int xoff, int yoff, int zoff, int mip, int w, int h, int depth, Object d, int sizeBytes, Element.DataType dt) {
        validate();
    ...
    synchronized void nAllocationRead1D(long id, int off, int mip, int count, Object d, int sizeBytes, Element.DataType dt) {
        validate();
        rsnAllocationRead1D(this.mContext, id, off, mip, count, d, sizeBytes, dt.mID);
    }

    synchronized void nAllocationRead2D(long id, int xoff, int yoff, int mip, int face, int w, int h, Object d, int sizeBytes, Element.DataType dt) {
        validate();
        ...
        public void run() {
            ...
            while (this.mRun) {
                ...
                if (msg == 4) {
                    ...
                    long bufferID = (((long) rbuf[1]) << 32) + (((long) rbuf[0]) & ExpandableListView.PACKED_POSITION_VALUE_NULL);
