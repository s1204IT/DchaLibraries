...
public final class Rect implements Parcelable {
    ...
    public void scaleRoundIn(float scale) {
        if (scale != 1.0f) {
            this.left = (int) Math.ceil(this.left * scale);
            this.top = (int) Math.ceil(this.top * scale);
            this.right = (int) Math.floor(this.right * scale);
            this.bottom = (int) Math.floor(this.bottom * scale);
        }
    }
}
