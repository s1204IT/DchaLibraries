...
public class VpnConfig implements Parcelable {
    ...
    public static CharSequence getVpnLabel(Context context, String packageName) throws PackageManager.NameNotFoundException {
        ...
        return (services == null || services.size() != 1) ? pm.getApplicationInfo(packageName, 0).loadLabel(pm) : services.get(0).loadLabel(pm);
    }
    ...
    public void addLegacyRoutes(String routesStr) {
        if (!routesStr.trim().equals(ProxyInfo.LOCAL_EXCL_LIST)) {
    ...
    public void addLegacyAddresses(String addressesStr) {
        if (!addressesStr.trim().equals(ProxyInfo.LOCAL_EXCL_LIST)) {
