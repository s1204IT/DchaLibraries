...
public class GsmAlphabet {
    ...
    public static String gsm8BitUnpackedToString(byte[] data, int offset, int length, String characterset) {
        ...
        while (true) {
            int i3 = i2;
            if (i3 >= offset + length || (c = data[i3] & BatteryStats.HistoryItem.CMD_NULL) == 255) {
                break;
            }
            if (c == 27) {
                if (prevWasEscape) {
                ...
                int c2 = c & 127;
                if (prevWasEscape) {
                    char shiftChar = c2 < shiftTableToChar.length() ? shiftTableToChar.charAt(c2) : ' ';
                    if (shiftChar == ' ') {
                        if (c2 < languageTableToChar.length()) {
                            ret.append(languageTableToChar.charAt(c2));
                        } else {
                            ret.append(' ');
                        }
                    } else {
                    ...
                    i = i3;
                } else if (!isMbcs || c2 < 128 || i3 + 1 >= offset + length) {
                    if (c2 < languageTableToChar.length()) {
                        ret.append(languageTableToChar.charAt(c2));
                        i = i3;
                    } else {
        ...
        for (int i = 0; i < sz && outByteIndex2 - offset < length; i++) {
            ...
            if (v == -1) {
                ...
                if (v == -1) {
                    ...
                    if ((outByteIndex2 + 1) - offset >= length) {
                        break;
                    }
