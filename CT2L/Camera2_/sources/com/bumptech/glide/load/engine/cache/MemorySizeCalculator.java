...
public class MemorySizeCalculator {
    ...
    MemorySizeCalculator(ActivityManager activityManager, ScreenDimensions screenDimensions) {
        ...
        if (targetMemoryCacheSize + targetPoolSize <= maxSize) {
            ...
            int part = Math.round(maxSize / 5.0f);
    ...
    private static int getMaxSize(ActivityManager activityManager) {
        ...
        return Math.round((isLowMemoryDevice ? LOW_MEMORY_MAX_SIZE_MULTIPLIER : MAX_SIZE_MULTIPLIER) * memoryClassBytes);
    }

    private static int toMb(int bytes) {
        return bytes / AccessibilityEventCompat.TYPE_TOUCH_INTERACTION_START;
    }
