...
public class Base64 {
    ...
    private static byte[] ascii = new byte[MotionEventCompat.ACTION_MASK];
    ...
    public static final byte[] encode(byte[] src, int lineFeed) {
        ...
        while (sidx + 3 <= src.length) {
            int sidx2 = sidx + 1;
            int bits24 = (src[sidx] & INVALID) << 16;
            int sidx3 = sidx2 + 1;
            int bits242 = bits24 | ((src[sidx2] & INVALID) << 8);
            int sidx4 = sidx3 + 1;
            int bits243 = bits242 | ((src[sidx3] & INVALID) << 0);
            int bits6 = (bits243 & 16515072) >> 18;
            int didx2 = didx + 1;
            dst[didx] = base64[bits6];
            int bits62 = (bits243 & 258048) >> 12;
            int didx3 = didx2 + 1;
            dst[didx2] = base64[bits62];
            int bits63 = (bits243 & 4032) >> 6;
            int didx4 = didx3 + 1;
            dst[didx3] = base64[bits63];
            int bits64 = bits243 & 63;
            didx = didx4 + 1;
            dst[didx4] = base64[bits64];
            lf += 4;
        ...
        if (src.length - sidx == 2) {
            int bits244 = ((src[sidx] & INVALID) << 16) | ((src[sidx + 1] & INVALID) << 8);
            int bits65 = (bits244 & 16515072) >> 18;
            int didx5 = didx + 1;
            dst[didx] = base64[bits65];
            int bits66 = (bits244 & 258048) >> 12;
            int didx6 = didx5 + 1;
            dst[didx5] = base64[bits66];
            int bits67 = (bits244 & 4032) >> 6;
            int didx7 = didx6 + 1;
            dst[didx6] = base64[bits67];
            int i = didx7 + 1;
            dst[didx7] = 61;
        } else if (src.length - sidx == 1) {
            int bits245 = (src[sidx] & INVALID) << 16;
            int bits68 = (bits245 & 16515072) >> 18;
            int didx8 = didx + 1;
            dst[didx] = base64[bits68];
            int bits69 = (bits245 & 258048) >> 12;
            int didx9 = didx8 + 1;
            dst[didx8] = base64[bits69];
            int didx10 = didx9 + 1;
            dst[didx9] = 61;
            int i2 = didx10 + 1;
            dst[didx10] = 61;
    ...
    public static final byte[] decode(byte[] src) throws IllegalArgumentException {
        ...
        while (didx < dst.length - 2) {
            dst[didx] = (byte) (((src[sidx] << 2) & MotionEventCompat.ACTION_MASK) | ((src[sidx + 1] >>> 4) & 3));
            dst[didx + 1] = (byte) (((src[sidx + 1] << 4) & MotionEventCompat.ACTION_MASK) | ((src[sidx + 2] >>> 2) & 15));
            dst[didx + 2] = (byte) (((src[sidx + 2] << 6) & MotionEventCompat.ACTION_MASK) | (src[sidx + 3] & 63));
            sidx += 4;
            didx += 3;
        }
        if (didx < dst.length) {
        ...
        int didx2 = didx + 1;
        if (didx2 < dst.length) {
            dst[didx2] = (byte) (((src[sidx + 1] << 4) & MotionEventCompat.ACTION_MASK) | ((src[sidx + 2] >>> 2) & 15));
        }
        return dst;
    }
