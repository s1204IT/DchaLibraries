...
final class WirelessChargerDetector {
    ...
    public boolean update(boolean isPowered, int plugType, int batteryLevel) {
        ...
        synchronized (this.mLock) {
            ...
            z = this.mPoweredWirelessly && !wasPoweredWirelessly && batteryLevel < WIRELESS_CHARGER_TURN_ON_BATTERY_LEVEL_LIMIT && !this.mAtRest;
        }
        return z;
    }
    ...
    private static boolean hasMoved(float x1, float y1, float z1, float x2, float y2, float z2) {
        ...
        if (mag1 < MIN_GRAVITY || mag1 > MAX_GRAVITY || mag2 < MIN_GRAVITY || mag2 > MAX_GRAVITY || dotProduct < mag1 * mag2 * MOVEMENT_ANGLE_COS_THRESHOLD) {
            return true;
        }
        return DEBUG;
    }
