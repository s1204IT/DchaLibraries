...
class UserUsageStatsService {
    ...
    private boolean mStatsChanged = DEBUG;
    ...
    void init(long currentTimeMillis) {
        ...
        IntervalStats[] arr$ = this.mCurrentStats;
        for (IntervalStats stat : arr$) {
    ...
    private <T> List<T> queryStats(int intervalType, long beginTime, long endTime, UsageStatsDatabase.StatCombiner<T> combiner) {
        ...
        if (intervalType >= 0 && intervalType < this.mCurrentStats.length) {
            ...
            if (beginTime < currentStats.endTime) {
                ...
                results = this.mDatabase.queryUsageStats(intervalType, beginTime, truncatedEndTime, combiner);
    ...
    UsageEvents queryEvents(final long beginTime, final long endTime) {
        ...
        List<UsageEvents.Event> results = queryStats(0, beginTime, endTime, new UsageStatsDatabase.StatCombiner<UsageEvents.Event>() {
            ...
            public void combine(IntervalStats stats, boolean mutable, List<UsageEvents.Event> accumulatedResult) {
                int startIndex;
                if (stats.events != null && (startIndex = stats.events.closestIndexOnOrAfter(beginTime)) >= 0) {
                    int size = stats.events.size();
                    for (int i = startIndex; i < size && stats.events.keyAt(i) < endTime; i++) {
    ...
    private void rolloverStats(long currentTimeMillis) {
        ...
        for (int i2 = 0; i2 < continueCount; i2++) {
            String name = continuePreviousDay.valueAt(i2);
            long beginTime = this.mCurrentStats[0].beginTime;
            IntervalStats[] arr$2 = this.mCurrentStats;
            for (IntervalStats stat2 : arr$2) {
    ...
    void checkin(final IndentingPrintWriter pw) {
        this.mDatabase.checkinDailyFiles(new UsageStatsDatabase.CheckinAction() {
            ...
            public boolean checkin(IntervalStats stats) {
                UserUsageStatsService.this.printIntervalStats(pw, stats, UserUsageStatsService.DEBUG);
                return true;
            }
        });
    }
    ...
    private String formatDateTime(long dateTime, boolean pretty) {
        return pretty ? "\"" + DateUtils.formatDateTime(this.mContext, dateTime, sDateFormatFlags) + "\"" : Long.toString(dateTime);
    }

    private String formatElapsedTime(long elapsedTime, boolean pretty) {
        return pretty ? "\"" + DateUtils.formatElapsedTime(elapsedTime / 1000) + "\"" : Long.toString(elapsedTime);
    }
    ...
    void printIntervalStats(IndentingPrintWriter pw, IntervalStats stats, boolean prettyDates) {
        ...
        for (int i2 = 0; i2 < configCount; i2++) {
            ConfigurationStats config = configStats.valueAt(i2);
        ...
        pw.println("events");
        pw.increaseIndent();
        TimeSparseArray<UsageEvents.Event> events = stats.events;
        int eventCount = events != null ? events.size() : 0;
        for (int i3 = 0; i3 < eventCount; i3++) {
            UsageEvents.Event event = (UsageEvents.Event) events.valueAt(i3);
