...
public class DozeParameters {
    ...
    public void dump(PrintWriter pw) {
        ...
        pw.print("    getPulseInDelay(pickup=false): ");
        pw.println(getPulseInDelay(false));
        pw.print("    getPulseInDelay(pickup=true): ");
        pw.println(getPulseInDelay(true));
    ...
    public int getPulseInDuration(boolean pickup) {
        return pickup ? getInt("doze.pulse.duration.in.pickup", R.integer.doze_pulse_duration_in_pickup) : getInt("doze.pulse.duration.in", R.integer.doze_pulse_duration_in);
    }

    public int getPulseInDelay(boolean pickup) {
        return pickup ? getInt("doze.pulse.delay.in.pickup", R.integer.doze_pulse_delay_in_pickup) : getInt("doze.pulse.delay.in", R.integer.doze_pulse_delay_in);
    }
    ...
    private int getInt(String propName, int resId) {
        ...
        return MathUtils.constrain(value, 0, 10000);
    }
    ...
    public static class PulseSchedule {
        ...
        public long getNextTime(long now, long notificationTime) {
            for (int i = 0; i < this.mSchedule.length; i++) {
                long time = notificationTime + ((long) (this.mSchedule[i] * 1000));
                if (time > now) {
                    return time;
                }
            }
