...
class GlobalScreenshot {
    ...
    public GlobalScreenshot(Context context) {
        ...
        this.mWindowLayoutParams = new WindowManager.LayoutParams(-1, -1, 0, 0, 2015, android.R.drawable.ic_media_route_connecting_dark_material, -3);
    ...
    public void saveScreenshotInWorkerThread(Runnable finisher) {
        ...
        this.mSaveInBgTask = new SaveImageInBackgroundTask(this.mContext, data, this.mNotificationManager, 789).execute(data);
    }
    ...
    private ValueAnimator createScreenshotDropInAnimation() {
        final Interpolator flashAlphaInterpolator = new Interpolator() {
            ...
            public float getInterpolation(float x) {
                if (x <= 0.60465115f) {
                    return (float) Math.sin(3.141592653589793d * ((double) (x / 0.60465115f)));
                }
                return 0.0f;
            }
        };
        ...
        anim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                float scaleT = (1.0f + GlobalScreenshot.this.mBgPaddingScale) - (scaleInterpolator.getInterpolation(t) * 0.27499998f);
    ...
    private ValueAnimator createScreenshotDropOutAnimation(int w, int h, boolean statusBarVisible, boolean navBarVisible) {
        ...
        if (!statusBarVisible || !navBarVisible) {
            ...
            float halfScreenWidth = (w - (2.0f * this.mBgPadding)) / 2.0f;
            float halfScreenHeight = (h - (2.0f * this.mBgPadding)) / 2.0f;
            ...
            anim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    float t = ((Float) animation.getAnimatedValue()).floatValue();
                    float scaleT = (0.725f + GlobalScreenshot.this.mBgPaddingScale) - (scaleInterpolator.getInterpolation(t) * 0.27500004f);
                    GlobalScreenshot.this.mBackgroundView.setAlpha((1.0f - t) * 0.5f);
    ...
    static void notifyScreenshotError(Context context, NotificationManager nManager) {
        Resources r = context.getResources();
        Notification.Builder b = new Notification.Builder(context).setTicker(r.getString(R.string.screenshot_failed_title)).setContentTitle(r.getString(R.string.screenshot_failed_title)).setContentText(r.getString(R.string.screenshot_failed_text)).setSmallIcon(R.drawable.stat_notify_image_error).setWhen(System.currentTimeMillis()).setVisibility(1).setCategory("err").setAutoCancel(true).setColor(context.getResources().getColor(android.R.color.system_accent3_600));
        Notification n = new Notification.BigTextStyle(b).bigText(r.getString(R.string.screenshot_failed_text)).build();
        nManager.notify(789, n);
    }
}
