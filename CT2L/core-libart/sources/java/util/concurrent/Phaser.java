...
public class Phaser {
    ...
    private int doArrive(int adjust) {
        ...
        do {
            ...
            s2 = s - ((long) adjust);
        } while (!UNSAFE.compareAndSwapLong(this, stateOffset, s, s2));
        if (unarrived == 1) {
            ...
            if (root == this) {
                ...
                UNSAFE.compareAndSwapLong(this, stateOffset, s2, n | (((long) nextPhase) << 32));
                releaseWaiters(phase);
            } else if (nextUnarrived == 0) {
                phase = this.parent.doArrive(ONE_DEREGISTER);
                UNSAFE.compareAndSwapLong(this, stateOffset, s2, s2 | 1);
            } else {
            ...
            if (counts != 1) {
                ...
                synchronized (this) {
                    if (this.state == s) {
                        ...
                        if (phase >= 0) {
                            while (!UNSAFE.compareAndSwapLong(this, stateOffset, s, (((long) phase) << 32) | adjust)) {
                                s = this.state;
    ...
    private long reconcileState() {
        ...
        while (true) {
            ...
            if (phase < 0) {
                ...
                int p = ((int) s) >>> 16;
                j = p == 0 ? 1L : (PARTIES_MASK & s) | ((long) p);
            }
            long s2 = j3 | j;
    ...
    public int bulkRegister(int parties) {
        ...
        return parties == 0 ? getPhase() : doRegister(parties);
    }
    ...
    public int arriveAndAwaitAdvance() {
        ...
        int nextPhase = (phase + 1) & Integer.MAX_VALUE;
        if (!UNSAFE.compareAndSwapLong(this, stateOffset, s2, n | (((long) nextPhase) << 32))) {
    ...
    public int awaitAdvance(int phase) {
        ...
        return p == phase ? root.internalAwaitAdvance(phase, null) : p;
    }
    ...
    public void forceTermination() {
        ...
        } while (!UNSAFE.compareAndSwapLong(root, stateOffset, s, Long.MIN_VALUE | s));
        releaseWaiters(0);
        releaseWaiters(1);
    }
