...
public class BigDecimal extends Number implements Comparable<BigDecimal>, Serializable {
    ...
    public static BigDecimal valueOf(long unscaledVal) {
        return (unscaledVal < 0 || unscaledVal >= 11) ? new BigDecimal(unscaledVal, 0) : BI_SCALED_BY_ZERO[(int) unscaledVal];
    }
    ...
    private static int roundingBehavior(int parityBit, int fraction, RoundingMode roundingMode) {
        switch (roundingMode) {
            case UNNECESSARY:
                ...
                throw new ArithmeticException("Rounding necessary");
            case UP:
                int increment = Integer.signum(fraction);
                return increment;
            case DOWN:
            default:
                return 0;
            case CEILING:
                int increment2 = Math.max(Integer.signum(fraction), 0);
                return increment2;
            case FLOOR:
                int increment3 = Math.min(Integer.signum(fraction), 0);
                return increment3;
            case HALF_UP:
    ...
    private int approxPrecision() {
        return this.precision > 0 ? this.precision : ((int) (((double) (this.bitLength - 1)) * LOG10_2)) + 1;
    }
