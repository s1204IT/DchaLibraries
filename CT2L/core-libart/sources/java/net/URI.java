...
public final class URI implements Comparable<URI>, Serializable {
    ...
    private void parseAuthority(boolean forceServer) throws URISyntaxException {
        ...
        if (this.authority != null) {
            ...
            int index2 = temp.lastIndexOf(58);
            int endIndex = temp.indexOf(93);
            int tempPort = -1;
            if (index2 != -1 && endIndex < index2) {
                tempHost = temp.substring(0, index2);
                if (index2 < temp.length() - 1) {
                    try {
                        char firstPortChar = temp.charAt(index2 + 1);
                        if (firstPortChar >= '0' && firstPortChar <= '9') {
                            tempPort = Integer.parseInt(temp.substring(index2 + 1));
                        } else {
                            if (forceServer) {
                                throw new URISyntaxException(this.authority, "Invalid port number", hostIndex + index2 + 1);
                            }
                            return;
                        }
    ...
    public int compareTo(URI uri) {
        ...
        if (this.scheme == null || uri.scheme == null || (ret4 = this.scheme.compareToIgnoreCase(uri.scheme)) == 0) {
            ...
            if (this.opaque && uri.opaque) {
                int ret5 = this.schemeSpecificPart.compareTo(uri.schemeSpecificPart);
                if (ret5 != 0) {
                    return ret5;
                }
            } else {
                ...
                if (this.authority != null && uri.authority != null) {
                    if (this.host != null && uri.host != null) {
                        ...
                        if (this.userInfo == null || uri.userInfo == null || (ret2 = this.userInfo.compareTo(uri.userInfo)) == 0) {
                            int ret6 = this.host.compareToIgnoreCase(uri.host);
                            if (ret6 == 0) {
                        ...
                        int ret7 = this.authority.compareTo(uri.authority);
                        if (ret7 != 0) {
                            return ret7;
                        }
                    }
                }
                int ret8 = this.path.compareTo(uri.path);
                if (ret8 == 0) {
            ...
            if (this.fragment == null || uri.fragment == null || (ret3 = this.fragment.compareTo(uri.fragment)) == 0) {
                return 0;
            }
            return ret3;
        }
    ...
    private boolean escapedEquals(String first, String second) {
        ...
        while (true) {
            ...
            int index1 = second.indexOf(37, prevIndex);
            if (index != index1) {
                return false;
            }
            if (index == -1) {
            ...
            if (first.regionMatches(prevIndex, second, prevIndex, index - prevIndex) && first.regionMatches(true, index + 1, second, index + 1, 2)) {
                prevIndex = index + 3;
            }
            return false;
        }
    ...
    private String normalize(String path, boolean discardRelativePrefix) {
        String path2 = UrlUtils.canonicalizePath(path, discardRelativePrefix);
        int colon = path2.indexOf(58);
        if (colon != -1) {
            int slash = path2.indexOf(47);
    ...
    public URI resolve(URI relative) {
        ...
        if (relative.path.isEmpty() && relative.scheme == null && relative.query == null) {
            URI result2 = duplicate();
            result2.fragment = relative.fragment;
            return result2;
        }
        URI result3 = duplicate();
        result3.fragment = relative.fragment;
        result3.query = relative.query;
        ...
        result3.path = UrlUtils.authoritySafePath(result3.authority, normalize(resolvedPath, true));
        result3.setSchemeSpecificPart();
        return result3;
    }
