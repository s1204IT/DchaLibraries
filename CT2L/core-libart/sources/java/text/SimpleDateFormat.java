...
public class SimpleDateFormat extends DateFormat {
    ...
    private void append(StringBuffer buffer, FieldPosition position, List<FieldPosition> fields, char format, int count) {
        ...
        switch (index) {
            ...
            case 15:
                ...
                int hour2 = this.calendar.get(10);
                if (hour2 == 0) {
                    hour2 = 12;
                }
                appendNumber(buffer, count, hour2);
                break;
            case 16:
        ...
        if (fields != null) {
            FieldPosition position2 = new FieldPosition(dateFormatField);
            position2.setBeginIndex(beginPosition);
            position2.setEndIndex(buffer.length());
            fields.add(position2);
            return;
        }
    ...
    private void appendDayOfWeek(StringBuffer buffer, int count, boolean standAlone) {
        ...
        if (count == 4) {
            days = standAlone ? ld.longStandAloneWeekdayNames : this.formatData.weekdays;
        } else if (count == 5) {
            days = standAlone ? ld.tinyStandAloneWeekdayNames : this.formatData.localeData.tinyWeekdayNames;
        } else {
            days = standAlone ? ld.shortStandAloneWeekdayNames : this.formatData.shortWeekdays;
        }
    ...
    private void appendMonth(StringBuffer buffer, int count, boolean standAlone) {
        ...
        LocaleData ld = this.formatData.localeData;
        if (count == 4) {
            months = standAlone ? ld.longStandAloneMonthNames : this.formatData.months;
        } else if (count == 5) {
            months = standAlone ? ld.tinyStandAloneMonthNames : ld.tinyMonthNames;
        } else {
            months = standAlone ? ld.shortStandAloneMonthNames : this.formatData.shortMonths;
        }
    ...
    private void appendMilliseconds(StringBuffer buffer, int count, int value) {
        this.numberFormat.setMinimumIntegerDigits(count > 3 ? 3 : count);
        ...
        if (count > 3) {
            ...
            this.numberFormat.format((Object) 0, buffer, p);
        }
    }
    ...
    private int parse(String string, int offset, char format, int count) {
        ...
        switch (index) {
            ...
            case 3:
                ...
                break;
            case 4:
                ParsePosition position2 = new ParsePosition(offset);
                Number result2 = parseNumber(absolute, string, position2);
                if (result2 == null) {
                    return (-position2.getErrorIndex()) - 1;
                }
                int hour = result2.intValue();
                if (hour == 24) {
                    hour = 0;
                }
                this.calendar.set(11, hour);
                return position2.getIndex();
            case 5:
                field = 11;
                break;
            case 6:
            ...
            case 15:
                ParsePosition position3 = new ParsePosition(offset);
                Number result3 = parseNumber(absolute, string, position3);
                if (result3 == null) {
                    return (-position3.getErrorIndex()) - 1;
                }
                int hour2 = result3.intValue();
                if (hour2 == 12) {
                    hour2 = 0;
                }
                this.calendar.set(10, hour2);
                return position3.getIndex();
            case 16:
                field = 10;
                break;
            case 17:
    ...
    private int parseFractionalSeconds(String string, int offset, int count) {
        ...
        double result = fractionalSeconds.doubleValue();
        int numDigitsParsed = parsePosition.getIndex() - offset;
        double divisor = Math.pow(10.0d, numDigitsParsed);
        this.calendar.set(14, (int) ((result / divisor) * 1000.0d));
        ...
        for (int i = 0; i < patternLength; i++) {
            ...
            if (next == 39) {
                ...
                if (last == next) {
                    ...
                    if (count > 0 && (offset = parse(string, offset, (char) last, -count)) < 0) {
                        return error(position, (-offset) - 1, zone);
                    }
                    last = next;
                    count = 1;
    ...
    private int parseText(String string, int offset, String[] options, int field) {
        ...
        for (int i = 0; i < options.length; i++) {
            ...
            if (optionLength != 0) {
                ...
                } else if (option.charAt(optionLength - 1) == '.' && string.regionMatches(true, offset, option, 0, optionLength - 1) && (bestIndex == -1 || optionLength - 1 > bestLength)) {
                    bestIndex = i;
    ...
    private int parseTimeZone(String string, int offset) {
        ...
        if (offset < string.length() && ((sign = string.charAt(offset)) == '+' || sign == '-')) {
            ...
            int raw = hour * Grego.MILLIS_PER_HOUR;
            int index = position.getIndex();
            if (index < string.length() && string.charAt(index) == ':') {
                position.setIndex(index + 1);
                Number result2 = this.numberFormat.parse(string, position);
                if (result2 == null) {
                    return (-position.getErrorIndex()) - 1;
                }
                int minute = result2.intValue();
                raw += Grego.MILLIS_PER_MINUTE * minute;
            } else if (hour >= 24) {
        ...
        String[][] arr$ = this.formatData.internalZoneStrings();
        for (String[] row : arr$) {
            for (int i = 1; i < 5; i++) {
                if (row[i] != null) {
                    if (string.regionMatches(true, offset, row[i], 0, row[i].length())) {
                        ...
                        this.calendar.setTimeZone(new SimpleTimeZone(raw2, ""));
    ...
    private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException {
        ...
        this.formatData = (DateFormatSymbols) fields.get("formatData", (Object) null);
