    ...
    private int lookupEntity(char[] buff, int offset, int length) {
        ...
        if (buff[offset] == '#') {
            if (length > 1 && (buff[offset + 1] == 'x' || buff[offset + 1] == 'X')) {
                try {
                    int result = Integer.parseInt(new String(buff, offset + 2, length - 2), 16);
                    return result;
            ...
            try {
                int result2 = Integer.parseInt(new String(buff, offset + 1, length - 1), 10);
                return result2;
            } catch (NumberFormatException e2) {
                return 0;
            }
        }
        int result3 = this.theSchema.getEntity(new String(buff, offset, length));
        return result3;
    }
