...
public class PrefixTimeZonesMap implements Externalizable {
    ...
    private List<String> lookupTimeZonesForNumber(long key) {
        ...
        return timezonesString == null ? new LinkedList() : tokenizeRawOutputString(timezonesString);
    }

    public List<String> lookupTimeZonesForNumber(Phonenumber.PhoneNumber number) {
        long phonePrefix = Long.parseLong(number.getCountryCode() + PhoneNumberUtil.getInstance().getNationalSignificantNumber(number));
