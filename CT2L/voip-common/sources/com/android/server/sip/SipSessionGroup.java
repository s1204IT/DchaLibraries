...
class SipSessionGroup implements SipListener {
    ...
    synchronized void reset() throws SipException {
        ...
        String local = null;
        try {
            InetAddress[] arr$ = InetAddress.getAllByName(server);
            int len$ = arr$.length;
            int i$ = 0;
            while (true) {
                if (i$ >= len$) {
        ...
        if (bytes != null) {
            try {
                if (message instanceof SIPMessage) {
                    str = ((SIPMessage) message).getMessageContent();
                } else {
    ...
    class SipSessionImpl extends ISipSession.Stub {
        ...
        private boolean processExceptions(EventObject evt) throws SipException {
            ...
            } else if (isCurrentTransaction((TransactionTerminatedEvent) evt)) {
            ...
            if (SipSessionGroup.expectResponse("REGISTER", evt)) {
                ...
                switch (statusCode) {
                    case 200:
                        ...
                        onRegistrationDone(state == 1 ? getExpiryTime(((ResponseEvent) evt).getResponse()) : -1);
                        return SSI_DBG;
                    case 401:
                    case 407:
        ...
        private boolean crossDomainAuthenticationRequired(Response response) {
            ...
            if (SipSessionGroup.this.mLocalProfile.getSipDomain().trim().equals(realm.trim())) {
                return false;
            }
            return SSI_DBG;
        }
        ...
        private boolean readyForCall(EventObject evt) throws SipException {
            ...
            if (!(evt instanceof RegisterCommand)) {
                if (SipSessionGroup.DEREGISTER != evt) {
                    return false;
                }
                this.mState = 2;
        ...
        private boolean outgoingCall(EventObject evt) throws SipException {
            ...
            ResponseEvent event2 = (ResponseEvent) evt;
            Response response = event2.getResponse();
            int statusCode = response.getStatusCode();
            switch (statusCode) {
                case 180:
                case 181:
                ...
                case 200:
                    ...
                    SipSessionGroup.this.mSipHelper.sendInviteAck(event2, this.mDialog);
                    this.mPeerSessionDescription = SipSessionGroup.this.extractContent(response);
                    establishCall(SSI_DBG);
                    break;
                case 401:
                case 407:
                    if (handleAuthentication(event2)) {
                        SipSessionGroup.this.addSipSession(this);
                    }
                    break;
                case 491:
        ...
        private boolean inCall(EventObject evt) throws SipException {
            if (SipSessionGroup.END_CALL != evt) {
                if (!SipSessionGroup.isRequestEvent("INVITE", evt)) {
                    ...
                    if ((evt instanceof ResponseEvent) && SipSessionGroup.expectResponse("NOTIFY", evt)) {
                        return SSI_DBG;
                    }
                    return false;
                }
    ...
    private static boolean isLoggable(SipSessionImpl s, EventObject evt) {
        if (isLoggable(s) && evt != null) {
            ...
            } else if (!(evt instanceof RequestEvent) || isRequestEvent("OPTIONS", evt)) {
            }
        }
        return false;
    }
