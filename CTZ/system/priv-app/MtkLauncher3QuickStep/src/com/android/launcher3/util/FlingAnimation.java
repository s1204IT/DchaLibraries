...
public class FlingAnimation implements ValueAnimator.AnimatorUpdateListener, Runnable {
    ...
    protected int initFlingUpDuration() {
        ...
        double dSqrt = ((-this.mUY) - Math.sqrt(f2)) / this.mAY;
        this.mAX = (float) (((((-this.mFrom.exactCenterX()) + this.mIconRect.exactCenterX()) - (this.mUX * dSqrt)) * 2.0d) / (dSqrt * dSqrt));
    ...
    protected int initFlingLeftDuration() {
        ...
        double dSqrt = ((-this.mUX) - Math.sqrt(f2)) / this.mAX;
        this.mAY = (float) (((((-this.mFrom.exactCenterY()) + this.mIconRect.exactCenterY()) - (this.mUY * dSqrt)) * 2.0d) / (dSqrt * dSqrt));
        return (int) Math.round(dSqrt);
    }
