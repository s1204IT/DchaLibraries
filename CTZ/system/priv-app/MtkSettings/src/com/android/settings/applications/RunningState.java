...
public class RunningState {
    ...
    static class ProcessItem extends BaseItem {
        ...
        void ensureLabel(PackageManager packageManager) throws PackageManager.NameNotFoundException {
        ...
        boolean buildDependencyChain(Context context, PackageManager packageManager, int i) throws PackageManager.NameNotFoundException {
    ...
    static class MergedItem extends BaseItem {
        ...
        public Drawable loadIcon(Context context, RunningState runningState) {
            ...
            return context.getDrawable(android.R.drawable.ic_lockscreen_unlock_normal);
        }
    }
    ...
    /* JADX DEBUG: Don't trust debug lines info. Repeating lines: [1325=4, 1364=6] */
    ...
    */
    private boolean update(Context context, ActivityManager activityManager) throws PackageManager.NameNotFoundException, SecurityException {
        int i;
        boolean z;
        long j;
        long j2;
        ...
        if (zBuildDependencyChain) {
            ...
            for (int i20 = 0; i20 < arrayList4.size(); i20++) {
                ...
                MergedItem mergedItem3 = new MergedItem(processItem8.mUserId);
                for (ServiceItem serviceItem3 : processItem8.mServices.values()) {
                    mergedItem3.mServices.add(serviceItem3);
                    serviceItem3.mMergedItem = mergedItem3;
                }
                mergedItem3.mProcess = processItem8;
                mergedItem3.mOtherProcesses.clear();
                for (int size6 = this.mProcessItems.size(); size6 < this.mProcessItems.size() - 1; size6++) {
                    mergedItem3.mOtherProcesses.add(this.mProcessItems.get(size6));
                }
                mergedItem3.update(context, false);
                if (mergedItem3.mUserId != this.mMyUserId) {
                    addOtherUserItem(context, arrayList6, this.mOtherUserMergedItems, mergedItem3);
                } else {
            ...
            if (processItem.mCurSeq == this.mSequence) {
                ...
                if (processItem.mRunningProcessInfo.importance >= 400) {
                    ...
                    if (arrayList != null) {
                        ...
                        while (i29 < i) {
                            try {
                                MergedItem mergedItem4 = this.mBackgroundItems.get(i29);
                                int[] iArr3 = iArr;
                                z = zUpdateSize;
                                try {
                                    i28 |= mergedItem4.mUserId != this.mMyUserId ? 1 : 0;
                                    arrayList7.add(mergedItem4);
                                    i29++;
                                    iArr = iArr3;
                                    zUpdateSize = z;
