...
class BackgroundTaskLoader implements Runnable {
    ...
    private void processLoadQueueItem() {
        final Task taskNextTask = this.mLoadQueue.nextTask();
        if (taskNextTask != null) {
            final Drawable icon = this.mIconLoader.getIcon(taskNextTask);
            if (DEBUG) {
                Log.d(TAG, "Loading thumbnail: " + taskNextTask.key);
            }
            final ThumbnailData taskThumbnail = ActivityManagerWrapper.getInstance().getTaskThumbnail(taskNextTask.key.id, true);
            if (!this.mCancelled) {
                this.mMainThreadHandler.post(new Runnable() { // from class: com.android.systemui.shared.recents.model.-$$Lambda$BackgroundTaskLoader$mJeiv3P4w5EJwXqKPoDi48s7tFI
                    @Override // java.lang.Runnable
                    public final void run() {
                        taskNextTask.notifyTaskDataLoaded(taskThumbnail, icon);
                    }
                });
            }
        }
